...
public class IScoreboardCriteria {
    ...
    public static Optional<IScoreboardCriteria> byName(String s) {
        ...
        if (iscoreboardcriteria != null) {
            ...
            return i < 0 ? Optional.empty() : BuiltInRegistries.STAT_TYPE.getOptional(MinecraftKey.bySeparator(s.substring(0, i), '.')).flatMap((statisticwrapper) -> {
                return getStat(statisticwrapper, MinecraftKey.bySeparator(s.substring(i + 1), '.'));
            });
        }
    }
    ...
    public static enum EnumScoreboardHealthDisplay implements INamable {
        ...
        public static final INamable.a<IScoreboardCriteria.EnumScoreboardHealthDisplay> CODEC = INamable.<IScoreboardCriteria.EnumScoreboardHealthDisplay>fromEnum(IScoreboardCriteria.EnumScoreboardHealthDisplay::values);

        private EnumScoreboardHealthDisplay(final String s) {
            this.id = s;
        }
