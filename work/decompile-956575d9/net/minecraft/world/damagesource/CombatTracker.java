    ...
    public IChatBaseComponent getDeathMessage() {
        if (this.entries.isEmpty()) {
            ...
            if (deathmessagetype == DeathMessageType.FALL_VARIANTS && combatentry1 != null) {
                ...
                IChatBaseComponent ichatbasecomponent = ChatComponentUtils.wrapInSquareBrackets(IChatBaseComponent.translatable(s + ".link")).withStyle(CombatTracker.INTENTIONAL_GAME_DESIGN_STYLE);

                return IChatBaseComponent.translatable(s + ".message", this.mob.getDisplayName(), ichatbasecomponent);
            } else {
