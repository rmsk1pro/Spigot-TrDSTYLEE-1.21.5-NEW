...
public record NextTickListEntry<T>(T type, BlockPosition pos, long triggerTick, TickListPriority priority, long subTickOrder) {
    ...
    public static <T> NextTickListEntry<T> probe(T t0, BlockPosition blockposition) {
        return new NextTickListEntry<T>(t0, blockposition, 0L, TickListPriority.NORMAL, 0L);
    }

    public TickListChunk<T> toSavedTick(long i) {
        return new TickListChunk<T>(this.type, this.pos, (int) (this.triggerTick - i), this.priority);
    }
}
