...
public class MobEffect implements Comparable<MobEffect> {
    ...
    public MobEffect withScaledDuration(float f) {
        MobEffect mobeffect = new MobEffect(this);

        mobeffect.duration = mobeffect.mapDuration((i) -> {
            return Math.max(MathHelper.floor((float) i * f), 1);
        });
        return mobeffect;
    }
    ...
    public boolean tickServer(WorldServer worldserver, EntityLiving entityliving, Runnable runnable) {
        if (!this.hasRemainingDuration()) {
            return false;
        } else {
            int i = this.isInfiniteDuration() ? entityliving.tickCount : this.duration;

            if (((MobEffectList) this.effect.value()).shouldApplyEffectTickThisTick(i, this.amplifier) && !((MobEffectList) this.effect.value()).applyEffectTick(worldserver, entityliving, this.amplifier)) {
                return false;
            } else {
    ...
    public void onMobRemoved(WorldServer worldserver, EntityLiving entityliving, Entity.RemovalReason entity_removalreason) {
        ((MobEffectList) this.effect.value()).onMobRemoved(worldserver, entityliving, this.amplifier, entity_removalreason);
    }

    public void onMobHurt(WorldServer worldserver, EntityLiving entityliving, DamageSource damagesource, float f) {
        ((MobEffectList) this.effect.value()).onMobHurt(worldserver, entityliving, this.amplifier, damagesource, f);
    }
    ...
    private static record b(int amplifier, int duration, boolean ambient, boolean showParticles, boolean showIcon, Optional<MobEffect.b> hiddenEffect) {
        ...
        public static final StreamCodec<ByteBuf, MobEffect.b> STREAM_CODEC = StreamCodec.<ByteBuf, MobEffect.b>recursive((streamcodec) -> {
    ...
    private static class a {
        ...
        public void setImmediate(MobEffect mobeffect) {
            this.factor = hasEffect(mobeffect) ? 1.0F : 0.0F;
        ...
        public void tick(MobEffect mobeffect) {
            this.factorPreviousFrame = this.factor;
            boolean flag = hasEffect(mobeffect);
            float f = flag ? 1.0F : 0.0F;

            if (this.factor != f) {
                MobEffectList mobeffectlist = (MobEffectList) mobeffect.getEffect().value();
                int i = flag ? mobeffectlist.getBlendInDurationTicks() : mobeffectlist.getBlendOutDurationTicks();
        ...
        private static boolean hasEffect(MobEffect mobeffect) {
            return !mobeffect.endsWithin(((MobEffectList) mobeffect.getEffect().value()).getBlendOutAdvanceTicks());
        }
