...
public final class CustomData {
    ...
    public MinecraftKey parseEntityId() {
        return (MinecraftKey) this.tag.read("id", MinecraftKey.CODEC).orElse((Object) null);
    }

    @Nullable
    public <T> T parseEntityType(HolderLookup.a holderlookup_a, ResourceKey<? extends IRegistry<T>> resourcekey) {
        MinecraftKey minecraftkey = this.parseEntityId();

        return (T) (minecraftkey == null ? null : holderlookup_a.lookup(resourcekey).flatMap((holderlookup_b) -> {
            return holderlookup_b.get(ResourceKey.create(resourcekey, minecraftkey));
    ...
    public <T> DataResult<T> read(MapDecoder<T> mapdecoder) {
        return this.<T>read(DynamicOpsNBT.INSTANCE, mapdecoder);
    }
