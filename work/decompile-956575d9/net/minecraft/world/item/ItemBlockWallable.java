...
public class ItemBlockWallable extends ItemBlock {
    ...
    public ItemBlockWallable(Block block, Block block1, EnumDirection enumdirection, Item.Info item_info) {
    ...
    protected IBlockData getPlacementState(BlockActionContext blockactioncontext) {
        ...
        for (EnumDirection enumdirection : blockactioncontext.getNearestLookingDirections()) {
            if (enumdirection != this.attachmentDirection.getOpposite()) {
                ...
                if (iblockdata2 != null && this.canPlace(iworldreader, iblockdata2, blockposition)) {
