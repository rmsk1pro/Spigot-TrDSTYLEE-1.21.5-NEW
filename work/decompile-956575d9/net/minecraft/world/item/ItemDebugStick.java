...
public class ItemDebugStick extends Item {
    ...
    public boolean canDestroyBlock(ItemStack itemstack, IBlockData iblockdata, World world, BlockPosition blockposition, EntityLiving entityliving) {
        if (!world.isClientSide && entityliving instanceof EntityHuman entityhuman) {
    ...
    public boolean handleInteraction(EntityHuman entityhuman, IBlockData iblockdata, GeneratorAccess generatoraccess, BlockPosition blockposition, boolean flag, ItemStack itemstack) {
        if (!entityhuman.canUseGameMasterBlocks()) {
            ...
            if (collection.isEmpty()) {
                message(entityhuman, IChatBaseComponent.translatable(this.descriptionId + ".empty", holder.getRegisteredName()));
                return false;
            } else {
                ...
                if (debugstickstate == null) {
                    ...
                    if (flag) {
                        ...
                        message(entityhuman, IChatBaseComponent.translatable(this.descriptionId + ".update", iblockstate.getName(), getNameHelper(iblockdata1, iblockstate)));
                    } else {
                        ...
                        message(entityhuman, IChatBaseComponent.translatable(this.descriptionId + ".select", iblockstate.getName(), getNameHelper(iblockdata, iblockstate)));
                    }
    ...
    private static <T> T getRelative(Iterable<T> iterable, @Nullable T t0, boolean flag) {
        return (T) (flag ? SystemUtils.findPreviousInIterable(iterable, t0) : SystemUtils.findNextInIterable(iterable, t0));
    }
