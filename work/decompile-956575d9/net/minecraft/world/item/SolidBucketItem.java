...
public class SolidBucketItem extends ItemBlock implements DispensibleContainerItem {
    ...
    public boolean emptyContents(@Nullable EntityLiving entityliving, World world, BlockPosition blockposition, @Nullable MovingObjectPositionBlock movingobjectpositionblock) {
        if (world.isInWorldBounds(blockposition) && world.isEmptyBlock(blockposition)) {
            ...
            world.gameEvent(entityliving, (Holder) GameEvent.FLUID_PLACE, blockposition);
            world.playSound(entityliving, blockposition, this.placeSound, SoundCategory.BLOCKS, 1.0F, 1.0F);
            return true;
        } else {
            return false;
        }
