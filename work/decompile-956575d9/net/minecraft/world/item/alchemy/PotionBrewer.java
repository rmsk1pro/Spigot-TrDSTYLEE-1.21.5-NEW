...
public class PotionBrewer {
    ...
    private boolean isContainer(ItemStack itemstack) {
        for (RecipeItemStack recipeitemstack : this.containers) {
    ...
    public boolean isContainerIngredient(ItemStack itemstack) {
        for (PotionBrewer.PredicatedCombination<Item> potionbrewer_predicatedcombination : this.containerMixes) {
            if (potionbrewer_predicatedcombination.ingredient.test(itemstack)) {
                return true;
            }
        }
    ...
    public boolean isPotionIngredient(ItemStack itemstack) {
        for (PotionBrewer.PredicatedCombination<PotionRegistry> potionbrewer_predicatedcombination : this.potionMixes) {
            if (potionbrewer_predicatedcombination.ingredient.test(itemstack)) {
                return true;
            }
        }
    ...
    public boolean isBrewablePotion(Holder<PotionRegistry> holder) {
        for (PotionBrewer.PredicatedCombination<PotionRegistry> potionbrewer_predicatedcombination : this.potionMixes) {
            if (potionbrewer_predicatedcombination.to.is(holder)) {
                return true;
            }
        }
    ...
    public boolean hasContainerMix(ItemStack itemstack, ItemStack itemstack1) {
        for (PotionBrewer.PredicatedCombination<Item> potionbrewer_predicatedcombination : this.containerMixes) {
            if (itemstack.is(potionbrewer_predicatedcombination.from) && potionbrewer_predicatedcombination.ingredient.test(itemstack1)) {
                return true;
            }
        }
    ...
    public boolean hasPotionMix(ItemStack itemstack, ItemStack itemstack1) {
        ...
        if (optional.isEmpty()) {
            ...
            for (PotionBrewer.PredicatedCombination<PotionRegistry> potionbrewer_predicatedcombination : this.potionMixes) {
                if (potionbrewer_predicatedcombination.from.is((Holder) optional.get()) && potionbrewer_predicatedcombination.ingredient.test(itemstack1)) {
                    return true;
                }
            }
    ...
    public ItemStack mix(ItemStack itemstack, ItemStack itemstack1) {
        if (itemstack1.isEmpty()) {
            ...
            if (optional.isEmpty()) {
                ...
                for (PotionBrewer.PredicatedCombination<Item> potionbrewer_predicatedcombination : this.containerMixes) {
                    if (itemstack1.is(potionbrewer_predicatedcombination.from) && potionbrewer_predicatedcombination.ingredient.test(itemstack)) {
                        return PotionContents.createItemStack((Item) potionbrewer_predicatedcombination.to.value(), (Holder) optional.get());
                    }
                }

                for (PotionBrewer.PredicatedCombination<PotionRegistry> potionbrewer_predicatedcombination1 : this.potionMixes) {
                    if (potionbrewer_predicatedcombination1.from.is((Holder) optional.get()) && potionbrewer_predicatedcombination1.ingredient.test(itemstack)) {
                        return PotionContents.createItemStack(itemstack1.getItem(), potionbrewer_predicatedcombination1.to);
                    }
                }
    ...
    public static class a {
        ...
        public void addContainerRecipe(Item item, Item item1, Item item2) {
            if (item.isEnabled(this.enabledFeatures) && item1.isEnabled(this.enabledFeatures) && item2.isEnabled(this.enabledFeatures)) {
                expectPotion(item);
                expectPotion(item2);
                this.containerMixes.add(new PotionBrewer.PredicatedCombination(item.builtInRegistryHolder(), RecipeItemStack.of((IMaterial) item1), item2.builtInRegistryHolder()));
            }
        }
        ...
        public void addContainer(Item item) {
            if (item.isEnabled(this.enabledFeatures)) {
                expectPotion(item);
                this.containers.add(RecipeItemStack.of((IMaterial) item));
            }
        }
        ...
        public void addMix(Holder<PotionRegistry> holder, Item item, Holder<PotionRegistry> holder1) {
            if (((PotionRegistry) holder.value()).isEnabled(this.enabledFeatures) && item.isEnabled(this.enabledFeatures) && ((PotionRegistry) holder1.value()).isEnabled(this.enabledFeatures)) {
                this.potionMixes.add(new PotionBrewer.PredicatedCombination(holder, RecipeItemStack.of((IMaterial) item), holder1));
            }
