...
public class ItemCooldown {
    ...
    public void tick() {
        ...
        if (!this.cooldowns.isEmpty()) {
            Iterator<Map.Entry<MinecraftKey, ItemCooldown.Info>> iterator = this.cooldowns.entrySet().iterator();

            while (iterator.hasNext()) {
                Map.Entry<MinecraftKey, ItemCooldown.Info> map_entry = (Entry) iterator.next();

                if (((ItemCooldown.Info) map_entry.getValue()).endTime <= this.tickCount) {
                    iterator.remove();
                    this.onCooldownEnded((MinecraftKey) map_entry.getKey());
                }
            }
        }
