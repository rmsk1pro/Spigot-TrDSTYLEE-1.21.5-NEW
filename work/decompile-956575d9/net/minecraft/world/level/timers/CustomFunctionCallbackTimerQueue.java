...
public class CustomFunctionCallbackTimerQueue<T> {
    ...
    public void schedule(String s, long i, CustomFunctionCallbackTimer<T> customfunctioncallbacktimer) {
        if (!this.events.contains(s, i)) {
            ...
            CustomFunctionCallbackTimerQueue.a<T> customfunctioncallbacktimerqueue_a = new CustomFunctionCallbackTimerQueue.a<T>(i, this.sequentialId, s, customfunctioncallbacktimer);
    ...
    private void loadEvent(NBTTagCompound nbttagcompound) {
        CustomFunctionCallbackTimer<T> customfunctioncallbacktimer = (CustomFunctionCallbackTimer) nbttagcompound.read("Callback", this.callbacksRegistry.codec()).orElse((Object) null);

        if (customfunctioncallbacktimer != null) {
            String s = nbttagcompound.getStringOr("Name", "");
            long i = nbttagcompound.getLongOr("TriggerTime", 0L);
    ...
    private NBTTagCompound storeEvent(CustomFunctionCallbackTimerQueue.a<T> customfunctioncallbacktimerqueue_a) {
        ...
        nbttagcompound.store("Callback", this.callbacksRegistry.codec(), customfunctioncallbacktimerqueue_a.callback);
