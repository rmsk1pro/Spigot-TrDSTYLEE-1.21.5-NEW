...
public class NestedLootTable extends LootSelectorEntry {
    ...
    public void createItemStack(Consumer<ItemStack> consumer, LootTableInfo loottableinfo) {
        ((LootTable) this.contents.map((resourcekey) -> {
            return (LootTable) loottableinfo.getResolver().get(resourcekey).map(Holder::value).orElse(LootTable.EMPTY);
        }, (loottable) -> {
            return loottable;
    ...
    public void validate(LootCollector lootcollector) {
        ...
        if (optional.isPresent()) {
            ...
            if (!lootcollector.allowsReferences()) {
                lootcollector.reportProblem("Uses reference to " + String.valueOf(resourcekey.location()) + ", but references are not allowed");
                return;
            }
        ...
        this.contents.ifLeft((resourcekey1) -> {
            lootcollector.resolver().get(resourcekey1).ifPresentOrElse((holder_c) -> {
