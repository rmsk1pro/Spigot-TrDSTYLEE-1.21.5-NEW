...
public interface ListOperation {
    ...
    default <T> List<T> apply(List<T> list, List<T> list1) {
        return this.<T>apply(list, list1, Integer.MAX_VALUE);
    }
    ...
    public static enum f implements INamable {
        ...
        public static final Codec<ListOperation.f> CODEC = INamable.<ListOperation.f>fromEnum(ListOperation.f::values);
    ...
    public static record d(int offset, Optional<Integer> size) implements ListOperation {
        ...
        public <T> List<T> apply(List<T> list, List<T> list1, int i) {
            ...
            if (this.offset > j) {
                ...
                immutablelist_builder.addAll(list.subList(0, this.offset));
                immutablelist_builder.addAll(list1);
                int k = this.offset + (Integer) this.size.orElse(list1.size());

                if (k < j) {
                    immutablelist_builder.addAll(list.subList(k, j));
                }
    ...
    public static record b(int offset) implements ListOperation {
        ...
        public <T> List<T> apply(List<T> list, List<T> list1, int i) {
            ...
            if (this.offset > j) {
                ...
                immutablelist_builder.addAll(list.subList(0, this.offset));
                immutablelist_builder.addAll(list1);
                immutablelist_builder.addAll(list.subList(this.offset, j));
    ...
    public static record e<T>(List<T> value, ListOperation operation) {
        ...
        public List<T> apply(List<T> list) {
            return this.operation.<T>apply(list, this.value);
        }
    }
}
