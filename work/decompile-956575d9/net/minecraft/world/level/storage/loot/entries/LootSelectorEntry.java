...
public abstract class LootSelectorEntry extends LootEntryAbstract {
    ...
    protected static <T extends LootSelectorEntry> Products.P4<RecordCodecBuilder.Mu<T>, Integer, Integer, List<LootItemCondition>, List<LootItemFunction>> singletonFields(RecordCodecBuilder.Instance<T> recordcodecbuilder_instance) {
        return recordcodecbuilder_instance.group(Codec.INT.optionalFieldOf("weight", 1).forGetter((lootselectorentry) -> {
        ...
        })).and(commonFields(recordcodecbuilder_instance).t1()).and(LootItemFunctions.ROOT_CODEC.listOf().optionalFieldOf("functions", List.of()).forGetter((lootselectorentry) -> {
