...
public class LootSelector {
    ...
    public void validate(LootCollector lootcollector) {
        ...
        for (int j = 0; j < this.functions.size(); ++j) {
            ((LootItemFunction) this.functions.get(j)).validate(lootcollector.forChild(".functions[" + j + "]"));
        }

        for (int k = 0; k < this.entries.size(); ++k) {
            ((LootEntryAbstract) this.entries.get(k)).validate(lootcollector.forChild(".entries[" + k + "]"));
        }
