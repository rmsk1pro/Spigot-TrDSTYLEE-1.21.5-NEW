...
public interface SignalGetter extends IBlockAccess {
    ...
    default int getDirectSignalTo(BlockPosition blockposition) {
        ...
        i = Math.max(i, this.getDirectSignal(blockposition.below(), EnumDirection.DOWN));
        if (i >= 15) {
            return i;
        } else {
    ...
    default int getBestNeighborSignal(BlockPosition blockposition) {
        ...
        for (EnumDirection enumdirection : SignalGetter.DIRECTIONS) {
            int j = this.getSignal(blockposition.relative(enumdirection), enumdirection);
