...
public class ChunkCache implements ICollisionAccess {
    ...
    public ChunkCache(World world, BlockPosition blockposition, BlockPosition blockposition1) {
        ...
        this.plains = Suppliers.memoize(() -> {
            return world.registryAccess().lookupOrThrow(Registries.BIOME).getOrThrow(Biomes.PLAINS);
        });
        ...
        for (int i1 = SectionPosition.blockToSectionCoord(blockposition.getX()); i1 <= SectionPosition.blockToSectionCoord(blockposition1.getX()); ++i1) {
            for (int j1 = SectionPosition.blockToSectionCoord(blockposition.getZ()); j1 <= SectionPosition.blockToSectionCoord(blockposition1.getZ()); ++j1) {
                IChunkAccess ichunkaccess = this.chunks[i1 - this.centerX][j1 - this.centerZ];
