...
public class IOWorker implements ChunkScanAccess, AutoCloseable {
    ...
    private boolean isOldChunk(NBTTagCompound nbttagcompound) {
        return nbttagcompound.getIntOr("DataVersion", 0) < 4295 ? true : nbttagcompound.getCompound("blending_data").isPresent();
    }
    ...
    public CompletableFuture<Optional<NBTTagCompound>> loadAsync(ChunkCoordIntPair chunkcoordintpair) {
        return this.<Optional<NBTTagCompound>>submitThrowingTask(() -> {
    ...
    private void storePendingChunk() {
        Map.Entry<ChunkCoordIntPair, IOWorker.a> map_entry = this.pendingWrites.pollFirstEntry();

        if (map_entry != null) {
            this.runStore((ChunkCoordIntPair) map_entry.getKey(), (IOWorker.a) map_entry.getValue());
            this.tellStorePending();
        }
    }
