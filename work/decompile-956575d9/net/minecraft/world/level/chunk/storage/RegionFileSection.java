...
public class RegionFileSection<R, P> implements AutoCloseable {
    ...
    protected void tick(BooleanSupplier booleansupplier) {
        ...
        while (((LongIterator) longiterator).hasNext() && booleansupplier.getAsBoolean()) {
    ...
    private void unpackPendingLoads() {
        synchronized (this.loadLock) {
            Iterator<Long2ObjectMap.Entry<CompletableFuture<Optional<RegionFileSection.a<P>>>>> iterator = Long2ObjectMaps.fastIterator(this.pendingLoads);

            while (((Iterator) iterator).hasNext()) {
                Long2ObjectMap.Entry<CompletableFuture<Optional<RegionFileSection.a<P>>>> long2objectmap_entry = (Entry) iterator.next();
                Optional<RegionFileSection.a<P>> optional = (Optional) ((CompletableFuture) long2objectmap_entry.getValue()).getNow((Object) null);
    ...
    private void unpackChunk(ChunkCoordIntPair chunkcoordintpair) {
        long i = chunkcoordintpair.toLong();
        CompletableFuture<Optional<RegionFileSection.a<P>>> completablefuture;
    ...
    private void writeChunk(ChunkCoordIntPair chunkcoordintpair) {
        RegistryOps<NBTBase> registryops = this.registryAccess.<NBTBase>createSerializationContext(DynamicOpsNBT.INSTANCE);
        Dynamic<NBTBase> dynamic = this.<NBTBase>writeChunk(chunkcoordintpair, registryops);
    ...
    private static record a<T>(Int2ObjectMap<T> sectionsByY, boolean versionChanged) {
        ...
        public static <T> RegionFileSection.a<T> parse(Codec<T> codec, DynamicOps<NBTBase> dynamicops, NBTBase nbtbase, SimpleRegionStorage simpleregionstorage, LevelHeightAccessor levelheightaccessor) {
            ...
            return new RegionFileSection.a<T>(int2objectmap, flag);
        }
    }
}
