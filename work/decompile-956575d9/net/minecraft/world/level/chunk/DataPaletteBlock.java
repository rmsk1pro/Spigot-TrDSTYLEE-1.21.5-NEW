...
public class DataPaletteBlock<T> implements DataPaletteExpandable<T>, PalettedContainerRO<T> {
    ...
    public DataPaletteBlock(Registry<T> registry, DataPaletteBlock.d datapaletteblock_d, DataPaletteBlock.a<T> datapaletteblock_a, DataBits databits, List<T> list) {
        ...
        this.data = new DataPaletteBlock.c<T>(datapaletteblock_a, databits, datapaletteblock_a.factory().create(datapaletteblock_a.bits(), registry, this, list));
    }
    ...
    private DataPaletteBlock(DataPaletteBlock<T> datapaletteblock) {
        this.registry = datapaletteblock.registry;
        this.strategy = datapaletteblock.strategy;
        this.data = datapaletteblock.data.copy(this);
    }
    ...
    private DataPaletteBlock.c<T> createOrReuseData(@Nullable DataPaletteBlock.c<T> datapaletteblock_c, int i) {
        DataPaletteBlock.a<T> datapaletteblock_a = this.strategy.<T>getConfiguration(this.registry, i);
    ...
    public void read(PacketDataSerializer packetdataserializer) {
        ...
        try {
            ...
            DataPaletteBlock.c<T> datapaletteblock_c = this.createOrReuseData(this.data, i);

            datapaletteblock_c.palette.read(packetdataserializer);
            packetdataserializer.readFixedSizeLongArray(datapaletteblock_c.storage.getRaw());
            this.data = datapaletteblock_c;
        } finally {
            this.release();
        }
    ...
    private static <T> DataResult<DataPaletteBlock<T>> unpack(Registry<T> registry, DataPaletteBlock.d datapaletteblock_d, PalettedContainerRO.a<T> palettedcontainerro_a) {
        ...
        DataPaletteBlock.a<T> datapaletteblock_a = datapaletteblock_d.<T>getConfiguration(registry, j);
        DataBits databits;
        ...
        if (j == 0) {
            ...
            try {
                if (datapaletteblock_a.factory() == DataPaletteBlock.d.GLOBAL_PALETTE_FACTORY) {
                    DataPalette<T> datapalette = new DataPaletteHash<T>(registry, j, (k, object) -> {
                        return 0;
                    }, list);
                    ...
                    databits = new SimpleBitStorage(datapaletteblock_a.bits(), i, aint);
                } else {
                    databits = new SimpleBitStorage(datapaletteblock_a.bits(), i, along);
                }
        ...
        return DataResult.success(new DataPaletteBlock(registry, datapaletteblock_d, datapaletteblock_a, databits, list));
    }
    ...
    public PalettedContainerRO.a<T> pack(Registry<T> registry, DataPaletteBlock.d datapaletteblock_d) {
        ...
        try {
            DataPaletteHash<T> datapalettehash = new DataPaletteHash<T>(registry, this.data.storage.getBits(), this.dummyPaletteResize);
            int i = datapaletteblock_d.size();
            int[] aint = new int[i];
    ...
    public DataPaletteBlock<T> recreate() {
        return new DataPaletteBlock<T>(this.registry, this.data.palette.valueFor(0), this.strategy);
    }
    ...
    private static record c<T>(DataPaletteBlock.a<T> configuration, DataBits storage, DataPalette<T> palette) {
        ...
        public DataPaletteBlock.c<T> copy(DataPaletteExpandable<T> datapaletteexpandable) {
            return new DataPaletteBlock.c<T>(this.configuration, this.storage.copy(), this.palette.copy(datapaletteexpandable));
        }
    }
    ...
    private static record a<T>(DataPalette.a factory, int bits) {
        ...
        public DataPaletteBlock.c<T> createData(Registry<T> registry, DataPaletteExpandable<T> datapaletteexpandable, int i) {
            DataBits databits = (DataBits) (this.bits == 0 ? new ZeroBitStorage(i) : new SimpleBitStorage(this.bits, i));
            DataPalette<T> datapalette = this.factory.<T>create(this.bits, registry, datapaletteexpandable, List.of());

            return new DataPaletteBlock.c<T>(this, databits, datapalette);
        }
    }
    ...
    public abstract static class d {
        ...
        <A> int calculateBitsForSerialization(Registry<A> registry, int i) {
            ...
            DataPaletteBlock.a<A> datapaletteblock_a = this.<A>getConfiguration(registry, j);
