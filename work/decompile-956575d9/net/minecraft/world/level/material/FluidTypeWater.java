...
public abstract class FluidTypeWater extends FluidTypeFlowing {
    ...
    public void animateTick(World world, BlockPosition blockposition, Fluid fluid, RandomSource randomsource) {
        if (!fluid.isSource() && !(Boolean) fluid.getValue(FluidTypeWater.FALLING)) {
            if (randomsource.nextInt(64) == 0) {
                world.playLocalSound((double) blockposition.getX() + 0.5D, (double) blockposition.getY() + 0.5D, (double) blockposition.getZ() + 0.5D, SoundEffects.WATER_AMBIENT, SoundCategory.AMBIENT, randomsource.nextFloat() * 0.25F + 0.75F, randomsource.nextFloat() + 0.5F, false);
            }
    ...
    protected boolean canConvertToSource(WorldServer worldserver) {
        return worldserver.getGameRules().getBoolean(GameRules.RULE_WATER_SOURCE_CONVERSION);
    }
