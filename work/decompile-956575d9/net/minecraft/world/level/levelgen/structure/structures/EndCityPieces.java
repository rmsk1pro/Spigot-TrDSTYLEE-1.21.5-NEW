...
public class EndCityPieces {
            ...
            if (endcitypieces_a2 != null) {
                for (Tuple<EnumBlockRotation, BlockPosition> tuple : EndCityPieces.TOWER_BRIDGES) {
                    if (randomsource.nextBoolean()) {
                        EndCityPieces.a endcitypieces_a3 = EndCityPieces.addHelper(list, EndCityPieces.addPiece(structuretemplatemanager, endcitypieces_a2, tuple.getB(), "bridge_end", enumblockrotation.getRotated(tuple.getA()), true));
    ...
    static final EndCityPieces.b TOWER_BRIDGE_GENERATOR = new EndCityPieces.b() {
        ...
        public boolean generate(StructureTemplateManager structuretemplatemanager, int i, EndCityPieces.a endcitypieces_a, BlockPosition blockposition, List<StructurePiece> list, RandomSource randomsource) {
            ...
            for (int l = 0; l < j; ++l) {
                if (randomsource.nextBoolean()) {
                    endcitypieces_a1 = EndCityPieces.addHelper(list, EndCityPieces.addPiece(structuretemplatemanager, endcitypieces_a1, new BlockPosition(0, k, -4), "bridge_piece", enumblockrotation, true));
                    k = 0;
                } else {
                    if (randomsource.nextBoolean()) {
                        endcitypieces_a1 = EndCityPieces.addHelper(list, EndCityPieces.addPiece(structuretemplatemanager, endcitypieces_a1, new BlockPosition(0, k, -4), "bridge_steep_stairs", enumblockrotation, true));
                    } else {
                        endcitypieces_a1 = EndCityPieces.addHelper(list, EndCityPieces.addPiece(structuretemplatemanager, endcitypieces_a1, new BlockPosition(0, k, -8), "bridge_gentle_stairs", enumblockrotation, true));
                    }
            ...
            if (!this.shipCreated && randomsource.nextInt(10 - i) == 0) {
                EndCityPieces.addHelper(list, EndCityPieces.addPiece(structuretemplatemanager, endcitypieces_a1, new BlockPosition(-8 + randomsource.nextInt(8), k, -70 + randomsource.nextInt(10)), "ship", enumblockrotation, true));
                this.shipCreated = true;
            } else if (!EndCityPieces.recursiveChildren(structuretemplatemanager, EndCityPieces.HOUSE_TOWER_GENERATOR, i + 1, endcitypieces_a1, new BlockPosition(-3, k + 1, -11), list, randomsource)) {
                return false;
            }

            endcitypieces_a1 = EndCityPieces.addHelper(list, EndCityPieces.addPiece(structuretemplatemanager, endcitypieces_a1, new BlockPosition(4, k, 0), "bridge_end", enumblockrotation.getRotated(EnumBlockRotation.CLOCKWISE_180), true));
            endcitypieces_a1.setGenDepth(-1);
            return true;
        }
    };
        ...
        public boolean generate(StructureTemplateManager structuretemplatemanager, int i, EndCityPieces.a endcitypieces_a, BlockPosition blockposition, List<StructurePiece> list, RandomSource randomsource) {
            ...
            for (int j = 0; j < 2 && randomsource.nextInt(3) != 0; ++j) {
                ...
                for (Tuple<EnumBlockRotation, BlockPosition> tuple : EndCityPieces.FAT_TOWER_BRIDGES) {
                    if (randomsource.nextBoolean()) {
                        EndCityPieces.a endcitypieces_a2 = EndCityPieces.addHelper(list, EndCityPieces.addPiece(structuretemplatemanager, endcitypieces_a1, tuple.getB(), "bridge_end", enumblockrotation.getRotated(tuple.getA()), true));
    ...
    public static class a extends DefinedStructurePiece {
        ...
        public a(StructureTemplateManager structuretemplatemanager, NBTTagCompound nbttagcompound) {
            super(WorldGenFeatureStructurePieceType.END_CITY_PIECE, nbttagcompound, structuretemplatemanager, (minecraftkey) -> {
                return makeSettings(nbttagcompound.getBooleanOr("OW", false), (EnumBlockRotation) nbttagcompound.read("Rot", EnumBlockRotation.LEGACY_CODEC).orElseThrow());
            });
        }
        ...
        protected void handleDataMarker(String s, BlockPosition blockposition, WorldAccess worldaccess, RandomSource randomsource, StructureBoundingBox structureboundingbox) {
            if (s.startsWith("Chest")) {
                ...
                if (s.startsWith("Sentry")) {
                    EntityShulker entityshulker = EntityTypes.SHULKER.create(worldaccess.getLevel(), EntitySpawnReason.STRUCTURE);
