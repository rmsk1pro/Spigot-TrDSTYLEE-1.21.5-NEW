...
public class RuinedPortalStructure extends Structure {
    ...
    public Optional<Structure.b> findGenerationPoint(Structure.a structure_a) {
        ...
        if (this.setups.size() > 1) {
            ...
            for (RuinedPortalStructure.a ruinedportalstructure_a1 : this.setups) {
                f += ruinedportalstructure_a1.weight();
            }

            float f1 = seededrandom.nextFloat();

            for (RuinedPortalStructure.a ruinedportalstructure_a2 : this.setups) {
        ...
        if (ruinedportalstructure_a == null) {
            ...
            if (seededrandom.nextFloat() < 0.05F) {
                minecraftkey = MinecraftKey.withDefaultNamespace(RuinedPortalStructure.STRUCTURE_LOCATION_GIANT_PORTALS[seededrandom.nextInt(RuinedPortalStructure.STRUCTURE_LOCATION_GIANT_PORTALS.length)]);
            } else {
                minecraftkey = MinecraftKey.withDefaultNamespace(RuinedPortalStructure.STRUCTURE_LOCATION_PORTALS[seededrandom.nextInt(RuinedPortalStructure.STRUCTURE_LOCATION_PORTALS.length)]);
            }

            DefinedStructure definedstructure = structure_a.structureTemplateManager().getOrCreate(minecraftkey);
            EnumBlockRotation enumblockrotation = (EnumBlockRotation) SystemUtils.getRandom(EnumBlockRotation.values(), seededrandom);
            ...
            return Optional.of(new Structure.b(blockposition3, (structurepiecesbuilder) -> {
                if (ruinedportalstructure_a.canBeCold()) {
                    ruinedportalpiece_a.cold = isCold(blockposition3, structure_a.chunkGenerator().getBiomeSource().getNoiseBiome(QuartPos.fromBlock(blockposition3.getX()), QuartPos.fromBlock(blockposition3.getY()), QuartPos.fromBlock(blockposition3.getZ()), randomstate.sampler()), chunkgenerator.getSeaLevel());
                }
    ...
    private static boolean isCold(BlockPosition blockposition, Holder<BiomeBase> holder, int i) {
        return ((BiomeBase) holder.value()).coldEnoughToSnow(blockposition, i);
    }
    ...
    private static int findSuitableY(RandomSource randomsource, ChunkGenerator chunkgenerator, RuinedPortalPiece.b ruinedportalpiece_b, boolean flag, int i, int j, StructureBoundingBox structureboundingbox, LevelHeightAccessor levelheightaccessor, RandomState randomstate) {
        ...
        if (ruinedportalpiece_b == RuinedPortalPiece.b.IN_NETHER) {
        ...
        } else if (ruinedportalpiece_b == RuinedPortalPiece.b.IN_MOUNTAIN) {
            int i1 = i - j;
            ...
            l = getRandomWithinInterval(randomsource, k, j1);
        ...
        for (k1 = l; k1 > k; --k1) {
            ...
            for (BlockColumn blockcolumn : list1) {
                IBlockData iblockdata = blockcolumn.getBlock(k1);
