    ...
    public static class a extends DefinedStructurePiece {
        ...
        public a(StructureTemplateManager structuretemplatemanager, NBTTagCompound nbttagcompound) {
            super(WorldGenFeatureStructurePieceType.IGLOO, nbttagcompound, structuretemplatemanager, (minecraftkey) -> {
                return makeSettings((EnumBlockRotation) nbttagcompound.read("Rot", EnumBlockRotation.LEGACY_CODEC).orElseThrow(), minecraftkey);
            });
        }

        private static DefinedStructureInfo makeSettings(EnumBlockRotation enumblockrotation, MinecraftKey minecraftkey) {
            return (new DefinedStructureInfo()).setRotation(enumblockrotation).setMirror(EnumBlockMirror.NONE).setRotationPivot((BlockPosition) IglooPieces.PIVOTS.get(minecraftkey)).addProcessor(DefinedStructureProcessorBlockIgnore.STRUCTURE_BLOCK).setLiquidSettings(LiquidSettings.IGNORE_WATERLOGGING);
        }
