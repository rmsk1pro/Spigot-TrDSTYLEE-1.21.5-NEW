...
public class WorldGenFeatureDefinedStructurePoolSingle extends WorldGenFeatureDefinedStructurePoolStructure {
    ...
    public List<DefinedStructure.BlockInfo> getDataMarkers(StructureTemplateManager structuretemplatemanager, BlockPosition blockposition, EnumBlockRotation enumblockrotation, boolean flag) {
        ...
        for (DefinedStructure.BlockInfo definedstructure_blockinfo : list) {
            ...
            if (nbttagcompound != null) {
                BlockPropertyStructureMode blockpropertystructuremode = (BlockPropertyStructureMode) nbttagcompound.read("mode", BlockPropertyStructureMode.LEGACY_CODEC).orElseThrow();
    ...
    public List<DefinedStructure.a> getShuffledJigsawBlocks(StructureTemplateManager structuretemplatemanager, BlockPosition blockposition, EnumBlockRotation enumblockrotation, RandomSource randomsource) {
        List<DefinedStructure.a> list = this.getTemplate(structuretemplatemanager).getJigsaws(blockposition, enumblockrotation);

        SystemUtils.shuffle(list, randomsource);
    ...
    static void sortBySelectionPriority(List<DefinedStructure.a> list) {
        list.sort(WorldGenFeatureDefinedStructurePoolSingle.HIGHEST_SELECTION_PRIORITY_FIRST);
    }
    ...
    public boolean place(StructureTemplateManager structuretemplatemanager, GeneratorAccessSeed generatoraccessseed, StructureManager structuremanager, ChunkGenerator chunkgenerator, BlockPosition blockposition, BlockPosition blockposition1, EnumBlockRotation enumblockrotation, StructureBoundingBox structureboundingbox, RandomSource randomsource, LiquidSettings liquidsettings, boolean flag) {
        ...
        if (!definedstructure.placeInWorld(generatoraccessseed, blockposition, blockposition1, definedstructureinfo, randomsource, 18)) {
            ...
            for (DefinedStructure.BlockInfo definedstructure_blockinfo : DefinedStructure.processBlockInfos(generatoraccessseed, blockposition, blockposition1, definedstructureinfo, this.getDataMarkers(structuretemplatemanager, blockposition, enumblockrotation, false))) {
