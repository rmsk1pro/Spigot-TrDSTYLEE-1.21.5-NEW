    ...
    private NoiseGenerator3(RandomSource randomsource, IntSortedSet intsortedset) {
        if (intsortedset.isEmpty()) {
            ...
            if (k < 1) {
                ...
                if (j > 0) {
                    long j1 = (long) (noisegenerator3handler.getValue(noisegenerator3handler.xo, noisegenerator3handler.yo, noisegenerator3handler.zo) * (double) Long.MAX_VALUE);
                    RandomSource randomsource1 = new SeededRandom(new LegacyRandomSource(j1));

                    for (int k1 = l - 1; k1 >= 0; --k1) {
                        if (k1 < k && intsortedset.contains(l - k1)) {
                            this.noiseLevels[k1] = new NoiseGenerator3Handler(randomsource1);
                        } else {
