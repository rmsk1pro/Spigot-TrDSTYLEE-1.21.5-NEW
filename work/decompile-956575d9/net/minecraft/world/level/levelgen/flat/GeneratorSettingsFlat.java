...
public class GeneratorSettingsFlat {
    ...
    public static final Codec<GeneratorSettingsFlat> CODEC = RecordCodecBuilder.create((instance) -> {
        return instance.group(RegistryCodecs.homogeneousList(Registries.STRUCTURE_SET).lenientOptionalFieldOf("structure_overrides").forGetter((generatorsettingsflat) -> {
        ...
        }), BiomeBase.CODEC.lenientOptionalFieldOf("biome").orElseGet(Optional::empty).forGetter((generatorsettingsflat) -> {
    ...
    public BiomeSettingsGeneration adjustGenerationSettings(Holder<BiomeBase> holder) {
        if (!holder.equals(this.biome)) {
            ...
            if (flag) {
                List<HolderSet<PlacedFeature>> list = biomesettingsgeneration.features();
            ...
            List<IBlockData> list1 = this.getLayers();

            for (int j = 0; j < list1.size(); ++j) {
                IBlockData iblockdata = (IBlockData) list1.get(j);

                if (!HeightMap.Type.MOTION_BLOCKING.isOpaque().test(iblockdata)) {
                    list1.set(j, (Object) null);
                    biomesettingsgeneration_b.addFeature(WorldGenStage.Decoration.TOP_LAYER_MODIFICATION, PlacementUtils.inlinePlaced(WorldGenerator.FILL_LAYER, new WorldGenFeatureFillConfiguration(j, iblockdata)));
                }
            }
