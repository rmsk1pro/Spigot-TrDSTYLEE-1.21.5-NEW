...
public class BlendingData {
    ...
    static final int CELL_COLUMN_COUNT = BlendingData.CELL_COLUMN_INSIDE_COUNT + BlendingData.CELL_COLUMN_OUTSIDE_COUNT;
    ...
    private BlendingData(int i, int j, Optional<double[]> optional) {
        this.heights = (double[]) optional.orElseGet(() -> {
            return (double[]) SystemUtils.make(new double[BlendingData.CELL_COLUMN_COUNT], (adouble) -> {
    ...
    @Nullable
    public static BlendingData unpack(@Nullable BlendingData.d blendingdata_d) {
        return blendingdata_d == null ? null : new BlendingData(blendingdata_d.minSection(), blendingdata_d.maxSection(), blendingdata_d.heights());
    }

    public BlendingData.d pack() {
        boolean flag = false;
        ...
        return new BlendingData.d(this.areaWithOldGeneration.getMinSectionY(), this.areaWithOldGeneration.getMaxSectionY() + 1, flag ? Optional.of(DoubleArrays.copy(this.heights)) : Optional.empty());
    }
    ...
    public static BlendingData getOrUpdateBlendingData(RegionLimitedWorldAccess regionlimitedworldaccess, int i, int j) {
        ...
        if (blendingdata != null && !ichunkaccess.getHighestGeneratedStatus().isBefore(ChunkStatus.BIOMES)) {
    ...
    public static Set<EnumDirection8> sideByGenerationAge(GeneratorAccessSeed generatoraccessseed, int i, int j, boolean flag) {
        ...
        for (EnumDirection8 enumdirection8 : EnumDirection8.values()) {
            int k = i + enumdirection8.getStepX();
            int l = j + enumdirection8.getStepZ();

            if (generatoraccessseed.getChunk(k, l).isOldNoiseGeneration() == flag) {
        ...
        if (!this.hasCalculatedData) {
            ...
            if (set.contains(EnumDirection8.WEST)) {
                for (int j = 1; j < BlendingData.QUARTS_PER_SECTION; ++j) {
                    this.addValuesForColumn(getInsideIndex(0, j), ichunkaccess, 0, 4 * j);
                }
            }

            if (set.contains(EnumDirection8.EAST)) {
                for (int k = 1; k < BlendingData.QUARTS_PER_SECTION; ++k) {
                    this.addValuesForColumn(getOutsideIndex(BlendingData.CELL_HORIZONTAL_MAX_INDEX_OUTSIDE, k), ichunkaccess, 15, 4 * k);
                }
            }

            if (set.contains(EnumDirection8.SOUTH)) {
                for (int l = 0; l < BlendingData.QUARTS_PER_SECTION; ++l) {
                    this.addValuesForColumn(getOutsideIndex(l, BlendingData.CELL_HORIZONTAL_MAX_INDEX_OUTSIDE), ichunkaccess, 4 * l, 15);
                }
            }
    ...
    private int getHeightAtXZ(IChunkAccess ichunkaccess, int i, int j) {
        ...
        if (ichunkaccess.hasPrimedHeightmap(HeightMap.Type.WORLD_SURFACE_WG)) {
            k = Math.min(ichunkaccess.getHeight(HeightMap.Type.WORLD_SURFACE_WG, i, j), this.areaWithOldGeneration.getMaxY());
        } else {
        ...
        while (blockposition_mutableblockposition.getY() > l) {
            if (BlendingData.SURFACE_BLOCKS.contains(ichunkaccess.getBlockState(blockposition_mutableblockposition).getBlock())) {
    ...
    private double[] getDensityColumn(IChunkAccess ichunkaccess, int i, int j, int k) {
        ...
        BlockPosition.MutableBlockPosition blockposition_mutableblockposition = new BlockPosition.MutableBlockPosition(i, this.areaWithOldGeneration.getMaxY() + 1, j);
        double d0 = read7(ichunkaccess, blockposition_mutableblockposition);

        for (int l = adouble.length - 2; l >= 0; --l) {
            double d1 = read1(ichunkaccess, blockposition_mutableblockposition);
            double d2 = read7(ichunkaccess, blockposition_mutableblockposition);

            adouble[l] = (d0 + d1 + d2) / 15.0D;
            d0 = d2;
        }

        int i1 = this.getCellYIndex(MathHelper.floorDiv(k, 8));
    ...
    public static record d(int minSection, int maxSection, Optional<double[]> heights) {

        private static final Codec<double[]> DOUBLE_ARRAY_CODEC = Codec.DOUBLE.listOf().xmap(Doubles::toArray, Doubles::asList);
        public static final Codec<BlendingData.d> CODEC = RecordCodecBuilder.create((instance) -> {
            return instance.group(Codec.INT.fieldOf("min_section").forGetter(BlendingData.d::minSection), Codec.INT.fieldOf("max_section").forGetter(BlendingData.d::maxSection), BlendingData.d.DOUBLE_ARRAY_CODEC.lenientOptionalFieldOf("heights").forGetter(BlendingData.d::heights)).apply(instance, BlendingData.d::new);
        }).validate(BlendingData.d::validateArraySize);

        private static DataResult<BlendingData.d> validateArraySize(BlendingData.d blendingdata_d) {
            return blendingdata_d.heights.isPresent() && ((double[]) blendingdata_d.heights.get()).length != BlendingData.CELL_COLUMN_COUNT ? DataResult.error(() -> {
                return "heights has to be of length " + BlendingData.CELL_COLUMN_COUNT;
            }) : DataResult.success(blendingdata_d);
        }
    }
