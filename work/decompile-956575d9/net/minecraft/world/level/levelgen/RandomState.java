...
public final class RandomState {
    ...
    private RandomState(GeneratorSettingBase generatorsettingbase, HolderGetter<NoiseGeneratorNormal.a> holdergetter, final long i) {
            ...
            public DensityFunction.c visitNoise(DensityFunction.c densityfunction_c) {
                ...
                if (flag) {
                    if (holder.is(Noises.TEMPERATURE)) {
                        NoiseGeneratorNormal noisegeneratornormal = NoiseGeneratorNormal.createLegacyNetherBiome(this.newLegacyInstance(0L), new NoiseGeneratorNormal.a(-7, 1.0D, new double[]{1.0D}));

                        return new DensityFunction.c(holder, noisegeneratornormal);
                    }

                    if (holder.is(Noises.VEGETATION)) {
                        NoiseGeneratorNormal noisegeneratornormal1 = NoiseGeneratorNormal.createLegacyNetherBiome(this.newLegacyInstance(1L), new NoiseGeneratorNormal.a(-7, 1.0D, new double[]{1.0D}));

                        return new DensityFunction.c(holder, noisegeneratornormal1);
                    }

                    if (holder.is(Noises.SHIFT)) {
                        NoiseGeneratorNormal noisegeneratornormal2 = NoiseGeneratorNormal.create(RandomState.this.random.fromHashOf(Noises.SHIFT.location()), new NoiseGeneratorNormal.a(0, 0.0D, new double[0]));

                        return new DensityFunction.c(holder, noisegeneratornormal2);
                    }
                }

                NoiseGeneratorNormal noisegeneratornormal3 = RandomState.this.getOrCreateNoise((ResourceKey) holder.unwrapKey().orElseThrow());

                return new DensityFunction.c(holder, noisegeneratornormal3);
            }

            private DensityFunction wrapNew(DensityFunction densityfunction) {
                if (densityfunction instanceof BlendedNoise blendednoise) {
                    RandomSource randomsource = flag ? this.newLegacyInstance(0L) : RandomState.this.random.fromHashOf(MinecraftKey.withDefaultNamespace("terrain"));
        ...
        DensityFunction.f densityfunction_f = new DensityFunction.f() {
            ...
            private DensityFunction wrapNew(DensityFunction densityfunction) {
                if (densityfunction instanceof DensityFunctions.j densityfunctions_j) {
                    return (DensityFunction) densityfunctions_j.function().value();
                } else if (densityfunction instanceof DensityFunctions.l densityfunctions_l) {
