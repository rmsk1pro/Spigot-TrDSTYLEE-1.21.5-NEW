...
public class WorldPresets {
    ...
    public static WorldDimensions createNormalWorldDimensions(HolderLookup.a holderlookup_a) {
        return ((WorldPreset) holderlookup_a.lookupOrThrow(Registries.WORLD_PRESET).getOrThrow(WorldPresets.NORMAL).value()).createWorldDimensions();
    }

    public static WorldDimension getNormalOverworld(HolderLookup.a holderlookup_a) {
        return (WorldDimension) ((WorldPreset) holderlookup_a.lookupOrThrow(Registries.WORLD_PRESET).getOrThrow(WorldPresets.NORMAL).value()).overworld().orElseThrow();
    }

    public static WorldDimensions createFlatWorldDimensions(HolderLookup.a holderlookup_a) {
        return ((WorldPreset) holderlookup_a.lookupOrThrow(Registries.WORLD_PRESET).getOrThrow(WorldPresets.FLAT).value()).createWorldDimensions();
    }
    ...
    private static class a {
        ...
        a(BootstrapContext<WorldPreset> bootstrapcontext) {
            ...
            HolderGetter<DimensionManager> holdergetter = bootstrapcontext.<DimensionManager>lookup(Registries.DIMENSION_TYPE);

            this.noiseSettings = bootstrapcontext.<GeneratorSettingBase>lookup(Registries.NOISE_SETTINGS);
            this.biomes = bootstrapcontext.<BiomeBase>lookup(Registries.BIOME);
            this.placedFeatures = bootstrapcontext.<PlacedFeature>lookup(Registries.PLACED_FEATURE);
            this.structureSets = bootstrapcontext.<StructureSet>lookup(Registries.STRUCTURE_SET);
            this.multiNoiseBiomeSourceParameterLists = bootstrapcontext.<MultiNoiseBiomeSourceParameterList>lookup(Registries.MULTI_NOISE_BIOME_SOURCE_PARAMETER_LIST);
