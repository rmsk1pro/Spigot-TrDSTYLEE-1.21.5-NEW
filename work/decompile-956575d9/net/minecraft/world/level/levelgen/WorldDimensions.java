...
public record WorldDimensions(Map<ResourceKey<WorldDimension>, WorldDimension> dimensions) {
    ...
    public WorldDimensions(IRegistry<WorldDimension> iregistry) {
        this((Map) iregistry.listElements().collect(Collectors.toMap(Holder.c::key, Holder.c::value)));
    }
    ...
    public WorldDimensions replaceOverworldGenerator(HolderLookup.a holderlookup_a, ChunkGenerator chunkgenerator) {
        HolderLookup<DimensionManager> holderlookup = holderlookup_a.lookupOrThrow(Registries.DIMENSION_TYPE);
        Map<ResourceKey<WorldDimension>, WorldDimension> map = withOverworld(holderlookup, this.dimensions, chunkgenerator);

        return new WorldDimensions(map);
    }

    public static Map<ResourceKey<WorldDimension>, WorldDimension> withOverworld(HolderLookup<DimensionManager> holderlookup, Map<ResourceKey<WorldDimension>, WorldDimension> map, ChunkGenerator chunkgenerator) {
        WorldDimension worlddimension = (WorldDimension) map.get(WorldDimension.OVERWORLD);
        Holder<DimensionManager> holder = (Holder<DimensionManager>) (worlddimension == null ? holderlookup.getOrThrow(BuiltinDimensionTypes.OVERWORLD) : worlddimension.type());
    ...
    public static Map<ResourceKey<WorldDimension>, WorldDimension> withOverworld(Map<ResourceKey<WorldDimension>, WorldDimension> map, Holder<DimensionManager> holder, ChunkGenerator chunkgenerator) {
        ImmutableMap.Builder<ResourceKey<WorldDimension>, WorldDimension> immutablemap_builder = ImmutableMap.builder();

        immutablemap_builder.putAll(map);
        immutablemap_builder.put(WorldDimension.OVERWORLD, new WorldDimension(holder, chunkgenerator));
    ...
    public WorldDimensions.b bake(IRegistry<WorldDimension> iregistry) {
        ...
        IRegistryWritable<WorldDimension> iregistrywritable = new RegistryMaterials<WorldDimension>(Registries.LEVEL_STEM, lifecycle);

        list.forEach((a0) -> {
            iregistrywritable.register(a0.key, a0.value, a0.registrationInfo());
        });
