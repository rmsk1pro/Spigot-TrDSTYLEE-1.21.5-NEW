        ...
        for (int k = -i; k <= i; ++k) {
            ...
            for (int l = -j; l <= j; ++l) {
                ...
                if (!flag3 && (!flag4 || vegetationpatchconfiguration.extraEdgeColumnChance != 0.0F && randomsource.nextFloat() <= vegetationpatchconfiguration.extraEdgeColumnChance)) {
                    ...
                    for (int i1 = 0; generatoraccessseed.isStateAtPosition(blockposition_mutableblockposition, BlockBase.BlockData::isAir) && i1 < vegetationpatchconfiguration.verticalRange; ++i1) {
                        blockposition_mutableblockposition.move(enumdirection);
                    }

                    for (int j1 = 0; generatoraccessseed.isStateAtPosition(blockposition_mutableblockposition, (iblockdata) -> {
                        return !iblockdata.isAir();
                    }) && j1 < vegetationpatchconfiguration.verticalRange; ++j1) {
                    ...
                    if (generatoraccessseed.isEmptyBlock(blockposition_mutableblockposition) && iblockdata.isFaceSturdy(generatoraccessseed, blockposition_mutableblockposition1, vegetationpatchconfiguration.surface.getDirection().getOpposite())) {
                        int k1 = vegetationpatchconfiguration.depth.sample(randomsource) + (vegetationpatchconfiguration.extraBottomBlockChance > 0.0F && randomsource.nextFloat() < vegetationpatchconfiguration.extraBottomBlockChance ? 1 : 0);
                        BlockPosition blockposition1 = blockposition_mutableblockposition1.immutable();
                        boolean flag5 = this.placeGround(generatoraccessseed, vegetationpatchconfiguration, predicate, randomsource, blockposition_mutableblockposition1, k1);
