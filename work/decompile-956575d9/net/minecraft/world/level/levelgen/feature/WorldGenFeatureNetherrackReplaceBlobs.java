...
public class WorldGenFeatureNetherrackReplaceBlobs extends WorldGenerator<WorldGenFeatureRadiusConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<WorldGenFeatureRadiusConfiguration> featureplacecontext) {
        WorldGenFeatureRadiusConfiguration worldgenfeatureradiusconfiguration = featureplacecontext.config();
        GeneratorAccessSeed generatoraccessseed = featureplacecontext.level();
        RandomSource randomsource = featureplacecontext.random();
        Block block = worldgenfeatureradiusconfiguration.targetState.getBlock();
        BlockPosition blockposition = findTarget(generatoraccessseed, featureplacecontext.origin().mutable().clamp(EnumDirection.EnumAxis.Y, generatoraccessseed.getMinY() + 1, generatoraccessseed.getMaxY()), block);
        ...
        if (blockposition == null) {
            ...
            for (BlockPosition blockposition1 : BlockPosition.withinManhattan(blockposition, i, j, k)) {
    ...
    private static BlockPosition findTarget(GeneratorAccess generatoraccess, BlockPosition.MutableBlockPosition blockposition_mutableblockposition, Block block) {
        while (blockposition_mutableblockposition.getY() > generatoraccess.getMinY() + 1) {
