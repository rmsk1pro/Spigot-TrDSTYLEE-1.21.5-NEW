...
public class TrunkPlacerFancy extends TrunkPlacer {
    ...
    public static final MapCodec<TrunkPlacerFancy> CODEC = RecordCodecBuilder.mapCodec((instance) -> {
    ...
    public List<WorldGenFoilagePlacer.a> placeTrunk(VirtualLevelReadable virtuallevelreadable, BiConsumer<BlockPosition, IBlockData> biconsumer, RandomSource randomsource, int i, BlockPosition blockposition, WorldGenFeatureTreeConfiguration worldgenfeaturetreeconfiguration) {
        ...
        double d0 = 1.0D;
        int i1 = Math.min(1, MathHelper.floor(1.382D + Math.pow(1.0D * (double) k / 13.0D, 2.0D)));
        int j1 = blockposition.getY() + l;
        int k1 = k - 5;
        List<TrunkPlacerFancy.a> list = Lists.newArrayList();

        list.add(new TrunkPlacerFancy.a(blockposition.above(k1), j1));
        ...
        for (; k1 >= 0; --k1) {
            ...
            if (f >= 0.0F) {
                for (int l1 = 0; l1 < i1; ++l1) {
                    ...
                    BlockPosition blockposition1 = blockposition.offset(MathHelper.floor(d4), k1 - 1, MathHelper.floor(d5));
                    ...
                    if (this.makeLimb(virtuallevelreadable, biconsumer, randomsource, blockposition1, blockposition2, false, worldgenfeaturetreeconfiguration)) {
                        int i2 = blockposition.getX() - blockposition1.getX();
                        int j2 = blockposition.getZ() - blockposition1.getZ();
                        double d6 = (double) blockposition1.getY() - Math.sqrt((double) (i2 * i2 + j2 * j2)) * 0.381D;
                        int k2 = d6 > (double) j1 ? j1 : (int) d6;
                        BlockPosition blockposition3 = new BlockPosition(blockposition.getX(), k2, blockposition.getZ());
        ...
        this.makeLimb(virtuallevelreadable, biconsumer, randomsource, blockposition, blockposition.above(l), true, worldgenfeaturetreeconfiguration);
        this.makeBranches(virtuallevelreadable, biconsumer, randomsource, k, blockposition, list, worldgenfeaturetreeconfiguration);
        List<WorldGenFoilagePlacer.a> list1 = Lists.newArrayList();

        for (TrunkPlacerFancy.a trunkplacerfancy_a : list) {
            if (this.trimBranches(k, trunkplacerfancy_a.getBranchBase() - blockposition.getY())) {
