...
public class WorldGenFossils extends WorldGenerator<FossilFeatureConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<FossilFeatureConfiguration> featureplacecontext) {
        ...
        StructureBoundingBox structureboundingbox = new StructureBoundingBox(chunkcoordintpair.getMinBlockX() - 16, generatoraccessseed.getMinY(), chunkcoordintpair.getMinBlockZ() - 16, chunkcoordintpair.getMaxBlockX() + 16, generatoraccessseed.getMaxY(), chunkcoordintpair.getMaxBlockZ() + 16);
        ...
        for (int k = 0; k < baseblockposition.getX(); ++k) {
        ...
        int i1 = Math.max(j - 15 - randomsource.nextInt(10), generatoraccessseed.getMinY() + 10);
        BlockPosition blockposition2 = definedstructure.getZeroPositionWithTransform(blockposition1.atY(i1), EnumBlockMirror.NONE, enumblockrotation);
        ...
        if (countEmptyCorners(generatoraccessseed, definedstructure.getBoundingBox(definedstructureinfo, blockposition2)) > fossilfeatureconfiguration.maxEmptyCornersAllowed) {
            ...
            definedstructure.placeInWorld(generatoraccessseed, blockposition2, blockposition2, definedstructureinfo, randomsource, 260);
            definedstructureinfo.clearProcessors();
            list = (fossilfeatureconfiguration.overlayProcessors.value()).list();
            Objects.requireNonNull(definedstructureinfo);
            list.forEach(definedstructureinfo::addProcessor);
            definedstructure1.placeInWorld(generatoraccessseed, blockposition2, blockposition2, definedstructureinfo, randomsource, 260);
            return true;
        }
    }
