...
public abstract class WorldChunkManager implements BiomeResolver {
    ...
    protected abstract MapCodec<? extends WorldChunkManager> codec();
    ...
    public Pair<BlockPosition, Holder<BiomeBase>> findClosestBiome3d(BlockPosition blockposition, int i, int j, int k, Predicate<Holder<BiomeBase>> predicate, Climate.Sampler climate_sampler, IWorldReader iworldreader) {
        ...
        if (set.isEmpty()) {
            ...
            int[] aint = MathHelper.outFromOrigin(blockposition.getY(), iworldreader.getMinY() + 1, iworldreader.getMaxY() + 1, k).toArray();

            for (BlockPosition.MutableBlockPosition blockposition_mutableblockposition : BlockPosition.spiralAround(BlockPosition.ZERO, l, EnumDirection.EAST, EnumDirection.SOUTH)) {
                ...
                for (int i2 : aint) {
                    ...
                    Holder<BiomeBase> holder = this.getNoiseBiome(k1, j2, l1, climate_sampler);

                    if (set.contains(holder)) {
                        return Pair.of(new BlockPosition(i1, i2, j1), holder);
                    }
                }
            }
