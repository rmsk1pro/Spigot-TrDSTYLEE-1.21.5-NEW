...
public final class BiomeBase {
    ...
    public static final Codec<Holder<BiomeBase>> CODEC = RegistryFileCodec.<Holder<BiomeBase>>create(Registries.BIOME, BiomeBase.DIRECT_CODEC);
    ...
    public BiomeBase.Precipitation getPrecipitationAt(BlockPosition blockposition, int i) {
        return !this.hasPrecipitation() ? BiomeBase.Precipitation.NONE : (this.coldEnoughToSnow(blockposition, i) ? BiomeBase.Precipitation.SNOW : BiomeBase.Precipitation.RAIN);
    }

    private float getHeightAdjustedTemperature(BlockPosition blockposition, int i) {
        float f = this.climateSettings.temperatureModifier.modifyTemperature(blockposition, this.getBaseTemperature());
        int j = i + 17;

        if (blockposition.getY() > j) {
            ...
            return f - (f1 + (float) blockposition.getY() - (float) j) * 0.05F / 40.0F;
        } else {
            return f;
        }
    }
    ...
    /** @deprecated */
    @Deprecated
    public float getTemperature(BlockPosition blockposition, int i) {
        ...
        if (!Float.isNaN(f)) {
            ...
            float f1 = this.getHeightAdjustedTemperature(blockposition, i);
    ...
    public boolean shouldFreeze(IWorldReader iworldreader, BlockPosition blockposition, boolean flag) {
        if (this.warmEnoughToRain(blockposition, iworldreader.getSeaLevel())) {
            return false;
        } else {
            if (iworldreader.isInsideBuildHeight(blockposition.getY()) && iworldreader.getBrightness(EnumSkyBlock.BLOCK, blockposition) < 10) {
    ...
    public boolean coldEnoughToSnow(BlockPosition blockposition, int i) {
        return !this.warmEnoughToRain(blockposition, i);
    }

    public boolean warmEnoughToRain(BlockPosition blockposition, int i) {
        return this.getTemperature(blockposition, i) >= 0.15F;
    }

    public boolean shouldMeltFrozenOceanIcebergSlightly(BlockPosition blockposition, int i) {
        return this.getTemperature(blockposition, i) > 0.1F;
    }
    ...
    public static enum Precipitation implements INamable {
        ...
        public static final Codec<BiomeBase.Precipitation> CODEC = INamable.<BiomeBase.Precipitation>fromEnum(BiomeBase.Precipitation::values);
        private final String name;

        private Precipitation(final String s) {
            this.name = s;
        }
    ...
    public static enum TemperatureModifier implements INamable {
        ...
        private final String name;
        public static final Codec<BiomeBase.TemperatureModifier> CODEC = INamable.<BiomeBase.TemperatureModifier>fromEnum(BiomeBase.TemperatureModifier::values);

        public abstract float modifyTemperature(BlockPosition blockposition, float f);

        TemperatureModifier(final String s) {
            this.name = s;
        }
