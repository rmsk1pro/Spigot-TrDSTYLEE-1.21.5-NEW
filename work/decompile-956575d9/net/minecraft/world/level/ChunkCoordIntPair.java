...
public class ChunkCoordIntPair {
    ...
    public static final Codec<ChunkCoordIntPair> CODEC = Codec.INT_STREAM.comapFlatMap((intstream) -> {
        return SystemUtils.fixedSize(intstream, 2).map((aint) -> {
            return new ChunkCoordIntPair(aint[0], aint[1]);
        });
    }, (chunkcoordintpair) -> {
        return IntStream.of(new int[]{chunkcoordintpair.x, chunkcoordintpair.z});
    }).stable();
    public static final StreamCodec<ByteBuf, ChunkCoordIntPair> STREAM_CODEC = new StreamCodec<ByteBuf, ChunkCoordIntPair>() {
        public ChunkCoordIntPair decode(ByteBuf bytebuf) {
            return PacketDataSerializer.readChunkPos(bytebuf);
        }

        public void encode(ByteBuf bytebuf, ChunkCoordIntPair chunkcoordintpair) {
            PacketDataSerializer.writeChunkPos(bytebuf, chunkcoordintpair);
        }
    };
    ...
    public static final int MAX_COORDINATE_VALUE = SectionPosition.blockToSectionCoord(BlockPosition.MAX_HORIZONTAL_COORDINATE) - ChunkCoordIntPair.SAFETY_MARGIN_CHUNKS;
    ...
    public int getChessboardDistance(ChunkCoordIntPair chunkcoordintpair) {
        return this.getChessboardDistance(chunkcoordintpair.x, chunkcoordintpair.z);
    }
    ...
    public int distanceSquared(ChunkCoordIntPair chunkcoordintpair) {
        return this.distanceSquared(chunkcoordintpair.x, chunkcoordintpair.z);
    }

    public int distanceSquared(long i) {
        return this.distanceSquared(getX(i), getZ(i));
    }
    ...
    public static Stream<ChunkCoordIntPair> rangeClosed(final ChunkCoordIntPair chunkcoordintpair, final ChunkCoordIntPair chunkcoordintpair1) {
        ...
        return StreamSupport.stream(new Spliterators.AbstractSpliterator<ChunkCoordIntPair>((long) (i * j), 64) {
            @Nullable
