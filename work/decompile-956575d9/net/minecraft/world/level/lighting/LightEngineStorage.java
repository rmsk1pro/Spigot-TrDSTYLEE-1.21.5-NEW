...
public abstract class LightEngineStorage<M extends LightEngineStorageArray<M>> {
    ...
    protected LightEngineStorage(EnumSkyBlock enumskyblock, ILightAccess ilightaccess, M m0) {
        ...
        this.visibleSectionData = ((LightEngineStorageArray) m0).copy();
    ...
    protected void markNewInconsistencies(LightEngine<M, ?> lightengine) {
        if (this.hasInconsistencies) {
            ...
            while (longiterator.hasNext()) {
                ...
                NibbleArray nibblearray = (NibbleArray) this.queuedSections.remove(i);
            ...
            this.toRemove.clear();
            ObjectIterator<Long2ObjectMap.Entry<NibbleArray>> objectiterator = Long2ObjectMaps.fastIterator(this.queuedSections);

            while (objectiterator.hasNext()) {
                Long2ObjectMap.Entry<NibbleArray> long2objectmap_entry = (Entry) objectiterator.next();
                long k = long2objectmap_entry.getLongKey();

                if (this.storingLightForSection(k)) {
                    NibbleArray nibblearray2 = (NibbleArray) long2objectmap_entry.getValue();

                    if (this.updatingSectionData.getLayer(k) != nibblearray2) {
                        this.updatingSectionData.setLayer(k, nibblearray2);
    ...
    protected boolean lightOnInColumn(long i) {
