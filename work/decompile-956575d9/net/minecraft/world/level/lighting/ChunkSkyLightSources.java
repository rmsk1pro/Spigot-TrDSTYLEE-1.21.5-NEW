...
public class ChunkSkyLightSources {
    ...
    private int findLowestSourceY(IChunkAccess ichunkaccess, int i, int j, int k) {
        ...
        for (int i1 = i; i1 >= 0; --i1) {
            ...
            if (chunksection.hasOnlyAir()) {
                ...
                int j1 = ichunkaccess.getSectionYFromSectionIndex(i1);
                ...
                for (int k1 = 15; k1 >= 0; --k1) {
                    IBlockData iblockdata1 = chunksection.getBlockState(j, k1, k);

                    if (isEdgeOccluded(iblockdata, iblockdata1)) {
    ...
    public boolean update(IBlockAccess iblockaccess, int i, int j, int k) {
        ...
        if (l < j1) {
            ...
            BlockPosition blockposition = this.mutablePos1.set(i, j + 1, k);
            IBlockData iblockdata = iblockaccess.getBlockState(blockposition);
            BlockPosition blockposition1 = this.mutablePos2.set(i, j, k);
            IBlockData iblockdata1 = iblockaccess.getBlockState(blockposition1);

            if (this.updateEdge(iblockaccess, i1, j1, blockposition, iblockdata, blockposition1, iblockdata1)) {
                return true;
            } else {
                BlockPosition blockposition2 = this.mutablePos1.set(i, j - 1, k);
                IBlockData iblockdata2 = iblockaccess.getBlockState(blockposition2);

                return this.updateEdge(iblockaccess, i1, j1, blockposition1, iblockdata1, blockposition2, iblockdata2);
            }
        }
    }
    ...
    private int findLowestSourceBelow(IBlockAccess iblockaccess, BlockPosition blockposition, IBlockData iblockdata) {
        ...
        while (blockposition_mutableblockposition1.getY() >= this.minY) {
            ...
            if (isEdgeOccluded(iblockdata1, iblockdata2)) {
                return blockposition_mutableblockposition.getY();
            }
    ...
    private static boolean isEdgeOccluded(IBlockData iblockdata, IBlockData iblockdata1) {
        if (iblockdata1.getLightBlock() != 0) {
            return true;
        } else {
            VoxelShape voxelshape = LightEngine.getOcclusionShape(iblockdata, EnumDirection.DOWN);
            VoxelShape voxelshape1 = LightEngine.getOcclusionShape(iblockdata1, EnumDirection.UP);
