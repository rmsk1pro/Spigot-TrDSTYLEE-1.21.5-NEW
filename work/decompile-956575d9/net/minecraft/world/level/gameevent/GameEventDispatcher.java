    ...
    public void post(Holder<GameEvent> holder, Vec3D vec3d, GameEvent.a gameevent_a) {
        ...
        for (int l1 = j; l1 <= i1; ++l1) {
            for (int i2 = l; i2 <= k1; ++i2) {
                IChunkAccess ichunkaccess = this.level.getChunkSource().getChunkNow(l1, i2);

                if (ichunkaccess != null) {
                    for (int j2 = k; j2 <= j1; ++j2) {
                        flag |= ichunkaccess.getListenerRegistry(j2).visitInRangeListeners(holder, vec3d, gameevent_a, gameeventlistenerregistry_a);
                    }
                }
            }
        }
