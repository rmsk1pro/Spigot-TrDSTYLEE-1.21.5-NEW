...
public abstract class IBlockDataHolder<O, S> {
    ...
    public static final Function<Map.Entry<IBlockState<?>, Comparable<?>>, String> PROPERTY_ENTRY_TO_STRING_FUNCTION = new Function<Map.Entry<IBlockState<?>, Comparable<?>>, String>() {
        public String apply(@Nullable Map.Entry<IBlockState<?>, Comparable<?>> map_entry) {
            if (map_entry == null) {
                return "<NULL>";
            } else {
                ...
                return s + "=" + this.getName(iblockstate, (Comparable) map_entry.getValue());
            }
        }
    ...
    public <T extends Comparable<T>> S cycle(IBlockState<T> iblockstate) {
        return (S) this.setValue(iblockstate, (Comparable) findNextInCollection(iblockstate.getPossibleValues(), this.getValue(iblockstate)));
    }
    ...
    public boolean hasProperty(IBlockState<?> iblockstate) {
    ...
    public <T extends Comparable<T>> T getValue(IBlockState<T> iblockstate) {
        ...
        if (comparable == null) {
            ...
            return (T) (iblockstate.getValueClass().cast(comparable));
        }
    }
    ...
    public <T extends Comparable<T>> T getValueOrElse(IBlockState<T> iblockstate, T t0) {
        return (T) (Objects.requireNonNullElse(this.getNullableValue(iblockstate), t0));
    }

    @Nullable
    private <T extends Comparable<T>> T getNullableValue(IBlockState<T> iblockstate) {
        Comparable<?> comparable = (Comparable) this.values.get(iblockstate);

        return (T) (comparable == null ? null : (Comparable) iblockstate.getValueClass().cast(comparable));
    }
    ...
    public <T extends Comparable<T>, V extends T> S setValue(IBlockState<T> iblockstate, V v0) {
        ...
        if (comparable == null) {
            ...
            return (S) this.setValueInternal(iblockstate, v0, comparable);
        }
    }
    ...
    public <T extends Comparable<T>, V extends T> S trySetValue(IBlockState<T> iblockstate, V v0) {
        ...
        return (S) (comparable == null ? this : this.setValueInternal(iblockstate, v0, comparable));
    }
    ...
    private <T extends Comparable<T>, V extends T> S setValueInternal(IBlockState<T> iblockstate, V v0, Comparable<?> comparable) {
        if (comparable.equals(v0)) {
            ...
            if (i < 0) {
                ...
                return (S) ((Object[]) this.neighbours.get(iblockstate))[i];
            }
        }
    }
    ...
    public void populateNeighbours(Map<Map<IBlockState<?>, Comparable<?>>, S> map) {
        if (this.neighbours != null) {
            ...
            while (objectiterator.hasNext()) {
                Map.Entry<IBlockState<?>, Comparable<?>> map_entry = (Entry) objectiterator.next();
                IBlockState<?> iblockstate = (IBlockState) map_entry.getKey();
