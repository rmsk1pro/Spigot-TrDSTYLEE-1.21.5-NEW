...
public class BlockStem extends VegetationBlock implements IBlockFragilePlantElement {
    ...
    protected BlockStem(ResourceKey<Block> resourcekey, ResourceKey<Block> resourcekey1, ResourceKey<Item> resourcekey2, BlockBase.Info blockbase_info) {
        ...
        this.registerDefaultState((IBlockData) (this.stateDefinition.any()).setValue(BlockStem.AGE, 0));
    }

    @Override
    protected VoxelShape getShape(IBlockData iblockdata, IBlockAccess iblockaccess, BlockPosition blockposition, VoxelShapeCollision voxelshapecollision) {
        return BlockStem.SHAPES[(Integer) iblockdata.getValue(BlockStem.AGE)];
    }
            ...
            if (randomsource.nextInt((int) (25.0F / f) + 1) == 0) {
                ...
                if (i < 7) {
                    ...
                    if (worldserver.getBlockState(blockposition1).isAir() && (iblockdata1.is(Blocks.FARMLAND) || iblockdata1.is(TagsBlock.DIRT))) {
                        IRegistry<Block> iregistry = worldserver.registryAccess().lookupOrThrow(Registries.BLOCK);
