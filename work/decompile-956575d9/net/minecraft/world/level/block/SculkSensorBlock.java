...
public class SculkSensorBlock extends BlockTileEntity implements IBlockWaterlogged {
    ...
    public SculkSensorBlock(BlockBase.Info blockbase_info) {
        ...
        this.registerDefaultState((IBlockData) ((IBlockData) ((IBlockData) (this.stateDefinition.any()).setValue(SculkSensorBlock.PHASE, SculkSensorPhase.INACTIVE)).setValue(SculkSensorBlock.POWER, 0)).setValue(SculkSensorBlock.WATERLOGGED, false));
    }
        ...
        if (getPhase(iblockdata) != SculkSensorPhase.ACTIVE) {
            if (getPhase(iblockdata) == SculkSensorPhase.COOLDOWN) {
                ...
                if (!(Boolean) iblockdata.getValue(SculkSensorBlock.WATERLOGGED)) {
                    worldserver.playSound((Entity) null, blockposition, SoundEffects.SCULK_CLICKING_STOP, SoundCategory.BLOCKS, 1.0F, worldserver.random.nextFloat() * 0.2F + 0.8F);
                }
            }
    ...
    protected void affectNeighborsAfterRemoval(IBlockData iblockdata, WorldServer worldserver, BlockPosition blockposition, boolean flag) {
        if (getPhase(iblockdata) == SculkSensorPhase.ACTIVE) {
            updateNeighbours(worldserver, blockposition, iblockdata);
        }
    ...
    public void activate(@Nullable Entity entity, World world, BlockPosition blockposition, IBlockData iblockdata, int i, int j) {
        ...
        if (!(Boolean) iblockdata.getValue(SculkSensorBlock.WATERLOGGED)) {
            world.playSound((Entity) null, (double) blockposition.getX() + 0.5D, (double) blockposition.getY() + 0.5D, (double) blockposition.getZ() + 0.5D, SoundEffects.SCULK_CLICKING, SoundCategory.BLOCKS, 1.0F, world.random.nextFloat() * 0.2F + 0.8F);
        }
    ...
    public static void tryResonateVibration(@Nullable Entity entity, World world, BlockPosition blockposition, int i) {
        for (EnumDirection enumdirection : EnumDirection.values()) {
            ...
            if (iblockdata.is(TagsBlock.VIBRATION_RESONATORS)) {
                ...
                world.playSound((Entity) null, blockposition1, SoundEffects.AMETHYST_BLOCK_RESONATE, SoundCategory.BLOCKS, 1.0F, f);
            }
        }
