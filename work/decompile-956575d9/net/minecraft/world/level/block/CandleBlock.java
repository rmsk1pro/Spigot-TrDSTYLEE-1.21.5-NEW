...
public class CandleBlock extends AbstractCandleBlock implements IBlockWaterlogged {
    ...
    private static final Int2ObjectMap<List<Vec3D>> PARTICLE_OFFSETS = (Int2ObjectMap) SystemUtils.make(new Int2ObjectOpenHashMap(4), (int2objectopenhashmap) -> {
        float f = 0.0625F;

        int2objectopenhashmap.put(1, List.of((new Vec3D(8.0D, 8.0D, 8.0D)).scale(0.0625D)));
        int2objectopenhashmap.put(2, List.of((new Vec3D(6.0D, 7.0D, 8.0D)).scale(0.0625D), (new Vec3D(10.0D, 8.0D, 7.0D)).scale(0.0625D)));
        int2objectopenhashmap.put(3, List.of((new Vec3D(8.0D, 5.0D, 10.0D)).scale(0.0625D), (new Vec3D(6.0D, 7.0D, 8.0D)).scale(0.0625D), (new Vec3D(9.0D, 8.0D, 7.0D)).scale(0.0625D)));
        int2objectopenhashmap.put(4, List.of((new Vec3D(7.0D, 5.0D, 9.0D)).scale(0.0625D), (new Vec3D(10.0D, 7.0D, 9.0D)).scale(0.0625D), (new Vec3D(6.0D, 7.0D, 6.0D)).scale(0.0625D), (new Vec3D(9.0D, 8.0D, 6.0D)).scale(0.0625D)));
    });
    ...
    public CandleBlock(BlockBase.Info blockbase_info) {
        ...
        this.registerDefaultState((IBlockData) ((IBlockData) ((IBlockData) (this.stateDefinition.any()).setValue(CandleBlock.CANDLES, 1)).setValue(CandleBlock.LIT, false)).setValue(CandleBlock.WATERLOGGED, false));
    }
    ...
    protected EnumInteractionResult useItemOn(ItemStack itemstack, IBlockData iblockdata, World world, BlockPosition blockposition, EntityHuman entityhuman, EnumHand enumhand, MovingObjectPositionBlock movingobjectpositionblock) {
        if (itemstack.isEmpty() && entityhuman.getAbilities().mayBuild && (Boolean) iblockdata.getValue(CandleBlock.LIT)) {
    ...
    protected VoxelShape getShape(IBlockData iblockdata, IBlockAccess iblockaccess, BlockPosition blockposition, VoxelShapeCollision voxelshapecollision) {
        return CandleBlock.SHAPES[(Integer) iblockdata.getValue(CandleBlock.CANDLES) - 1];
    }
