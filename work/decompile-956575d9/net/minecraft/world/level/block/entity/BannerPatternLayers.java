...
public record BannerPatternLayers(List<BannerPatternLayers.b> layers) implements TooltipProvider {
    ...
    public void addToTooltip(Item.b item_b, Consumer<IChatBaseComponent> consumer, TooltipFlag tooltipflag, DataComponentGetter datacomponentgetter) {
        for (int i = 0; i < Math.min(this.layers().size(), 6); ++i) {
            consumer.accept(((BannerPatternLayers.b) this.layers().get(i)).description().withStyle(EnumChatFormat.GRAY));
        }
    ...
    public static class a {
        ...
        private final ImmutableList.Builder<BannerPatternLayers.b> layers = ImmutableList.builder();
