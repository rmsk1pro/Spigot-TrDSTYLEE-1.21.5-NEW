...
public class CreakingHeartBlockEntity extends TileEntity {
        ...
        if (world instanceof WorldServer worldserver) {
            ...
            if (creakingheartblockentity.emitter > 0) {
                ...
                if (creakingheartblockentity.emitter % 10 == 0 && creakingheartblockentity.emitterTarget != null) {
                    ...
                    worldserver.playSound((Entity) null, blockposition1, SoundEffects.CREAKING_HEART_HURT, SoundCategory.BLOCKS, f1, 1.0F);
                }
            ...
            if (creakingheartblockentity.ticker-- < 0) {
                ...
                IBlockData iblockdata1 = updateCreakingState(world, iblockdata, blockposition, creakingheartblockentity);

                if (iblockdata1 != iblockdata) {
                    world.setBlock(blockposition, iblockdata1, 3);
                    if (iblockdata1.getValue(CreakingHeartBlock.STATE) == CreakingHeartState.UPROOTED) {
                        return;
                    }
                }

                if (creakingheartblockentity.creakingInfo == null) {
                    if (iblockdata1.getValue(CreakingHeartBlock.STATE) == CreakingHeartState.AWAKE) {
                        if (world.getDifficulty() != EnumDifficulty.PEACEFUL) {
                            if (worldserver.getGameRules().getBoolean(GameRules.RULE_DOMOBSPAWNING)) {
                                ...
                                if (entityhuman != null) {
                                    Creaking creaking = spawnProtector(worldserver, creakingheartblockentity);
                    ...
                    if (optional.isPresent()) {
                        ...
                        if (!CreakingHeartBlock.isNaturalNight(world) && !creaking1.isPersistenceRequired() || creakingheartblockentity.distanceToCreaking() > 34.0D || creaking1.playerIsStuckInYou()) {
    ...
    private static IBlockData updateCreakingState(World world, IBlockData iblockdata, BlockPosition blockposition, CreakingHeartBlockEntity creakingheartblockentity) {
        if (!CreakingHeartBlock.hasRequiredLogs(iblockdata, world, blockposition) && creakingheartblockentity.creakingInfo == null) {
            ...
            boolean flag = CreakingHeartBlock.isNaturalNight(world);

            return (IBlockData) iblockdata.setValue(CreakingHeartBlock.STATE, flag ? CreakingHeartState.AWAKE : CreakingHeartState.DORMANT);
        }
    }
    ...
    private static Creaking spawnProtector(WorldServer worldserver, CreakingHeartBlockEntity creakingheartblockentity) {
        ...
        Optional<Creaking> optional = SpawnUtil.<Creaking>trySpawnMob(EntityTypes.CREAKING, EntitySpawnReason.SPAWNER, worldserver, blockposition, 5, 16, 8, SpawnUtil.a.ON_TOP_OF_COLLIDER_NO_LEAVES, true);
        ...
        if (optional.isEmpty()) {
            ...
            worldserver.gameEvent(creaking, (Holder) GameEvent.ENTITY_PLACE, creaking.position());
                    ...
                    if (this.getBlockState().getValue(CreakingHeartBlock.STATE) == CreakingHeartState.AWAKE) {
                        ...
                        for (int j = 0; j < i; ++j) {
                            this.spreadResin().ifPresent((blockposition) -> {
                                ...
                                this.level.gameEvent(GameEvent.BLOCK_PLACE, blockposition, GameEvent.a.of(this.getBlockState()));
                            });
                        }
                    }
        ...
        BlockPosition.breadthFirstTraversal(this.worldPosition, 2, 64, (blockposition, consumer) -> {
            ...
            if (!this.level.getBlockState(blockposition).is(TagsBlock.PALE_OAK_LOGS)) {
                ...
                for (EnumDirection enumdirection : SystemUtils.shuffledCopy(EnumDirection.values(), this.level.random)) {
                    ...
                    if (iblockdata.is(Blocks.RESIN_CLUMP) && !MultifaceBlock.hasFace(iblockdata, enumdirection1)) {
                        this.level.setBlock(blockposition1, (IBlockData) iblockdata.setValue(MultifaceBlock.getFaceProperty(enumdirection1), true), 3);
    ...
    protected void saveAdditional(NBTTagCompound nbttagcompound, HolderLookup.a holderlookup_a) {
        ...
        if (this.creakingInfo != null) {
            nbttagcompound.store("creaking", UUIDUtil.CODEC, (UUID) this.creakingInfo.map(Entity::getUUID, (uuid) -> {
                return uuid;
            }));
        }
