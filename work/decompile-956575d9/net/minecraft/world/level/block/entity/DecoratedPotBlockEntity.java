...
public class DecoratedPotBlockEntity extends TileEntity implements RandomizableContainer, ContainerSingleItem.a {
    ...
    protected void saveAdditional(NBTTagCompound nbttagcompound, HolderLookup.a holderlookup_a) {
        ...
        if (!this.decorations.equals(PotDecorations.EMPTY)) {
            nbttagcompound.store("sherds", PotDecorations.CODEC, this.decorations);
        }

        if (!this.trySaveLootTable(nbttagcompound) && !this.item.isEmpty()) {
            nbttagcompound.store("item", ItemStack.CODEC, holderlookup_a.createSerializationContext(DynamicOpsNBT.INSTANCE), this.item);
        }
    ...
    protected void loadAdditional(NBTTagCompound nbttagcompound, HolderLookup.a holderlookup_a) {
        ...
        this.decorations = (PotDecorations) nbttagcompound.read("sherds", PotDecorations.CODEC).orElse(PotDecorations.EMPTY);
        if (!this.tryLoadLootTable(nbttagcompound)) {
            ...
            this.item = (ItemStack) nbttagcompound.read("item", ItemStack.CODEC, registryops).orElse(ItemStack.EMPTY);
        } else {
            this.item = ItemStack.EMPTY;
        }
