...
public final class TrialSpawner {
    ...
    public TrialSpawner(TrialSpawner.b trialspawner_b, PlayerDetector playerdetector, PlayerDetector.a playerdetector_a) {
        this(Holder.direct(TrialSpawnerConfig.DEFAULT), Holder.direct(TrialSpawnerConfig.DEFAULT), new TrialSpawnerData(), 36000, 14, trialspawner_b, playerdetector, playerdetector_a);
    }

    public TrialSpawner(Holder<TrialSpawnerConfig> holder, Holder<TrialSpawnerConfig> holder1, TrialSpawnerData trialspawnerdata, int i, int j, TrialSpawner.b trialspawner_b, PlayerDetector playerdetector, PlayerDetector.a playerdetector_a) {
        this.normalConfig = holder;
        this.ominousConfig = holder1;
    ...
    public Optional<UUID> spawnMob(WorldServer worldserver, BlockPosition blockposition) {
        ...
        if (optional.isEmpty()) {
            ...
            Vec3D vec3d = (Vec3D) nbttagcompound.read("Pos", Vec3D.CODEC).orElseGet(() -> {
                return new Vec3D((double) blockposition.getX() + (randomsource.nextDouble() - randomsource.nextDouble()) * (double) this.getConfig().spawnRange() + 0.5D, (double) (blockposition.getY() + randomsource.nextInt(3) - 1), (double) blockposition.getZ() + (randomsource.nextDouble() - randomsource.nextDouble()) * (double) this.getConfig().spawnRange() + 0.5D);
            });
            ...
            if (!worldserver.noCollision(((EntityTypes) optional.get()).getSpawnAABB(vec3d.x, vec3d.y, vec3d.z))) {
                ...
                if (!EntityPositionTypes.checkSpawnRules((EntityTypes) optional.get(), worldserver, EntitySpawnReason.TRIAL_SPAWNER, blockposition1, worldserver.getRandom())) {
                    ...
                    Entity entity = EntityTypes.loadEntityRecursive(nbttagcompound, worldserver, EntitySpawnReason.TRIAL_SPAWNER, (entity1) -> {
    ...
    public void overrideEntityToSpawn(EntityTypes<?> entitytypes, World world) {
        this.data.reset();
        this.normalConfig = Holder.<TrialSpawnerConfig>direct((this.normalConfig.value()).withSpawning(entitytypes));
        this.ominousConfig = Holder.<TrialSpawnerConfig>direct((this.ominousConfig.value()).withSpawning(entitytypes));
