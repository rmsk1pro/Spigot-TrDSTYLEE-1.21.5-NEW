...
public class HangingMossBlock extends Block implements IBlockFragilePlantElement {
    ...
    public HangingMossBlock(BlockBase.Info blockbase_info) {
        ...
        this.registerDefaultState((IBlockData) (this.stateDefinition.any()).setValue(HangingMossBlock.TIP, true));
    }

    @Override
    protected VoxelShape getShape(IBlockData iblockdata, IBlockAccess iblockaccess, BlockPosition blockposition, VoxelShapeCollision voxelshapecollision) {
        return (Boolean) iblockdata.getValue(HangingMossBlock.TIP) ? HangingMossBlock.SHAPE_TIP : HangingMossBlock.SHAPE_BASE;
    }
    ...
    public void animateTick(IBlockData iblockdata, World world, BlockPosition blockposition, RandomSource randomsource) {
        if (randomsource.nextInt(500) == 0) {
            ...
            if (iblockdata1.is(TagsBlock.PALE_OAK_LOGS) || iblockdata1.is(Blocks.PALE_OAK_LEAVES)) {
                world.playLocalSound((double) blockposition.getX(), (double) blockposition.getY(), (double) blockposition.getZ(), SoundEffects.PALE_HANGING_MOSS_IDLE, SoundCategory.AMBIENT, 1.0F, 1.0F, false);
            }
        }
    ...
    protected IBlockData updateShape(IBlockData iblockdata, IWorldReader iworldreader, ScheduledTickAccess scheduledtickaccess, BlockPosition blockposition, EnumDirection enumdirection, BlockPosition blockposition1, IBlockData iblockdata1, RandomSource randomsource) {
        ...
        return (IBlockData) iblockdata.setValue(HangingMossBlock.TIP, !iworldreader.getBlockState(blockposition.below()).is(this));
    }
