    ...
    private boolean addBlockLine(BlockPosition blockposition, EnumDirection enumdirection) {
        ...
        if (iblockdata.isAir()) {
            ...
            if (i + this.toPush.size() > 12) {
                ...
                while (true) {
                    BlockPosition blockposition2 = blockposition.relative(this.pushDirection, l);
                    int i1 = this.toPush.indexOf(blockposition2);

                    if (i1 > -1) {
                        this.reorderListAtCollision(j, i1);

                        for (int j1 = 0; j1 <= i1 + j; ++j1) {
                            BlockPosition blockposition3 = (BlockPosition) this.toPush.get(j1);
