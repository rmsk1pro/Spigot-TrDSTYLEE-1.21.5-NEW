...
public class TileEntityPiston extends TileEntity {
    ...
    public TileEntityPiston(BlockPosition blockposition, IBlockData iblockdata) {
        ...
        this.movedState = TileEntityPiston.DEFAULT_BLOCK_STATE;
    ...
    private static void moveEntityByPiston(EnumDirection enumdirection, Entity entity, double d0, EnumDirection enumdirection1) {
        ...
        entity.applyEffectsFromBlocks(vec3d, entity.position());
    ...
    private static void moveStuckEntities(World world, BlockPosition blockposition, float f, TileEntityPiston tileentitypiston) {
        if (tileentitypiston.isStickyForEntities()) {
            ...
            if (enumdirection.getAxis().isHorizontal()) {
                ...
                AxisAlignedBB axisalignedbb = moveByPositionAndProgress(blockposition, new AxisAlignedBB(0.0D, d0, 0.0D, 1.0D, 1.5000010000000001D, 1.0D), tileentitypiston);
                double d1 = (double) (f - tileentitypiston.progress);

                for (Entity entity : world.getEntities((Entity) null, axisalignedbb, (entity1) -> {
                    return matchesStickyCritera(axisalignedbb, entity1, blockposition);
                })) {
    ...
    private static boolean matchesStickyCritera(AxisAlignedBB axisalignedbb, Entity entity, BlockPosition blockposition) {
        return entity.getPistonPushReaction() == EnumPistonReaction.NORMAL && entity.onGround() && (entity.isSupportedBy(blockposition) || entity.getX() >= axisalignedbb.minX && entity.getX() <= axisalignedbb.maxX && entity.getZ() >= axisalignedbb.minZ && entity.getZ() <= axisalignedbb.maxZ);
    }
    ...
    public void finalTick() {
        if (this.level != null && (this.progressO < 1.0F || this.level.isClientSide)) {
            ...
            if (this.level.getBlockState(this.worldPosition).is(Blocks.MOVING_PISTON)) {
                ...
                this.level.neighborChanged(this.worldPosition, iblockdata.getBlock(), ExperimentalRedstoneUtils.initialOrientation(this.level, this.getPushDirection(), (EnumDirection) null));
            }
        }
            ...
            if (world.isClientSide && tileentitypiston.deathTicks < 5) {
                ...
                if (world.getBlockState(blockposition).is(Blocks.MOVING_PISTON)) {
                    ...
                    if (iblockdata1.isAir()) {
                        world.setBlock(blockposition, tileentitypiston.movedState, 340);
                        ...
                        world.neighborChanged(blockposition, iblockdata1.getBlock(), ExperimentalRedstoneUtils.initialOrientation(world, tileentitypiston.getPushDirection(), (EnumDirection) null));
                    }
                }
    ...
    protected void loadAdditional(NBTTagCompound nbttagcompound, HolderLookup.a holderlookup_a) {
        ...
        this.movedState = (IBlockData) nbttagcompound.read("blockState", IBlockData.CODEC, registryops).orElse(TileEntityPiston.DEFAULT_BLOCK_STATE);
        this.direction = (EnumDirection) nbttagcompound.read("facing", EnumDirection.LEGACY_ID_CODEC).orElse(EnumDirection.DOWN);
        this.progress = nbttagcompound.getFloatOr("progress", 0.0F);
        this.progressO = this.progress;
        this.extending = nbttagcompound.getBooleanOr("extending", false);
        this.isSourcePiston = nbttagcompound.getBooleanOr("source", false);
    }
    ...
    protected void saveAdditional(NBTTagCompound nbttagcompound, HolderLookup.a holderlookup_a) {
        ...
        nbttagcompound.store("blockState", IBlockData.CODEC, registryops, this.movedState);
        nbttagcompound.store("facing", EnumDirection.LEGACY_ID_CODEC, this.direction);
