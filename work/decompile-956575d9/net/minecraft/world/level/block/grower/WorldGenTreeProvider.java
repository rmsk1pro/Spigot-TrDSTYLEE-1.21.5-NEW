...
public final class WorldGenTreeProvider {
    ...
    public boolean growTree(WorldServer worldserver, ChunkGenerator chunkgenerator, BlockPosition blockposition, IBlockData iblockdata, RandomSource randomsource) {
                ...
                for (int i = 0; i >= -1; --i) {
                    for (int j = 0; j >= -1; --j) {
                        if (isTwoByTwoSapling(iblockdata, worldserver, blockposition, i, j)) {
                            WorldGenFeatureConfigured<?, ?> worldgenfeatureconfigured = holder.value();
                            IBlockData iblockdata1 = Blocks.AIR.defaultBlockState();

                            worldserver.setBlock(blockposition.offset(i, 0, j), iblockdata1, 260);
                            worldserver.setBlock(blockposition.offset(i + 1, 0, j), iblockdata1, 260);
                            worldserver.setBlock(blockposition.offset(i, 0, j + 1), iblockdata1, 260);
                            worldserver.setBlock(blockposition.offset(i + 1, 0, j + 1), iblockdata1, 260);
                            if (worldgenfeatureconfigured.place(worldserver, chunkgenerator, randomsource, blockposition.offset(i, 0, j))) {
                                return true;
                            }

                            worldserver.setBlock(blockposition.offset(i, 0, j), iblockdata, 260);
                            worldserver.setBlock(blockposition.offset(i + 1, 0, j), iblockdata, 260);
                            worldserver.setBlock(blockposition.offset(i, 0, j + 1), iblockdata, 260);
                            worldserver.setBlock(blockposition.offset(i + 1, 0, j + 1), iblockdata, 260);
                            return false;
                        }
                    }
                }
        ...
        if (resourcekey1 == null) {
            ...
            if (holder1 == null) {
                ...
                worldserver.setBlock(blockposition, iblockdata2, 260);
    ...
    private boolean hasFlowers(GeneratorAccess generatoraccess, BlockPosition blockposition) {
        for (BlockPosition blockposition1 : BlockPosition.MutableBlockPosition.betweenClosed(blockposition.below().north(2).west(2), blockposition.above().south(2).east(2))) {
            if (generatoraccess.getBlockState(blockposition1).is(TagsBlock.FLOWERS)) {
                return true;
            }
        }
    ...
    static {
        ...
        PALE_OAK = new WorldGenTreeProvider("pale_oak", Optional.of(TreeFeatures.PALE_OAK_BONEMEAL), Optional.empty(), Optional.empty());
    }
}
