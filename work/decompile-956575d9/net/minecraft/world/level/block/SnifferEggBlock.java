...
public class SnifferEggBlock extends Block {
    ...
    public SnifferEggBlock(BlockBase.Info blockbase_info) {
        ...
        this.registerDefaultState((IBlockData) (this.stateDefinition.any()).setValue(SnifferEggBlock.HATCH, 0));
    }
    ...
    public void tick(IBlockData iblockdata, WorldServer worldserver, BlockPosition blockposition, RandomSource randomsource) {
        if (!this.isReadyToHatch(iblockdata)) {
            worldserver.playSound((Entity) null, blockposition, SoundEffects.SNIFFER_EGG_CRACK, SoundCategory.BLOCKS, 0.7F, 0.9F + randomsource.nextFloat() * 0.2F);
            ...
            worldserver.playSound((Entity) null, blockposition, SoundEffects.SNIFFER_EGG_HATCH, SoundCategory.BLOCKS, 0.7F, 0.9F + randomsource.nextFloat() * 0.2F);
            worldserver.destroyBlock(blockposition, false);
            Sniffer sniffer = EntityTypes.SNIFFER.create(worldserver, EntitySpawnReason.BREEDING);

            if (sniffer != null) {
                Vec3D vec3d = blockposition.getCenter();

                sniffer.setBaby(true);
                sniffer.snapTo(vec3d.x(), vec3d.y(), vec3d.z(), MathHelper.wrapDegrees(worldserver.random.nextFloat() * 360.0F), 0.0F);
