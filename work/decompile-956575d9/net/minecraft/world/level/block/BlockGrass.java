...
public class BlockGrass extends BlockDirtSnowSpreadable implements IBlockFragilePlantElement {
    ...
    public void performBonemeal(WorldServer worldserver, RandomSource randomsource, BlockPosition blockposition, IBlockData iblockdata) {
        ...
        Optional<Holder.c<PlacedFeature>> optional = worldserver.registryAccess().lookupOrThrow(Registries.PLACED_FEATURE).get(VegetationPlacements.GRASS_BONEMEAL);
        ...
        for (int i = 0; i < 128; ++i) {
            ...
            for (int j = 0; j < i / 16; ++j) {
                ...
                if (!worldserver.getBlockState(blockposition2.below()).is(this) || worldserver.getBlockState(blockposition2).isCollisionShapeFullBlock(worldserver, blockposition2)) {
                    continue label51;
                }
            }
            ...
            if (iblockdata2.is(iblockdata1.getBlock()) && randomsource.nextInt(10) == 0) {
                IBlockFragilePlantElement iblockfragileplantelement = (IBlockFragilePlantElement) iblockdata1.getBlock();

                if (iblockfragileplantelement.isValidBonemealTarget(worldserver, blockposition2, iblockdata2)) {
                    iblockfragileplantelement.performBonemeal(worldserver, randomsource, blockposition2, iblockdata2);
                }
            }
            ...
            if (iblockdata2.isAir()) {
                ...
                if (randomsource.nextInt(8) == 0) {
                    ...
                    holder = ((WorldGenFeatureRandomPatchConfiguration) ((WorldGenFeatureConfigured) list.get(k)).config()).feature();
                } else {
