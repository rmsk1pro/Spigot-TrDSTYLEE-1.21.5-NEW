...
public class PathfinderNormal extends PathfinderAbstract {
    ...
    public PathPoint getStart() {
        ...
        if (!this.mob.canStandOnFluid(iblockdata.getFluidState())) {
            if (this.canFloat() && this.mob.isInWater()) {
                while (true) {
                    if (!iblockdata.is(Blocks.WATER) && iblockdata.getFluidState() != FluidTypes.WATER.getSource(false)) {
                        --i;
                        break;
                    }
                ...
                while (blockposition_mutableblockposition.getY() > this.currentContext.level().getMinY()) {
    ...
    public int getNeighbors(PathPoint[] apathpoint, PathPoint pathpoint) {
        ...
        for (EnumDirection enumdirection1 : EnumDirection.EnumDirectionLimit.HORIZONTAL) {
            ...
            if (this.isDiagonalValid(pathpoint, this.reusableNeighbors[enumdirection1.get2DDataValue()], this.reusableNeighbors[enumdirection2.get2DDataValue()])) {
                PathPoint pathpoint2 = this.findAcceptedNode(pathpoint.x + enumdirection1.getStepX() + enumdirection2.getStepX(), pathpoint.y, pathpoint.z + enumdirection1.getStepZ() + enumdirection2.getStepZ(), j, d0, enumdirection1, pathtype1);
    ...
    protected double getFloorLevel(BlockPosition blockposition) {
        IBlockAccess iblockaccess = this.currentContext.level();

        return (this.canFloat() || this.isAmphibious()) && iblockaccess.getFluidState(blockposition).is(TagsFluid.WATER) ? (double) blockposition.getY() + 0.5D : getFloorLevel(iblockaccess, blockposition);
    }
