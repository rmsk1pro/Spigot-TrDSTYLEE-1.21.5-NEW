...
public abstract class VoxelShape {
    ...
    public VoxelShape singleEncompassing() {
        return this.isEmpty() ? VoxelShapes.empty() : VoxelShapes.box(this.min(EnumDirection.EnumAxis.X), this.min(EnumDirection.EnumAxis.Y), this.min(EnumDirection.EnumAxis.Z), this.max(EnumDirection.EnumAxis.X), this.max(EnumDirection.EnumAxis.Y), this.max(EnumDirection.EnumAxis.Z));
    }
    ...
    public abstract DoubleList getCoords(EnumDirection.EnumAxis enumdirection_enumaxis);

    public boolean isEmpty() {
        return this.shape.isEmpty();
    }

    public VoxelShape move(Vec3D vec3d) {
        return this.move(vec3d.x, vec3d.y, vec3d.z);
    }

    public VoxelShape move(BaseBlockPosition baseblockposition) {
        return this.move((double) baseblockposition.getX(), (double) baseblockposition.getY(), (double) baseblockposition.getZ());
    }
    ...
    public VoxelShape getFaceShape(EnumDirection enumdirection) {
        if (!this.isEmpty() && this != VoxelShapes.block()) {
            ...
            VoxelShape voxelshape1 = this.calculateFace(enumdirection);

            this.faces[enumdirection.ordinal()] = voxelshape1;
            return voxelshape1;
        } else {
            return this;
        }
    ...
    private VoxelShape calculateFace(EnumDirection enumdirection) {
        ...
        if (this.isCubeLikeAlong(enumdirection_enumaxis)) {
            return this;
        } else {
            ...
            VoxelShapeSlice voxelshapeslice = new VoxelShapeSlice(this, enumdirection_enumaxis, i);

            return (VoxelShape) (voxelshapeslice.isEmpty() ? VoxelShapes.empty() : (voxelshapeslice.isCubeLike() ? VoxelShapes.block() : voxelshapeslice));
        }
    }
    ...
    private boolean isCubeLikeAlong(EnumDirection.EnumAxis enumdirection_enumaxis) {
                    ...
                    for (int i2 = k; i2 < l; ++i2) {
                        for (int j2 = i1; j2 < j1; ++j2) {
                            if (this.shape.isFullWide(enumaxiscycle1, l1, i2, j2)) {
                                double d3 = this.get(enumdirection_enumaxis, l1) - d1;
                ...
                for (int k2 = i - 1; k2 >= 0; --k2) {
                    for (int l2 = k; l2 < l; ++l2) {
                        for (int i3 = i1; i3 < j1; ++i3) {
                            if (this.shape.isFullWide(enumaxiscycle1, k2, l2, i3)) {
                                double d4 = this.get(enumdirection_enumaxis, k2 + 1) - d2;
