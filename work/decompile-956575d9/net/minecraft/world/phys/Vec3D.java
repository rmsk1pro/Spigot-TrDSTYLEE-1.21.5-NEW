...
public class Vec3D implements IPosition {
    ...
    public static final StreamCodec<ByteBuf, Vec3D> STREAM_CODEC = new StreamCodec<ByteBuf, Vec3D>() {
        public Vec3D decode(ByteBuf bytebuf) {
            return PacketDataSerializer.readVec3(bytebuf);
        }

        public void encode(ByteBuf bytebuf, Vec3D vec3d) {
            PacketDataSerializer.writeVec3(bytebuf, vec3d);
        }
    };
    ...
    public Vec3D normalize() {
        ...
        return d0 < (double) 1.0E-5F ? Vec3D.ZERO : new Vec3D(this.x / d0, this.y / d0, this.z / d0);
    }
    ...
    public Vec3D subtract(double d0) {
        return this.subtract(d0, d0, d0);
    }
    ...
    public Vec3D add(double d0) {
        return this.add(d0, d0, d0);
    }
    ...
    public boolean closerThan(Vec3D vec3d, double d0, double d1) {
        double d2 = vec3d.x() - this.x;
        double d3 = vec3d.y() - this.y;
        double d4 = vec3d.z() - this.z;

        return MathHelper.lengthSquared(d2, d4) < MathHelper.square(d0) && Math.abs(d3) < d1;
    }
    ...
    public Vec3D horizontal() {
        return new Vec3D(this.x, 0.0D, this.z);
    }
    ...
    public Vec3D projectedOn(Vec3D vec3d) {
        return vec3d.lengthSqr() == 0.0D ? vec3d : vec3d.scale(this.dot(vec3d)).scale(1.0D / vec3d.lengthSqr());
    }
}
