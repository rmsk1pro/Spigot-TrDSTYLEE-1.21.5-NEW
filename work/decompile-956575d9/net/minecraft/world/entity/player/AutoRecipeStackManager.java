...
public class AutoRecipeStackManager<T> {
    ...
    List<T> getUniqueAvailableIngredientItems(Iterable<? extends AutoRecipeStackManager.a<T>> iterable) {
        ...
        while (objectiterator.hasNext()) {
            ...
            if (reference2intmap_entry.getIntValue() > 0 && anyIngredientMatches(iterable, reference2intmap_entry.getKey())) {
    ...
    private static <T> boolean anyIngredientMatches(Iterable<? extends AutoRecipeStackManager.a<T>> iterable, T t0) {
        for (AutoRecipeStackManager.a<T> autorecipestackmanager_a : iterable) {
            if (autorecipestackmanager_a.acceptsItem(t0)) {
                return true;
            }
        }
    ...
    public int getResultUpperBound(List<? extends AutoRecipeStackManager.a<T>> list) {
        ...
        label31:
        for (AutoRecipeStackManager.a<T> autorecipestackmanager_a : list) {
            int j = 0;
            ...
            while (objectiterator.hasNext()) {
                ...
                if (k > j) {
                    if (autorecipestackmanager_a.acceptsItem(reference2intmap_entry.getKey())) {
                        j = k;
                    }
