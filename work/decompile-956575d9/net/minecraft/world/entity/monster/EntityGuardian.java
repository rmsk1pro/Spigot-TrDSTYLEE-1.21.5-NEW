...
public class EntityGuardian extends EntityMonster {
    ...
    private static final DataWatcherObject<Boolean> DATA_ID_MOVING = DataWatcher.<Boolean>defineId(EntityGuardian.class, DataWatcherRegistry.BOOLEAN);
    private static final DataWatcherObject<Integer> DATA_ID_ATTACK_TARGET = DataWatcher.<Integer>defineId(EntityGuardian.class, DataWatcherRegistry.INT);
    ...
    public static AttributeProvider.Builder createAttributes() {
        return EntityMonster.createMonsterAttributes().add(GenericAttributes.ATTACK_DAMAGE, 6.0D).add(GenericAttributes.MOVEMENT_SPEED, 0.5D).add(GenericAttributes.MAX_HEALTH, 30.0D);
    }
    ...
    protected void defineSynchedData(DataWatcher.a datawatcher_a) {
        ...
        datawatcher_a.define(EntityGuardian.DATA_ID_MOVING, false);
        datawatcher_a.define(EntityGuardian.DATA_ID_ATTACK_TARGET, 0);
    }
        ...
        if (this.isAlive()) {
            if (this.level().isClientSide) {
                ...
                if (this.isMoving() && this.isInWater()) {
                    Vec3D vec3d1 = this.getViewVector(0.0F);

                    for (int i = 0; i < 2; ++i) {
                        this.level().addParticle(Particles.BUBBLE, this.getRandomX(0.5D) - vec3d1.x * 1.5D, this.getRandomY() - vec3d1.y * 1.5D, this.getRandomZ(0.5D) - vec3d1.z * 1.5D, 0.0D, 0.0D, 0.0D);
                    }
                }
    ...
    public static boolean checkGuardianSpawnRules(EntityTypes<? extends EntityGuardian> entitytypes, GeneratorAccess generatoraccess, EntitySpawnReason entityspawnreason, BlockPosition blockposition, RandomSource randomsource) {
        return (randomsource.nextInt(20) == 0 || !generatoraccess.canSeeSkyFromBelowWater(blockposition)) && generatoraccess.getDifficulty() != EnumDifficulty.PEACEFUL && (EntitySpawnReason.isSpawner(entityspawnreason) || generatoraccess.getFluidState(blockposition).is(TagsFluid.WATER)) && generatoraccess.getFluidState(blockposition.below()).is(TagsFluid.WATER);
    }
    ...
    public boolean hurtServer(WorldServer worldserver, DamageSource damagesource, float f) {
        if (!this.isMoving() && !damagesource.is(DamageTypeTags.AVOIDS_GUARDIAN_THORNS) && !damagesource.is(DamageTypes.THORNS)) {
            ...
            if (entity instanceof EntityLiving) {
                ...
                entityliving.hurtServer(worldserver, this.damageSources().thorns(this), 2.0F);
            }
        }
            ...
            if (entityliving != null) {
                ...
                if (!this.guardian.hasLineOfSight(entityliving)) {
                    ...
                    if (this.attackTime == 0) {
                        ...
                        WorldServer worldserver = getServerLevel((Entity) this.guardian);
