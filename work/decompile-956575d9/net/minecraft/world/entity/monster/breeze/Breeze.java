...
public class Breeze extends EntityMonster {
    ...
    private static final ProjectileDeflection PROJECTILE_DEFLECTION = (iprojectile, entity, randomsource) -> {
        entity.level().playSound((Entity) null, entity, SoundEffects.BREEZE_DEFLECT, entity.getSoundSource(), 1.0F, 1.0F);
        ProjectileDeflection.REVERSE.deflect(iprojectile, entity, randomsource);
    };

    public static AttributeProvider.Builder createAttributes() {
        return EntityInsentient.createMobAttributes().add(GenericAttributes.MOVEMENT_SPEED, (double) 0.63F).add(GenericAttributes.MAX_HEALTH, 30.0D).add(GenericAttributes.FOLLOW_RANGE, 24.0D).add(GenericAttributes.ATTACK_DAMAGE, 3.0D);
    }
    ...
    protected BehaviorController.b<Breeze> brainProvider() {
        return BehaviorController.<Breeze>provider(BreezeAi.MEMORY_TYPES, BreezeAi.SENSOR_TYPES);
    }
    ...
    public void emitJumpTrailParticles() {
        if (++this.jumpTrailStartedTick <= 5) {
            ...
            Vec3D vec3d1 = this.position().add(vec3d).add(0.0D, (double) 0.1F, 0.0D);
    ...
    public boolean isInvulnerableTo(WorldServer worldserver, DamageSource damagesource) {
        return damagesource.getEntity() instanceof Breeze || super.isInvulnerableTo(worldserver, damagesource);
    }
