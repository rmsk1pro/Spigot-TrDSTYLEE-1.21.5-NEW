...
public class EntitySpider extends EntityMonster {
    ...
    private static final DataWatcherObject<Byte> DATA_FLAGS_ID = DataWatcher.<Byte>defineId(EntitySpider.class, DataWatcherRegistry.BYTE);
    ...
    public GroupDataEntity finalizeSpawn(WorldAccess worldaccess, DifficultyDamageScaler difficultydamagescaler, EntitySpawnReason entityspawnreason, @Nullable GroupDataEntity groupdataentity) {
        ...
        if (randomsource.nextInt(100) == 0) {
            EntitySkeleton entityskeleton = EntityTypes.SKELETON.create(this.level(), EntitySpawnReason.JOCKEY);

            if (entityskeleton != null) {
                entityskeleton.snapTo(this.getX(), this.getY(), this.getZ(), this.getYRot(), 0.0F);
                entityskeleton.finalizeSpawn(worldaccess, difficultydamagescaler, entityspawnreason, (GroupDataEntity) null);
        ...
        if (groupdataentity == null) {
            ...
            if (worldaccess.getDifficulty() == EnumDifficulty.HARD && randomsource.nextFloat() < 0.1F * difficultydamagescaler.getSpecialMultiplier()) {
                ((EntitySpider.GroupDataSpider) groupdataentity).setRandomEffect(randomsource);
            }
        }

        if (groupdataentity instanceof EntitySpider.GroupDataSpider entityspider_groupdataspider) {
