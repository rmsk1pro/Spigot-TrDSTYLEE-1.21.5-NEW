...
public class EntityTypes<T extends Entity> implements FeatureElement, EntityTypeTest<Entity, T> {
    ...
    public static Optional<Entity> create(NBTTagCompound nbttagcompound, World world, EntitySpawnReason entityspawnreason) {
        return SystemUtils.<Entity>ifElse(by(nbttagcompound).map((entitytypes) -> {
            return entitytypes.create(world, entityspawnreason);
        }), (entity) -> {
            entity.load(nbttagcompound);
        }, () -> {
            EntityTypes.LOGGER.warn("Skipping Entity with id {}", nbttagcompound.getStringOr("id", "[invalid]"));
        });
    }
    ...
    public static Entity loadEntityRecursive(NBTTagCompound nbttagcompound, World world, EntitySpawnReason entityspawnreason, Function<Entity, Entity> function) {
        return (Entity) loadStaticEntity(nbttagcompound, world, entityspawnreason).map(function).map((entity) -> {
            NBTTagList nbttaglist = nbttagcompound.getListOrEmpty("Passengers");

            for (int i = 0; i < nbttaglist.size(); ++i) {
                Entity entity1 = loadEntityRecursive(nbttaglist.getCompoundOrEmpty(i), world, entityspawnreason, function);
    ...
    public static Stream<Entity> loadEntitiesRecursive(List<? extends NBTBase> list, World world, EntitySpawnReason entityspawnreason) {
        return list.stream().flatMap((nbtbase) -> {
            return nbtbase.asCompound().stream();
        }).mapMulti((nbttagcompound, consumer) -> {
            loadEntityRecursive(nbttagcompound, world, entityspawnreason, (entity) -> {
    ...
    private static Optional<Entity> loadStaticEntity(NBTTagCompound nbttagcompound, World world, EntitySpawnReason entityspawnreason) {
        try {
            return create(nbttagcompound, world, entityspawnreason);
