...
public class GoatAi {
    ...
    private static final PathfinderTargetCondition RAM_TARGET_CONDITIONS = PathfinderTargetCondition.forCombat().selector((entityliving, worldserver) -> {
        return !entityliving.getType().equals(EntityTypes.GOAT) && (worldserver.getGameRules().getBoolean(GameRules.RULE_MOBGRIEFING) || !entityliving.getType().equals(EntityTypes.ARMOR_STAND)) && worldserver.getWorldBorder().isWithinBounds(entityliving.getBoundingBox());
    });
    ...
    private static void initRamActivity(BehaviorController<Goat> behaviorcontroller) {
        behaviorcontroller.addActivityWithConditions(Activity.RAM, ImmutableList.of(Pair.of(0, new RamTarget((goat) -> {
            ...
            return SoundEffects.GOAT_HORN_BREAK;
