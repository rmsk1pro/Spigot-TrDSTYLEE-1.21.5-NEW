...
public class Camel extends EntityHorseAbstract {
    ...
    public static final DataWatcherObject<Boolean> DASH = DataWatcher.<Boolean>defineId(Camel.class, DataWatcherRegistry.BOOLEAN);
    public static final DataWatcherObject<Long> LAST_POSE_CHANGE_TICK = DataWatcher.<Long>defineId(Camel.class, DataWatcherRegistry.LONG);
    ...
    public void readAdditionalSaveData(NBTTagCompound nbttagcompound) {
        ...
        long i = nbttagcompound.getLongOr("LastPoseTick", 0L);
    ...
    public static AttributeProvider.Builder createAttributes() {
        return createBaseHorseAttributes().add(GenericAttributes.MAX_HEALTH, 32.0D).add(GenericAttributes.MOVEMENT_SPEED, (double) 0.09F).add(GenericAttributes.JUMP_STRENGTH, (double) 0.42F).add(GenericAttributes.STEP_HEIGHT, 1.5D);
    }
    ...
    public static boolean checkCamelSpawnRules(EntityTypes<Camel> entitytypes, GeneratorAccess generatoraccess, EntitySpawnReason entityspawnreason, BlockPosition blockposition, RandomSource randomsource) {
        return generatoraccess.getBlockState(blockposition.below()).is(TagsBlock.CAMELS_SPAWNABLE_ON) && isBrightEnoughToSpawn(generatoraccess, blockposition);
    }
    ...
    public void tick() {
        ...
        if (this.dashCooldown > 0) {
            --this.dashCooldown;
            if (this.dashCooldown == 0) {
                this.level().playSound((Entity) null, this.blockPosition(), SoundEffects.CAMEL_DASH_READY, SoundCategory.NEUTRAL, 1.0F, 1.0F);
            }
        }
    ...
    public Camel getBreedOffspring(WorldServer worldserver, EntityAgeable entityageable) {
        return EntityTypes.CAMEL.create(worldserver, EntitySpawnReason.BREEDING);
    }
    ...
    protected Vec3D getPassengerAttachmentPoint(Entity entity, EntitySize entitysize, float f) {
        ...
        boolean flag = i == 0;
        float f1 = 0.5F;
        float f2 = (float) (this.isRemoved() ? (double) 0.01F : this.getBodyAnchorAnimationYOffset(flag, 0.0F, entitysize, f));
        ...
        return (new Vec3D(0.0D, (double) f2, (double) (f1 * f))).yRot(-this.getYRot() * ((float) Math.PI / 180F));
    }
    ...
    protected Holder<SoundEffect> getEquipSound(EnumItemSlot enumitemslot, ItemStack itemstack, Equippable equippable) {
        return (Holder<SoundEffect>) (enumitemslot == EnumItemSlot.SADDLE ? SoundEffects.CAMEL_SADDLE : super.getEquipSound(enumitemslot, itemstack, equippable));
    }
