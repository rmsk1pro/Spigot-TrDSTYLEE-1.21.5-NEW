...
public class PathfinderGoalHorseTrap extends PathfinderGoal {
    ...
    public void tick() {
        ...
        this.horse.setAge(0);
        EntityLightning entitylightning = EntityTypes.LIGHTNING_BOLT.create(worldserver, EntitySpawnReason.TRIGGERED);
    ...
    private EntityHorseAbstract createHorse(DifficultyDamageScaler difficultydamagescaler) {
        EntityHorseSkeleton entityhorseskeleton = EntityTypes.SKELETON_HORSE.create(this.horse.level(), EntitySpawnReason.TRIGGERED);

        if (entityhorseskeleton != null) {
            entityhorseskeleton.finalizeSpawn((WorldServer) this.horse.level(), difficultydamagescaler, EntitySpawnReason.TRIGGERED, (GroupDataEntity) null);
    ...
    private EntitySkeleton createSkeleton(DifficultyDamageScaler difficultydamagescaler, EntityHorseAbstract entityhorseabstract) {
        EntitySkeleton entityskeleton = EntityTypes.SKELETON.create(entityhorseabstract.level(), EntitySpawnReason.TRIGGERED);

        if (entityskeleton != null) {
            entityskeleton.finalizeSpawn((WorldServer) entityhorseabstract.level(), difficultydamagescaler, EntitySpawnReason.TRIGGERED, (GroupDataEntity) null);
