...
public class EntityPufferFish extends EntityFish {
    ...
    public void readAdditionalSaveData(NBTTagCompound nbttagcompound) {
        ...
        this.setPuffState(Math.min(nbttagcompound.getIntOr("PuffState", 0), 2));
    }
    ...
    public void playerTouch(EntityHuman entityhuman) {
        ...
        if (entityhuman instanceof EntityPlayer entityplayer) {
            if (i > 0 && entityhuman.hurtServer(entityplayer.serverLevel(), this.damageSources().mobAttack(this), (float) (1 + i))) {
                if (!this.isSilent()) {
