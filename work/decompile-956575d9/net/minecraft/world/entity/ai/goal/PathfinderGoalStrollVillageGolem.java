...
public class PathfinderGoalStrollVillageGolem extends PathfinderGoalRandomStroll {
    ...
    private Vec3D getPositionTowardsVillagerWhoWantsGolem() {
        ...
        List<EntityVillager> list = worldserver.getEntities(EntityTypes.VILLAGER, this.mob.getBoundingBox().inflate(32.0D), this::doesVillagerWantGolem);
    ...
    private SectionPosition getRandomVillageSection() {
        WorldServer worldserver = (WorldServer) this.mob.level();
        List<SectionPosition> list = (List) SectionPosition.cube(SectionPosition.of((EntityAccess) this.mob), 2).filter((sectionposition) -> {
    ...
    private BlockPosition getRandomPoiWithinSection(SectionPosition sectionposition) {
        ...
        List<BlockPosition> list = (List) villageplace.getInRange((holder) -> {
            return true;
