...
public class PathfinderGoalEatTile extends PathfinderGoal {
    ...
    private static final Predicate<IBlockData> IS_EDIBLE = (iblockdata) -> {
        return iblockdata.is(TagsBlock.EDIBLE_FOR_SHEEP);
    };
    ...
    public boolean canUse() {
        if (this.mob.getRandom().nextInt(this.mob.isBaby() ? 50 : 1000) != 0) {
            ...
            return PathfinderGoalEatTile.IS_EDIBLE.test(this.level.getBlockState(blockposition)) ? true : this.level.getBlockState(blockposition.below()).is(Blocks.GRASS_BLOCK);
        }
    }
