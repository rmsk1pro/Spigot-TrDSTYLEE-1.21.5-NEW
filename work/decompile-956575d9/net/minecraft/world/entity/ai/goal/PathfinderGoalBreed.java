...
public class PathfinderGoalBreed extends PathfinderGoal {
    ...
    public PathfinderGoalBreed(EntityAnimal entityanimal, double d0, Class<? extends EntityAnimal> oclass) {
        ...
        this.level = getServerLevel((Entity) entityanimal);
    ...
    private EntityAnimal getFreePartner() {
        List<? extends EntityAnimal> list = this.level.<EntityAnimal>getNearbyEntities(this.partnerClass, PathfinderGoalBreed.PARTNER_TARGETING, this.animal, this.animal.getBoundingBox().inflate(8.0D));
        double d0 = Double.MAX_VALUE;
        EntityAnimal entityanimal = null;

        for (EntityAnimal entityanimal1 : list) {
            if (this.animal.canMate(entityanimal1) && !entityanimal1.isPanicking() && this.animal.distanceToSqr((Entity) entityanimal1) < d0) {
