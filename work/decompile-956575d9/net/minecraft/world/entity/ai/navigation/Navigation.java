...
public class Navigation extends NavigationAbstract {
    ...
    public PathEntity createPath(BlockPosition blockposition, int i) {
        Chunk chunk = this.level.getChunkSource().getChunkNow(SectionPosition.blockToSectionCoord(blockposition.getX()), SectionPosition.blockToSectionCoord(blockposition.getZ()));

        if (chunk == null) {
            return null;
        } else {
            if (chunk.getBlockState(blockposition).isAir()) {
                ...
                blockposition_mutableblockposition.setY(blockposition.getY() + 1);
            ...
            if (!chunk.getBlockState(blockposition).isSolid()) {
                return super.createPath(blockposition, i);
            } else {
                BlockPosition.MutableBlockPosition blockposition_mutableblockposition1 = blockposition.mutable().move(EnumDirection.UP);

                while (blockposition_mutableblockposition1.getY() <= this.level.getMaxY() && chunk.getBlockState(blockposition_mutableblockposition1).isSolid()) {
                    blockposition_mutableblockposition1.move(EnumDirection.UP);
                }

                return super.createPath(blockposition_mutableblockposition1.immutable(), i);
            }
        }
    }
    ...
    private int getSurfaceY() {
        if (this.mob.isInWater() && this.canFloat()) {
            ...
            while (iblockdata.is(Blocks.WATER)) {
                ++i;
