...
public class BehaviorBetterJob {
    ...
    private static boolean competesForSameJobsite(GlobalPos globalpos, Holder<VillagePlaceType> holder, EntityVillager entityvillager) {
        ...
        return optional.isPresent() && globalpos.equals(optional.get()) && hasMatchingProfession(holder, entityvillager.getVillagerData().profession());
    }

    private static boolean hasMatchingProfession(Holder<VillagePlaceType> holder, Holder<VillagerProfession> holder1) {
        return ((VillagerProfession) holder1.value()).heldJobSite().test(holder);
    }
}
