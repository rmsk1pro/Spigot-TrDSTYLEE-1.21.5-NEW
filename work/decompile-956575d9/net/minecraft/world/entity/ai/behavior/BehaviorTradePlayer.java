...
public class BehaviorTradePlayer extends Behavior<EntityVillager> {
    ...
    public boolean checkExtraStartConditions(WorldServer worldserver, EntityVillager entityvillager) {
        ...
        if (behaviorcontroller.getMemory(MemoryModuleType.INTERACTION_TARGET).isEmpty()) {
            return false;
        } else {
    ...
    private void updateDisplayItems(EntityVillager entityvillager) {
        for (MerchantRecipe merchantrecipe : entityvillager.getOffers()) {
            if (!merchantrecipe.isOutOfStock() && this.playerItemStackMatchesCostOfOffer(merchantrecipe)) {
                this.displayItems.add(merchantrecipe.assemble());
            }
        }
    ...
    private boolean playerItemStackMatchesCostOfOffer(MerchantRecipe merchantrecipe) {
        return ItemStack.isSameItem(this.playerItemStack, merchantrecipe.getCostA()) || ItemStack.isSameItem(this.playerItemStack, merchantrecipe.getCostB());
    }
    ...
    private EntityLiving lookAtTarget(EntityVillager entityvillager) {
        ...
        behaviorcontroller.setMemory(MemoryModuleType.LOOK_TARGET, new BehaviorPositionEntity(entityliving, true));
        return entityliving;
    }
