...
public class BehavorMove extends Behavior<EntityInsentient> {
    ...
    protected boolean canStillUse(WorldServer worldserver, EntityInsentient entityinsentient, long i) {
        if (this.path != null && this.lastTargetPos != null) {
            Optional<MemoryTarget> optional = entityinsentient.getBrain().<MemoryTarget>getMemory(MemoryModuleType.WALK_TARGET);
            boolean flag = (Boolean) optional.map(BehavorMove::isWalkTargetSpectator).orElse(false);
            NavigationAbstract navigationabstract = entityinsentient.getNavigation();

            return !navigationabstract.isDone() && optional.isPresent() && !this.reachedTarget(entityinsentient, (MemoryTarget) optional.get()) && !flag;
        } else {
            return false;
        }
    }
    ...
    protected void start(WorldServer worldserver, EntityInsentient entityinsentient, long i) {
        entityinsentient.getBrain().setMemory(MemoryModuleType.PATH, this.path);
    ...
    protected void tick(WorldServer worldserver, EntityInsentient entityinsentient, long i) {
        ...
        if (this.path != pathentity) {
            ...
            behaviorcontroller.setMemory(MemoryModuleType.PATH, pathentity);
        }
    ...
    private boolean tryComputePath(EntityInsentient entityinsentient, MemoryTarget memorytarget, long i) {
        ...
        if (this.reachedTarget(entityinsentient, memorytarget)) {
            ...
            if (flag) {
                ...
                behaviorcontroller.setMemory(MemoryModuleType.CANT_REACH_WALK_TARGET_SINCE, i);
            }
            ...
            Vec3D vec3d = DefaultRandomPos.getPosTowards((EntityCreature) entityinsentient, 10, 7, Vec3D.atBottomCenterOf(blockposition), (double) ((float) Math.PI / 2F));
    ...
    private static boolean isWalkTargetSpectator(MemoryTarget memorytarget) {
        BehaviorPosition behaviorposition = memorytarget.getTarget();

        if (behaviorposition instanceof BehaviorPositionEntity behaviorpositionentity) {
            return behaviorpositionentity.getEntity().isSpectator();
        } else {
            return false;
        }
    }
