...
public class AnimalPanic<E extends EntityCreature> extends Behavior<E> {
    ...
    protected void start(WorldServer worldserver, E e0, long i) {
        e0.getBrain().setMemory(MemoryModuleType.IS_PANICKING, true);
        ((EntityCreature) e0).getBrain().eraseMemory(MemoryModuleType.WALK_TARGET);
    }
    ...
    protected void tick(WorldServer worldserver, E e0, long i) {
        if (e0.getNavigation().isDone()) {
            ...
            if (vec3d != null) {
                e0.getBrain().setMemory(MemoryModuleType.WALK_TARGET, new MemoryTarget(vec3d, this.speedMultiplier, 0));
            }
        }
