...
public class BehaviorStrollRandomUnconstrained {
    ...
    public static OneShot<EntityCreature> stroll(float f, boolean flag) {
        return strollFlyOrSwim(f, (entitycreature) -> {
            ...
            return !entitycreature.isInWater();
        });
    }
    ...
    public static BehaviorControl<EntityCreature> swim(float f) {
        return strollFlyOrSwim(f, BehaviorStrollRandomUnconstrained::getTargetSwimPos, Entity::isInWater);
    }
    ...
    private static Vec3D getTargetSwimPos(EntityCreature entitycreature) {
        ...
        for (int[] aint : BehaviorStrollRandomUnconstrained.SWIM_XY_DISTANCE_TIERS) {
            if (vec3d == null) {
                vec3d1 = BehaviorUtil.getRandomSwimmablePos(entitycreature, aint[0], aint[1]);
            } else {
                vec3d1 = entitycreature.position().add(entitycreature.position().vectorTo(vec3d).normalize().multiply((double) aint[0], (double) aint[1], (double) aint[0]));
            }

            if (vec3d1 == null || entitycreature.level().getFluidState(BlockPosition.containing(vec3d1)).isEmpty()) {
                return vec3d;
            }
    ...
    private static Vec3D getTargetFlyPos(EntityCreature entitycreature, int i, int j) {
        ...
        return AirAndWaterRandomPos.getPos(entitycreature, i, j, -2, vec3d.x, vec3d.z, (double) ((float) Math.PI / 2F));
    }
}
