...
public class BehaviorPositionValidate {
                    ...
                    if (worldserver.dimension() == globalpos.dimension() && blockposition.closerToCenterThan(entityliving.position(), 16.0D)) {
                        ...
                        if (worldserver1 != null && worldserver1.getPoiManager().exists(blockposition, predicate)) {
                            if (bedIsOccupied(worldserver1, blockposition, entityliving)) {
                                ...
                                if (!bedIsOccupiedByVillager(worldserver1, blockposition)) {
    ...
    private static boolean bedIsOccupiedByVillager(WorldServer worldserver, BlockPosition blockposition) {
        List<EntityVillager> list = worldserver.<EntityVillager>getEntitiesOfClass(EntityVillager.class, new AxisAlignedBB(blockposition), EntityLiving::isSleeping);
