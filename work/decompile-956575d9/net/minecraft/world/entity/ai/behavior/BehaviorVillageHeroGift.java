...
public class BehaviorVillageHeroGift extends Behavior<EntityVillager> {
    ...
    protected void start(WorldServer worldserver, EntityVillager entityvillager, long i) {
        ...
        entityvillager.getBrain().setMemory(MemoryModuleType.INTERACTION_TARGET, entityhuman);
    ...
    private static ResourceKey<LootTable> getLootTableToThrow(EntityVillager entityvillager) {
        if (entityvillager.isBaby()) {
            ...
            Optional<ResourceKey<VillagerProfession>> optional = entityvillager.getVillagerData().profession().unwrapKey();

            return optional.isEmpty() ? LootTables.UNEMPLOYED_GIFT : (ResourceKey) BehaviorVillageHeroGift.GIFTS.getOrDefault(optional.get(), LootTables.UNEMPLOYED_GIFT);
        }
    }
