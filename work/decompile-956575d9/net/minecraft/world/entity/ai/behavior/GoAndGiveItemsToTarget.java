...
public class GoAndGiveItemsToTarget<E extends EntityLiving & InventoryCarrier> extends Behavior<E> {
        ...
        if (!optional.isEmpty()) {
            ...
            if (d0 < 3.0D) {
                ...
                if (!itemstack.isEmpty()) {
                    ...
                    e0.getBrain().setMemory(MemoryModuleType.ITEM_PICKUP_COOLDOWN_TICKS, 60);
                }
            }
    ...
    public static void throwItem(EntityLiving entityliving, ItemStack itemstack, Vec3D vec3d) {
        Vec3D vec3d1 = new Vec3D((double) 0.2F, (double) 0.3F, (double) 0.2F);
        ...
        if (world.getGameTime() % 7L == 0L && world.random.nextDouble() < 0.9D) {
            float f = (Float) SystemUtils.getRandom(Allay.THROW_SOUND_PITCHES, world.getRandom());

            world.playSound((Entity) null, (Entity) entityliving, SoundEffects.ALLAY_THROW, SoundCategory.NEUTRAL, 1.0F, f);
        }
