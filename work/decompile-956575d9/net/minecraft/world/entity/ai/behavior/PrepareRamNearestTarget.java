...
public class PrepareRamNearestTarget<E extends EntityCreature> extends Behavior<E> {
    ...
    protected void stop(WorldServer worldserver, E e0, long i) {
        BehaviorController<?> behaviorcontroller = ((EntityCreature) e0).getBrain();
    ...
    protected void tick(WorldServer worldserver, E e0, long i) {
        if (!this.ramCandidate.isEmpty()) {
            e0.getBrain().setMemory(MemoryModuleType.WALK_TARGET, new MemoryTarget(((PrepareRamNearestTarget.a) this.ramCandidate.get()).getStartPosition(), this.walkSpeed, 0));
            e0.getBrain().setMemory(MemoryModuleType.LOOK_TARGET, new BehaviorPositionEntity(((PrepareRamNearestTarget.a) this.ramCandidate.get()).getTarget(), true));
            ...
            if (flag) {
                ...
                if (blockposition.equals(((PrepareRamNearestTarget.a) this.ramCandidate.get()).getStartPosition())) {
                    ...
                    if (i - (Long) this.reachedRamPositionTimestamp.get() >= (long) this.ramPrepareTime) {
                        e0.getBrain().setMemory(MemoryModuleType.RAM_TARGET, this.getEdgeOfBlock(blockposition, ((PrepareRamNearestTarget.a) this.ramCandidate.get()).getTargetPosition()));
                        worldserver.playSound((Entity) null, (Entity) e0, (SoundEffect) this.getPrepareRamSound.apply(e0), SoundCategory.NEUTRAL, 1.0F, e0.getVoicePitch());
    ...
    private boolean isWalkableBlock(EntityCreature entitycreature, BlockPosition blockposition) {
        return entitycreature.getNavigation().isStableDestination(blockposition) && entitycreature.getPathfindingMalus(PathfinderNormal.getPathTypeStatic((EntityInsentient) entitycreature, blockposition)) == 0.0F;
    }
