...
public class LongJumpToRandomPos<E extends EntityInsentient> extends Behavior<E> {
    ...
    public static <E extends EntityInsentient> boolean defaultAcceptableLandingSpot(E e0, BlockPosition blockposition) {
        ...
        return world.getBlockState(blockposition1).isSolidRender() && e0.getPathfindingMalus(PathfinderNormal.getPathTypeStatic(e0, blockposition)) == 0.0F;
    }
    ...
    protected boolean checkExtraStartConditions(WorldServer worldserver, EntityInsentient entityinsentient) {
        ...
        if (!flag) {
            entityinsentient.getBrain().setMemory(MemoryModuleType.LONG_JUMP_COOLDOWN_TICKS, this.timeBetweenLongJumps.sample(worldserver.random) / 2);
        }
    ...
    protected boolean canStillUse(WorldServer worldserver, EntityInsentient entityinsentient, long i) {
        boolean flag = this.initialPosition.isPresent() && ((Vec3D) this.initialPosition.get()).equals(entityinsentient.position()) && this.findJumpTries > 0 && !entityinsentient.isInWater() && (this.chosenJump != null || !this.jumpCandidates.isEmpty());
    ...
    protected void tick(WorldServer worldserver, E e0, long i) {
        if (this.chosenJump != null) {
            if (i - this.prepareJumpStart >= 40L) {
                ...
                worldserver.playSound((Entity) null, (Entity) e0, (SoundEffect) this.getJumpSound.apply(e0), SoundCategory.NEUTRAL, 1.0F, 1.0F);
            }
        } else {
            --this.findJumpTries;
    ...
    protected void pickCandidate(WorldServer worldserver, E e0, long i) {
        while (true) {
            if (!this.jumpCandidates.isEmpty()) {
                ...
                BlockPosition blockposition = longjumptorandompos_a.targetPos();
                ...
                e0.getBrain().setMemory(MemoryModuleType.LOOK_TARGET, new BehaviorTarget(blockposition));
    ...
    protected Optional<LongJumpToRandomPos.a> getJumpCandidate(WorldServer worldserver) {
        Optional<LongJumpToRandomPos.a> optional = WeightedRandom2.<LongJumpToRandomPos.a>getRandomItem(worldserver.random, this.jumpCandidates, LongJumpToRandomPos.a::weight);
    ...
    protected Vec3D calculateOptimalJumpVector(EntityInsentient entityinsentient, Vec3D vec3d) {
        ...
        for (int i : list) {
            Optional<Vec3D> optional = LongJumpUtil.calculateJumpVectorForAngle(entityinsentient, vec3d, f, i, true);
    ...
    public static record a(BlockPosition targetPos, int weight) {
