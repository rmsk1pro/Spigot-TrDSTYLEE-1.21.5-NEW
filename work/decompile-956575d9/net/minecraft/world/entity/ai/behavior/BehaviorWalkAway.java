        ...
        return BehaviorBuilder.create((behaviorbuilder_b) -> {
            return behaviorbuilder_b.group(behaviorbuilder_b.registered(MemoryModuleType.WALK_TARGET), behaviorbuilder_b.present(memorymoduletype)).apply(behaviorbuilder_b, (memoryaccessor, memoryaccessor1) -> {
                return (worldserver, entitycreature, j) -> {
                    Optional<MemoryTarget> optional = behaviorbuilder_b.<MemoryTarget>tryGet(memoryaccessor);
                    ...
                    if (optional.isPresent() && !flag) {
                        ...
                        if (!vec3d.closerThan(vec3d1, (double) i)) {
                            ...
                            if (optional.isPresent() && ((MemoryTarget) optional.get()).getSpeedModifier() == f) {
                                Vec3D vec3d2 = ((MemoryTarget) optional.get()).getTarget().currentPosition().subtract(vec3d);
                                Vec3D vec3d3 = vec3d1.subtract(vec3d);

                                if (vec3d2.dot(vec3d3) < 0.0D) {
                                    return false;
                                }
                            }

                            for (int k = 0; k < 10; ++k) {
                                Vec3D vec3d4 = LandRandomPos.getPosAway(entitycreature, 16, 7, vec3d1);

                                if (vec3d4 != null) {
                                    memoryaccessor.set(new MemoryTarget(vec3d4, f, 0));
                                    break;
                                }
                            }
