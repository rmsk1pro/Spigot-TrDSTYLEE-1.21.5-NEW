...
public class RandomPositionGenerator {
    ...
    public static BlockPosition generateRandomDirectionWithinRadians(RandomSource randomsource, int i, int j, int k, double d0, double d1, double d2) {
        double d3 = MathHelper.atan2(d1, d0) - (double) ((float) Math.PI / 2F);
    ...
    public static BlockPosition moveUpOutOfSolid(BlockPosition blockposition, int i, Predicate<BlockPosition> predicate) {
        if (!predicate.test(blockposition)) {
            ...
            while (blockposition_mutableblockposition.getY() <= i && predicate.test(blockposition_mutableblockposition)) {
    ...
    public static BlockPosition moveUpToAboveSolid(BlockPosition blockposition, int i, int j, Predicate<BlockPosition> predicate) {
        if (i < 0) {
            ...
            while (blockposition_mutableblockposition.getY() <= j && predicate.test(blockposition_mutableblockposition)) {
                blockposition_mutableblockposition.move(EnumDirection.UP);
            }

            int k = blockposition_mutableblockposition.getY();

            while (blockposition_mutableblockposition.getY() <= j && blockposition_mutableblockposition.getY() - k < i) {
                blockposition_mutableblockposition.move(EnumDirection.UP);
                if (predicate.test(blockposition_mutableblockposition)) {
