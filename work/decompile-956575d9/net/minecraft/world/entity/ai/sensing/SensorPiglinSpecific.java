...
public class SensorPiglinSpecific extends Sensor<EntityLiving> {
    ...
    protected void doTick(WorldServer worldserver, EntityLiving entityliving) {
        ...
        for (EntityLiving entityliving1 : nearestvisiblelivingentities.findAll((entityliving2) -> {
            return true;
        })) {
            if (entityliving1 instanceof EntityHoglin entityhoglin) {
            ...
            } else if (entityliving1 instanceof EntityPiglinBrute entitypiglinbrute) {
                list.add(entitypiglinbrute);
            } else if (entityliving1 instanceof EntityPiglin entitypiglin) {
            ...
            } else if (entityliving1 instanceof EntityHuman entityhuman) {
                if (optional5.isEmpty() && !PiglinAI.isWearingSafeArmor(entityhuman) && entityliving.canAttack(entityliving1)) {
                ...
                if (optional4.isEmpty() && PiglinAI.isZombified(entityliving1.getType())) {
        ...
        for (EntityLiving entityliving2 : (List) behaviorcontroller.getMemory(MemoryModuleType.NEAREST_LIVING_ENTITIES).orElse(ImmutableList.of())) {
            if (entityliving2 instanceof EntityPiglinAbstract entitypiglinabstract) {
        ...
        behaviorcontroller.setMemory(MemoryModuleType.NEARBY_ADULT_PIGLINS, list1);
        behaviorcontroller.setMemory(MemoryModuleType.NEAREST_VISIBLE_ADULT_PIGLINS, list);
        behaviorcontroller.setMemory(MemoryModuleType.VISIBLE_ADULT_PIGLIN_COUNT, list.size());
        behaviorcontroller.setMemory(MemoryModuleType.VISIBLE_ADULT_HOGLIN_COUNT, i);
    }
