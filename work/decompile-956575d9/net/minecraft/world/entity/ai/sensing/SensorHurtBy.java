...
public class SensorHurtBy extends Sensor<EntityLiving> {
    ...
    protected void doTick(WorldServer worldserver, EntityLiving entityliving) {
        ...
        if (damagesource != null) {
            behaviorcontroller.setMemory(MemoryModuleType.HURT_BY, entityliving.getLastDamageSource());
            Entity entity = damagesource.getEntity();

            if (entity instanceof EntityLiving) {
                behaviorcontroller.setMemory(MemoryModuleType.HURT_BY_ENTITY, (EntityLiving) entity);
            }
        } else {
