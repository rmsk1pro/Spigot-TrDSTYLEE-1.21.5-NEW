...
public class MobSensor<T extends EntityLiving> extends Sensor<T> {
    ...
    public void checkForMobsNearby(T t0) {
        Optional<List<EntityLiving>> optional = ((EntityLiving) t0).getBrain().<List<EntityLiving>>getMemory(MemoryModuleType.NEAREST_LIVING_ENTITIES);
    ...
    public void mobDetected(T t0) {
        ((EntityLiving) t0).getBrain().setMemoryWithExpiry(this.toSet, true, (long) this.memoryTimeToLive);
    }

    public void clearMemory(T t0) {
        ((EntityLiving) t0).getBrain().eraseMemory(this.toSet);
    }
}
