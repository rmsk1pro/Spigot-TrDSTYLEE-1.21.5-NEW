...
public abstract class AbstractWindCharge extends EntityFireball implements ItemSupplier {
    ...
    protected AxisAlignedBB makeBoundingBox(Vec3D vec3d) {
        ...
        return new AxisAlignedBB(vec3d.x - (double) f, vec3d.y - (double) 0.15F, vec3d.z - (double) f, vec3d.x + (double) f, vec3d.y - (double) 0.15F + (double) f1, vec3d.z + (double) f);
    }
    ...
    protected void onHitEntity(MovingObjectPositionEntity movingobjectpositionentity) {
        ...
        if (world instanceof WorldServer worldserver) {
            ...
            if (entity1.hurtServer(worldserver, damagesource, 1.0F) && entity1 instanceof EntityLiving entityliving3) {
                EnchantmentManager.doPostAttackEffects(worldserver, entityliving3, damagesource);
            }
    ...
    protected void onHitBlock(MovingObjectPositionBlock movingobjectpositionblock) {
        ...
        if (!this.level().isClientSide) {
            BaseBlockPosition baseblockposition = movingobjectpositionblock.getDirection().getUnitVec3i();
