...
public class EntityMinecartHopper extends EntityMinecartContainer implements IHopper {
    ...
    public boolean suckInItems() {
        if (TileEntityHopper.suckInItems(this.level(), this)) {
            ...
            for (EntityItem entityitem : this.level().getEntitiesOfClass(EntityItem.class, this.getBoundingBox().inflate(0.25D, 0.0D, 0.25D), IEntitySelector.ENTITY_STILL_ALIVE)) {
                if (TileEntityHopper.addItem(this, entityitem)) {
                    return true;
                }
            }
    ...
    protected void readAdditionalSaveData(NBTTagCompound nbttagcompound) {
        ...
        this.enabled = nbttagcompound.getBooleanOr("Enabled", true);
    }
