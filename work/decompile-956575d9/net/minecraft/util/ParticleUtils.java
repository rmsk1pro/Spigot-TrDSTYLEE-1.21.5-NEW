...
public class ParticleUtils {
    ...
    public static void spawnSmashAttackParticles(GeneratorAccess generatoraccess, BlockPosition blockposition, int i) {
        ...
        for (int j = 0; (float) j < (float) i / 3.0F; ++j) {
            double d0 = vec3d.x + generatoraccess.getRandom().nextGaussian() / 2.0D;
            double d1 = vec3d.y;
            double d2 = vec3d.z + generatoraccess.getRandom().nextGaussian() / 2.0D;
            double d3 = generatoraccess.getRandom().nextGaussian() * (double) 0.2F;
            double d4 = generatoraccess.getRandom().nextGaussian() * (double) 0.2F;
            double d5 = generatoraccess.getRandom().nextGaussian() * (double) 0.2F;

            generatoraccess.addParticle(particleparamblock, d0, d1, d2, d3, d4, d5);
        }

        for (int k = 0; (float) k < (float) i / 1.5F; ++k) {
            double d6 = vec3d.x + 3.5D * Math.cos((double) k) + generatoraccess.getRandom().nextGaussian() / 2.0D;
            double d7 = vec3d.y;
            double d8 = vec3d.z + 3.5D * Math.sin((double) k) + generatoraccess.getRandom().nextGaussian() / 2.0D;
            double d9 = generatoraccess.getRandom().nextGaussian() * (double) 0.05F;
            double d10 = generatoraccess.getRandom().nextGaussian() * (double) 0.05F;
            double d11 = generatoraccess.getRandom().nextGaussian() * (double) 0.05F;

            generatoraccess.addParticle(particleparamblock, d6, d7, d8, d9, d10, d11);
        }
