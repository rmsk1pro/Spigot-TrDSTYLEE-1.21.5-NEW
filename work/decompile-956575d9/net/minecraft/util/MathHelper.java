...
public class MathHelper {
    ...
    public static float wrapDegrees(long i) {
        float f = (float) (i % 360L);
    ...
    public static double atan2(double d0, double d1) {
        ...
        if (Double.isNaN(d2)) {
            ...
            d1 *= d4;
            d0 *= d4;
            double d5 = MathHelper.FRAC_BIAS + d0;
            int i = (int) Double.doubleToRawLongBits(d5);
            double d6 = MathHelper.ASIN_TAB[i];
            double d7 = MathHelper.COS_TAB[i];
            double d8 = d5 - MathHelper.FRAC_BIAS;
    ...
    public static int hsvToArgb(float f, float f1, float f2, int i) {
        ...
        return ARGB.color(i, clamp((int) (f7 * 255.0F), 0, 255), clamp((int) (f8 * 255.0F), 0, 255), clamp((int) (f9 * 255.0F), 0, 255));
    }
    ...
    public static Vec3D lerp(double d0, Vec3D vec3d, Vec3D vec3d1) {
        return new Vec3D(lerp(d0, vec3d.x, vec3d1.x), lerp(d0, vec3d.y, vec3d1.y), lerp(d0, vec3d.z, vec3d1.z));
    }
    ...
    public static float rotLerpRad(float f, float f1, float f2) {
        ...
        for (f3 = f2 - f1; f3 < -(float) Math.PI; f3 += ((float) Math.PI * 2F)) {
            ;
        }
    ...
    public static float length(float f, float f1) {
        return (float) Math.sqrt(lengthSquared((double) f, (double) f1));
    }
    ...
    public static float lengthSquared(float f, float f1, float f2) {
