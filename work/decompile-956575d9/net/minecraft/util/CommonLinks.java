...
public class CommonLinks {
    ...
    public static String extendRealms(@Nullable String s, UUID uuid, boolean flag) {
        if (s == null) {
            return "https://aka.ms/ExtendJavaRealms";
        } else {
            String s1 = extendRealms(s, uuid);

            return s1 + "&ref=" + (flag ? "expiredTrial" : "expiredRealm");
        }
    }

    public static String extendRealms(@Nullable String s, UUID uuid) {
        return s == null ? "https://aka.ms/ExtendJavaRealms" : "https://aka.ms/ExtendJavaRealms?subscriptionId=" + s + "&profileId=" + UndashedUuid.toString(uuid);
    }
}
