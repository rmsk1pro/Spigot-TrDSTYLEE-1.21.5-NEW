...
public class DataConverterEntityUUID extends DataConverterUUIDBase {
    ...
    protected TypeRewriteRule makeRule() {
        return this.fixTypeEverywhereTyped("EntityUUIDFixes", this.getInputSchema().getType(this.typeReference), (typed) -> {
            ...
            for (String s : DataConverterEntityUUID.ABSTRACT_HORSES) {
                typed = this.updateNamedChoice(typed, s, DataConverterEntityUUID::updateAnimalOwner);
            }

            for (String s1 : DataConverterEntityUUID.TAMEABLE_ANIMALS) {
                typed = this.updateNamedChoice(typed, s1, DataConverterEntityUUID::updateAnimalOwner);
            }

            for (String s2 : DataConverterEntityUUID.ANIMALS) {
                typed = this.updateNamedChoice(typed, s2, DataConverterEntityUUID::updateAnimal);
            }

            for (String s3 : DataConverterEntityUUID.MOBS) {
                typed = this.updateNamedChoice(typed, s3, DataConverterEntityUUID::updateMob);
            }

            for (String s4 : DataConverterEntityUUID.LIVING_ENTITIES) {
                typed = this.updateNamedChoice(typed, s4, DataConverterEntityUUID::updateLivingEntity);
            }

            for (String s5 : DataConverterEntityUUID.PROJECTILES) {
                typed = this.updateNamedChoice(typed, s5, DataConverterEntityUUID::updateProjectile);
            }
