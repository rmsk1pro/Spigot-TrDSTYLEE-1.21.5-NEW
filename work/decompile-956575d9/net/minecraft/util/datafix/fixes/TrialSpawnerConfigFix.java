...
public class TrialSpawnerConfigFix extends NamedEntityWriteReadFix {
    ...
    public TrialSpawnerConfigFix(Schema schema) {
        super(schema, true, "Trial Spawner config tag fixer", DataConverterTypes.BLOCK_ENTITY, "minecraft:trial_spawner");
    }
    ...
    private static <T> Dynamic<T> moveToConfigTag(Dynamic<T> dynamic) {
        ...
        for (String s : list) {
            Optional<Dynamic<T>> optional = dynamic.get(s).get().result();

            if (optional.isPresent()) {
                map.put(dynamic.createString(s), (Dynamic) optional.get());
