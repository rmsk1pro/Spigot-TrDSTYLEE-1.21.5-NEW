            ...
            if (s1 != null) {
                ...
                if (i < 0) {
                    ...
                    if ("stat.mineBlock".equals(s2)) {
                        String s3 = upgradeBlock(s.substring(i + 1).replace('.', ':'));

                        return new DataConverterStatistic.a("minecraft:mined", s3);
                    } else {
                        String s4 = (String) DataConverterStatistic.ITEM_KEYS.get(s2);

                        if (s4 != null) {
                            String s5 = s.substring(i + 1).replace('.', ':');
                            String s6 = upgradeItem(s5);
                            String s7 = s6 == null ? s5 : s6;

                            return new DataConverterStatistic.a(s4, s7);
                        } else {
                            String s8 = (String) DataConverterStatistic.ENTITY_KEYS.get(s2);

                            if (s8 != null) {
                                String s9 = s.substring(i + 1).replace('.', ':');
                                String s10 = (String) DataConverterStatistic.ENTITIES.getOrDefault(s9, s9);

                                return new DataConverterStatistic.a(s8, s10);
                            } else {
                                return null;
                            }
                        }
                ...
                for (Map.Entry<? extends Dynamic<?>, ? extends Dynamic<?>> map_entry : ((Map) optional.get()).entrySet()) {
                    if (((Dynamic) map_entry.getValue()).asNumber().result().isPresent()) {
                        ...
                        if (dataconverterstatistic_a != null) {
                            ...
                            map.put(dynamic1, dynamic2.set(dataconverterstatistic_a.typeKey(), (Dynamic) map_entry.getValue()));
                        }
                    }
                }
            }
