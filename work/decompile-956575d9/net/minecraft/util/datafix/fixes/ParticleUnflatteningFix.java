...
public class ParticleUnflatteningFix extends DataFix {
    ...
    private <T> Dynamic<T> updateItem(Dynamic<T> dynamic, String s) {
        ...
        if (i == -1) {
            ...
            Dynamic<T> dynamic2 = parseTag(dynamic.getOps(), s.substring(i));
    ...
    @Nullable
    private static <T> Dynamic<T> parseTag(DynamicOps<T> dynamicops, String s) {
        try {
            return new Dynamic(dynamicops, MojangsonParser.create(dynamicops).parseFully(s));
