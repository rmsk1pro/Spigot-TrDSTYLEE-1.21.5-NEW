...
public class DataConverterSchemaV2501 extends DataConverterSchemaNamed {
    ...
    private static void registerFurnace(Schema schema, Map<String, Supplier<TypeTemplate>> map, String s) {
        schema.register(map, s, () -> {
            return DSL.optionalFields("Items", DSL.list(DataConverterTypes.ITEM_STACK.in(schema)), "CustomName", DataConverterTypes.TEXT_COMPONENT.in(schema), "RecipesUsed", DSL.compoundList(DataConverterTypes.RECIPE.in(schema), DSL.constType(DSL.intType())));
        });
    }
