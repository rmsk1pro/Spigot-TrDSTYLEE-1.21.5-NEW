...
public interface INamable {
    ...
    static <E extends Enum<E> & INamable> INamable.a<E> fromEnumWithMapping(Supplier<E[]> supplier, Function<String, String> function) {
        ...
        return new INamable.a<E>(ae, function1);
    }
    ...
    static <T extends INamable> Codec<T> fromValues(Supplier<T[]> supplier) {
        ...
        ToIntFunction<T> tointfunction = SystemUtils.<T>createIndexLookup(Arrays.asList(at));

        return new INamable.b<T>(at, function, tointfunction);
    }
    ...
    public static class a<E extends Enum<E> & INamable> extends INamable.b<E> {
        ...
        public E byName(@Nullable String s) {
            return (E) (this.resolver.apply(s));
        }

        public E byName(@Nullable String s, E e0) {
            return (E) (Objects.requireNonNullElse(this.byName(s), e0));
        }

        public E byName(@Nullable String s, Supplier<? extends E> supplier) {
            return (E) (Objects.requireNonNullElseGet(this.byName(s), supplier));
        }
    }
}
