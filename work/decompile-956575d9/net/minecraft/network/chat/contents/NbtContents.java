...
public class NbtContents implements ComponentContents {
    ...
    public static final MapCodec<NbtContents> CODEC = RecordCodecBuilder.mapCodec((instance) -> {
        return instance.group(Codec.STRING.fieldOf("nbt").forGetter(NbtContents::getNbtPath), Codec.BOOL.lenientOptionalFieldOf("interpret", false).forGetter(NbtContents::isInterpreting), ComponentSerialization.CODEC.lenientOptionalFieldOf("separator").forGetter(NbtContents::getSeparator), DataSource.CODEC.forGetter(NbtContents::getDataSource)).apply(instance, NbtContents::new);
    });
    public static final ComponentContents.a<NbtContents> TYPE = new ComponentContents.a<NbtContents>(NbtContents.CODEC, "nbt");
    ...
    public IChatMutableComponent resolve(@Nullable CommandListenerWrapper commandlistenerwrapper, @Nullable Entity entity, int i) throws CommandSyntaxException {
        if (commandlistenerwrapper != null && this.compiledNbtPath != null) {
            Stream<NBTBase> stream = this.dataSource.getData(commandlistenerwrapper).flatMap((nbttagcompound) -> {
                try {
            ...
            if (this.interpreting) {
                RegistryOps<NBTBase> registryops = commandlistenerwrapper.registryAccess().<NBTBase>createSerializationContext(DynamicOpsNBT.INSTANCE);
                ...
                return (IChatMutableComponent) stream.flatMap((nbtbase) -> {
                    try {
                        IChatBaseComponent ichatbasecomponent1 = (IChatBaseComponent) ComponentSerialization.CODEC.parse(registryops, nbtbase).getOrThrow();

                        return Stream.of(ChatComponentUtils.updateForEntity(commandlistenerwrapper, ichatbasecomponent1, entity, i));
                    } catch (Exception exception) {
                        NbtContents.LOGGER.warn("Failed to parse component: {}", nbtbase, exception);
                        return Stream.of();
                    }
                ...
                return (IChatMutableComponent) ChatComponentUtils.updateForEntity(commandlistenerwrapper, this.separator, entity, i).map((ichatmutablecomponent) -> {
                    return (IChatMutableComponent) stream1.map(IChatBaseComponent::literal).reduce((ichatmutablecomponent1, ichatmutablecomponent2) -> {
                    ...
                    return IChatBaseComponent.literal((String) stream1.collect(Collectors.joining(", ")));
                });
            }
        } else {
    ...
    private static String asString(NBTBase nbtbase) {
