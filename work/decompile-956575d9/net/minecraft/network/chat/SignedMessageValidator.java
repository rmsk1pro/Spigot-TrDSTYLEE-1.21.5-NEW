    ...
    public static class a implements SignedMessageValidator {
        ...
        private boolean validate(PlayerChatMessage playerchatmessage) {
            if (this.expired.getAsBoolean()) {
                SignedMessageValidator.a.LOGGER.error("Received message with expired profile public key from {} with session {}", playerchatmessage.sender(), playerchatmessage.link().sessionId());
                return false;
            } else if (!playerchatmessage.verify(this.validator)) {
                SignedMessageValidator.a.LOGGER.error("Received message with invalid signature (is the session wrong, or signature cache out of sync?): {}", PlayerChatMessage.describeSigned(playerchatmessage));
                return false;
            } else {
