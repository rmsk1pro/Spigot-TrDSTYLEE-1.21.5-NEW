...
public class ChatComponentUtils {
    ...
    private static ChatModifier resolveStyle(@Nullable CommandListenerWrapper commandlistenerwrapper, ChatModifier chatmodifier, @Nullable Entity entity, int i) throws CommandSyntaxException {
        ...
        if (chathoverable instanceof ChatHoverable.e chathoverable_e) {
            ChatHoverable.e chathoverable_e1 = chathoverable_e;

            try {
                ichatbasecomponent = chathoverable_e1.value();
            } catch (Throwable throwable) {
                throw new MatchException(throwable.toString(), throwable);
            }

            ChatHoverable chathoverable1 = ichatbasecomponent;

            chathoverable1 = new ChatHoverable.e(updateForEntity(commandlistenerwrapper, chathoverable1, entity, i + 1));
    ...
    public static IChatMutableComponent copyOnClickText(String s) {
        return wrapInSquareBrackets(IChatBaseComponent.literal(s).withStyle((chatmodifier) -> {
            return chatmodifier.withColor(EnumChatFormat.GREEN).withClickEvent(new ChatClickable.CopyToClipboard(s)).withHoverEvent(new ChatHoverable.e(IChatBaseComponent.translatable("chat.copy.click"))).withInsertion(s);
        }));
    }
}
