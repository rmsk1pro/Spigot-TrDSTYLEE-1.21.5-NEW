...
public class PacketDebug {
    ...
    public static void sendWireUpdates(World world, RedstoneWireOrientationsDebugPayload redstonewireorientationsdebugpayload) {
        if (world instanceof WorldServer worldserver) {
            sendPacketToAllPlayers(worldserver, redstonewireorientationsdebugpayload);
        }
    ...
    private static List<String> getMemoryDescriptions(EntityLiving entityliving, long i) {
        ...
        for (Map.Entry<MemoryModuleType<?>, Optional<? extends ExpirableMemory<?>>> map_entry : map.entrySet()) {
            MemoryModuleType<?> memorymoduletype = (MemoryModuleType) map_entry.getKey();
            Optional<? extends ExpirableMemory<?>> optional = (Optional) map_entry.getValue();
            String s;
    ...
    private static String getShortDescription(WorldServer worldserver, @Nullable Object object) {
        if (object == null) {
            ...
            return entity1 == null ? object.toString() : getShortDescription(worldserver, entity1);
