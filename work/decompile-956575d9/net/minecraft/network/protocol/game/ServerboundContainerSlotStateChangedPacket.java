...
public record ServerboundContainerSlotStateChangedPacket(int slotId, int containerId, boolean newState) implements Packet<PacketListenerPlayIn> {
    ...
    public static final StreamCodec<PacketDataSerializer, ServerboundContainerSlotStateChangedPacket> STREAM_CODEC = Packet.<PacketDataSerializer, ServerboundContainerSlotStateChangedPacket>codec(ServerboundContainerSlotStateChangedPacket::write, ServerboundContainerSlotStateChangedPacket::new);

    private ServerboundContainerSlotStateChangedPacket(PacketDataSerializer packetdataserializer) {
        this(packetdataserializer.readVarInt(), packetdataserializer.readContainerId(), packetdataserializer.readBoolean());
    }

    private void write(PacketDataSerializer packetdataserializer) {
        packetdataserializer.writeVarInt(this.slotId);
        packetdataserializer.writeContainerId(this.containerId);
