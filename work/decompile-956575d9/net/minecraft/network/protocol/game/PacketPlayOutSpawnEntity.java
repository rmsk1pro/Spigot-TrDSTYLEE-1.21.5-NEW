...
public class PacketPlayOutSpawnEntity implements Packet<PacketListenerPlayOut> {
    ...
    public static final StreamCodec<RegistryFriendlyByteBuf, PacketPlayOutSpawnEntity> STREAM_CODEC = Packet.<RegistryFriendlyByteBuf, PacketPlayOutSpawnEntity>codec(PacketPlayOutSpawnEntity::write, PacketPlayOutSpawnEntity::new);
    ...
    public PacketPlayOutSpawnEntity(int i, UUID uuid, double d0, double d1, double d2, float f, float f1, EntityTypes<?> entitytypes, int j, Vec3D vec3d, double d3) {
        ...
        this.yRot = MathHelper.packDegrees(f1);
        this.yHeadRot = MathHelper.packDegrees((float) d3);
