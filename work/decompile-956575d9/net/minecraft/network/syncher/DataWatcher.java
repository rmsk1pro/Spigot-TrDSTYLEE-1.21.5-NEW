...
public class DataWatcher {
    ...
    public <T> T get(DataWatcherObject<T> datawatcherobject) {
        return (T) this.getItem(datawatcherobject).getValue();
    }
    ...
    public <T> void set(DataWatcherObject<T> datawatcherobject, T t0, boolean flag) {
        DataWatcher.Item<T> datawatcher_item = this.<T>getItem(datawatcherobject);
    ...
    public List<DataWatcher.c<?>> packDirty() {
        if (!this.isDirty) {
            ...
            for (DataWatcher.Item<?> datawatcher_item : this.itemsById) {
    ...
    public static record c<T>(int id, DataWatcherSerializer<T> serializer, T value) {
        ...
        public static <T> DataWatcher.c<T> create(DataWatcherObject<T> datawatcherobject, T t0) {
            ...
            return new DataWatcher.c<T>(datawatcherobject.id(), datawatcherserializer, datawatcherserializer.copy(t0));
        }
        ...
        private static <T> DataWatcher.c<T> read(RegistryFriendlyByteBuf registryfriendlybytebuf, int i, DataWatcherSerializer<T> datawatcherserializer) {
            return new DataWatcher.c<T>(i, datawatcherserializer, datawatcherserializer.codec().decode(registryfriendlybytebuf));
        }
    }
    ...
    public static class Item<T> {
        ...
        public DataWatcher.c<T> value() {
            return DataWatcher.c.<T>create(this.accessor, this.value);
        }
    }
