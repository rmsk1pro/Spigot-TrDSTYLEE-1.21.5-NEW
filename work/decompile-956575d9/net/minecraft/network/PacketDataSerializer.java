...
public class PacketDataSerializer extends ByteBuf {
    ...
    public <T> T readWithCodecTrusted(DynamicOps<NBTBase> dynamicops, Codec<T> codec) {
        return (T) this.readWithCodec(dynamicops, codec, NBTReadLimiter.unlimitedHeap());
    }
    ...
    public <T> T readWithCodec(DynamicOps<NBTBase> dynamicops, Codec<T> codec, NBTReadLimiter nbtreadlimiter) {
        ...
        return (T) codec.parse(dynamicops, nbtbase).getOrThrow((s) -> {
    ...
    public <E extends Enum<E>> void writeEnumSet(EnumSet<E> enumset, Class<E> oclass) {
        E[] ae = (E[]) (oclass.getEnumConstants());
    ...
    public static <T, B extends ByteBuf> T readNullable(B b0, StreamDecoder<? super B, T> streamdecoder) {
        return (T) (b0.readBoolean() ? streamdecoder.decode(b0) : null);
    }
    ...
    public PacketDataSerializer writeLongArray(long[] along) {
        writeLongArray(this, along);
        return this;
    }

    public static void writeLongArray(ByteBuf bytebuf, long[] along) {
        VarInt.write(bytebuf, along.length);
        writeFixedSizeLongArray(bytebuf, along);
    }

    public PacketDataSerializer writeFixedSizeLongArray(long[] along) {
        writeFixedSizeLongArray(this, along);
        return this;
    }

    public static void writeFixedSizeLongArray(ByteBuf bytebuf, long[] along) {
        for (long i : along) {
            bytebuf.writeLong(i);
        }
    ...
    public long[] readFixedSizeLongArray(long[] along) {
        return readFixedSizeLongArray(this, along);
    }
    ...
    public static long[] readFixedSizeLongArray(ByteBuf bytebuf, long[] along) {
        for (int i = 0; i < along.length; ++i) {
            along[i] = bytebuf.readLong();
        }
    ...
    public GlobalPos readGlobalPos() {
        ResourceKey<World> resourcekey = this.<World>readResourceKey(Registries.DIMENSION);
    ...
    public MovingObjectPositionBlock readBlockHitResult() {
        ...
        return new MovingObjectPositionBlock(new Vec3D((double) blockposition.getX() + (double) f, (double) blockposition.getY() + (double) f1, (double) blockposition.getZ() + (double) f2), enumdirection, blockposition, flag, flag1);
    }
    ...
    public void writeBlockHitResult(MovingObjectPositionBlock movingobjectpositionblock) {
        ...
        this.writeBoolean(movingobjectpositionblock.isWorldBorderHit());
    }
