...
public interface ByteBufCodecs {
    ...
    StreamCodec<ByteBuf, Float> ROTATION_BYTE = ByteBufCodecs.BYTE.map(MathHelper::unpackDegrees, MathHelper::packDegrees);
    ...
    StreamCodec<ByteBuf, long[]> LONG_ARRAY = new StreamCodec<ByteBuf, long[]>() {
        ...
        public void encode(ByteBuf bytebuf, long[] along) {
            PacketDataSerializer.writeLongArray(bytebuf, along);
        }
    };
    ...
    StreamCodec<ByteBuf, Integer> CONTAINER_ID = new StreamCodec<ByteBuf, Integer>() {
        public Integer decode(ByteBuf bytebuf) {
            return PacketDataSerializer.readContainerId(bytebuf);
        }

        public void encode(ByteBuf bytebuf, Integer integer) {
            PacketDataSerializer.writeContainerId(bytebuf, integer);
        }
    };
    ...
    static <T> StreamCodec<RegistryFriendlyByteBuf, T> fromCodecWithRegistries(final Codec<T> codec, Supplier<NBTReadLimiter> supplier) {
        ...
        return new StreamCodec<RegistryFriendlyByteBuf, T>() {
            public T decode(RegistryFriendlyByteBuf registryfriendlybytebuf) {
                ...
                RegistryOps<NBTBase> registryops = registryfriendlybytebuf.registryAccess().<NBTBase>createSerializationContext(DynamicOpsNBT.INSTANCE);

                return (T) codec.parse(registryops, nbtbase).getOrThrow((s) -> {
                    return new DecoderException("Failed to decode: " + s + " " + String.valueOf(nbtbase));
                });
            }
    ...
    static <B extends ByteBuf, V, C extends Collection<V>> StreamCodec<B, C> collection(final IntFunction<C> intfunction, final StreamCodec<? super B, V> streamcodec, final int i) {
        return new StreamCodec<B, C>() {
            public C decode(B b0) {
                ...
                C c0 = (C) ((Collection) intfunction.apply(Math.min(j, 65536)));
    ...
    static <B extends ByteBuf, K, V, M extends Map<K, V>> StreamCodec<B, M> map(final IntFunction<? extends M> intfunction, final StreamCodec<? super B, K> streamcodec, final StreamCodec<? super B, V> streamcodec1, final int i) {
        return new StreamCodec<B, M>() {
            ...
            public M decode(B b0) {
                int j = ByteBufCodecs.readCount(b0, i);
                M m0 = (M) ((Map) intfunction.apply(Math.min(j, 65536)));

                for (int k = 0; k < j; ++k) {
                    K k0 = (K) streamcodec.decode(b0);
                    V v0 = (V) streamcodec1.decode(b0);
        ...
        return (streamcodec) -> {
            return new StreamCodec<B, V>() {
                public V decode(B b0) {
                    int j = VarInt.read(b0);

                    if (j > i) {
                        throw new DecoderException("Buffer size " + j + " is larger than allowed limit of " + i);
                    } else {
                        int k = b0.readerIndex();
                        B b1 = (B) ((ByteBuf) bifunction.apply(b0, b0.slice(k, j)));
                ...
                public void encode(B b0, V v0) {
                    B b1 = (B) ((ByteBuf) bifunction.apply(b0, b0.alloc().buffer()));

                    try {
                        streamcodec.encode(b1, v0);
                        int j = b1.readableBytes();

                        if (j > i) {
                            throw new EncoderException("Buffer size " + j + " is  larger than allowed limit of " + i);
                        }
    ...
    static <V> StreamCodec.a<RegistryFriendlyByteBuf, V, V> lengthPrefixed(int i) {
    ...
    static <T> StreamCodec<RegistryFriendlyByteBuf, HolderSet<T>> holderSet(final ResourceKey<? extends IRegistry<T>> resourcekey) {
        return new StreamCodec<RegistryFriendlyByteBuf, HolderSet<T>>() {
            ...
            public HolderSet<T> decode(RegistryFriendlyByteBuf registryfriendlybytebuf) {
                ...
                if (i == -1) {
                    IRegistry<T> iregistry = registryfriendlybytebuf.registryAccess().lookupOrThrow(resourcekey);

                    return (HolderSet) iregistry.get(TagKey.create(resourcekey, (MinecraftKey) MinecraftKey.STREAM_CODEC.decode(registryfriendlybytebuf))).orElseThrow();
                } else {
