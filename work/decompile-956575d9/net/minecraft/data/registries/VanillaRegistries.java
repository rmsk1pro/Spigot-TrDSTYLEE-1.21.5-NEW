...
public class VanillaRegistries {
            ...
            list.stream().flatMap(HolderSet::stream).forEach((holder) -> {
                holder.unwrap().ifLeft((resourcekey) -> {
                    ...
                    if (!validatePlacedFeature((PlacedFeature) holder_c1.value())) {
                        String s = String.valueOf(resourcekey.location());

                        SystemUtils.logAndPauseIfInIde("Placed feature " + s + " in biome " + String.valueOf(minecraftkey) + " is missing BiomeFilter.biome()");
                    }

                }).ifRight((placedfeature) -> {
                    if (!validatePlacedFeature(placedfeature)) {
                        SystemUtils.logAndPauseIfInIde("Placed inline feature in biome " + String.valueOf(holder_c) + " is missing BiomeFilter.biome()");
                    }
    ...
    public static HolderLookup.a createLookup() {
        IRegistryCustom.Dimension iregistrycustom_dimension = IRegistryCustom.fromRegistryOfRegistries(BuiltInRegistries.REGISTRY);
        HolderLookup.a holderlookup_a = VanillaRegistries.BUILDER.build(iregistrycustom_dimension);

        validateThatAllBiomeFeaturesHaveBiomeFilter(holderlookup_a);
