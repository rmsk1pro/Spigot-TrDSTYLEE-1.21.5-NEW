...
public class FeatureUtils {
    ...
    private static BlockPredicate simplePatchPredicate(List<Block> list) {
        ...
        if (!list.isEmpty()) {
            blockpredicate = BlockPredicate.allOf(BlockPredicate.ONLY_IN_AIR_PREDICATE, BlockPredicate.matchesBlocks(EnumDirection.DOWN.getUnitVec3i(), list));
        } else {
    ...
    public static ResourceKey<WorldGenFeatureConfigured<?, ?>> createKey(String s) {
        return ResourceKey.create(Registries.CONFIGURED_FEATURE, MinecraftKey.withDefaultNamespace(s));
    }
