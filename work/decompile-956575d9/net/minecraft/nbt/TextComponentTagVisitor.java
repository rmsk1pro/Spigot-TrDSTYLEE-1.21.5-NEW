...
public class TextComponentTagVisitor implements TagVisitor {
    ...
    public void visitString(NBTTagString nbttagstring) {
        String s = NBTTagString.quoteAndEscape(nbttagstring.value());
        String s1 = s.substring(0, 1);
        IChatBaseComponent ichatbasecomponent = IChatBaseComponent.literal(s.substring(1, s.length() - 1)).withStyle(TextComponentTagVisitor.SYNTAX_HIGHLIGHTING_STRING);

        this.result.append(s1).append(ichatbasecomponent).append(s1);
    }

    @Override
    public void visitByte(NBTTagByte nbttagbyte) {
        this.result.append((IChatBaseComponent) IChatBaseComponent.literal(String.valueOf(nbttagbyte.value())).withStyle(TextComponentTagVisitor.SYNTAX_HIGHLIGHTING_NUMBER)).append(TextComponentTagVisitor.BYTE_TYPE);
    }

    @Override
    public void visitShort(NBTTagShort nbttagshort) {
        this.result.append((IChatBaseComponent) IChatBaseComponent.literal(String.valueOf(nbttagshort.value())).withStyle(TextComponentTagVisitor.SYNTAX_HIGHLIGHTING_NUMBER)).append(TextComponentTagVisitor.SHORT_TYPE);
    }

    @Override
    public void visitInt(NBTTagInt nbttagint) {
        this.result.append((IChatBaseComponent) IChatBaseComponent.literal(String.valueOf(nbttagint.value())).withStyle(TextComponentTagVisitor.SYNTAX_HIGHLIGHTING_NUMBER));
    }

    @Override
    public void visitLong(NBTTagLong nbttaglong) {
        this.result.append((IChatBaseComponent) IChatBaseComponent.literal(String.valueOf(nbttaglong.value())).withStyle(TextComponentTagVisitor.SYNTAX_HIGHLIGHTING_NUMBER)).append(TextComponentTagVisitor.LONG_TYPE);
    }

    @Override
    public void visitFloat(NBTTagFloat nbttagfloat) {
        this.result.append((IChatBaseComponent) IChatBaseComponent.literal(String.valueOf(nbttagfloat.value())).withStyle(TextComponentTagVisitor.SYNTAX_HIGHLIGHTING_NUMBER)).append(TextComponentTagVisitor.FLOAT_TYPE);
    }

    @Override
    public void visitDouble(NBTTagDouble nbttagdouble) {
        this.result.append((IChatBaseComponent) IChatBaseComponent.literal(String.valueOf(nbttagdouble.value())).withStyle(TextComponentTagVisitor.SYNTAX_HIGHLIGHTING_NUMBER)).append(TextComponentTagVisitor.DOUBLE_TYPE);
    }
    ...
    public void visitLongArray(NBTTagLongArray nbttaglongarray) {
        ...
        for (int i = 0; i < along.length && i < 128; ++i) {
            IChatBaseComponent ichatbasecomponent = IChatBaseComponent.literal(String.valueOf(along[i])).withStyle(TextComponentTagVisitor.SYNTAX_HIGHLIGHTING_NUMBER);

            this.result.append(" ").append(ichatbasecomponent).append(TextComponentTagVisitor.LONG_TYPE);
            if (i != along.length - 1) {
                this.result.append(TextComponentTagVisitor.ELEMENT_SEPARATOR);
            }
        }
    ...
    private static boolean shouldWrapListElements(NBTTagList nbttaglist) {
    ...
    public void visitList(NBTTagList nbttaglist) {
        ...
        } else if (!shouldWrapListElements(nbttaglist)) {
            this.result.append("[");
    ...
    public void visitCompound(NBTTagCompound nbttagcompound) {
        if (nbttagcompound.isEmpty()) {
            ...
            this.result.append("{");
            Collection<String> collection = nbttagcompound.keySet();
    ...
    protected static IChatBaseComponent handleEscapePretty(String s) {
        if (TextComponentTagVisitor.SIMPLE_VALUE.matcher(s).matches()) {
            ...
            IChatBaseComponent ichatbasecomponent = IChatBaseComponent.literal(s1.substring(1, s1.length() - 1)).withStyle(TextComponentTagVisitor.SYNTAX_HIGHLIGHTING_KEY);

            return IChatBaseComponent.literal(s2).append(ichatbasecomponent).append(s2);
        }
    }
