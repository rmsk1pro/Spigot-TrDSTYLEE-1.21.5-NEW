...
public class ResourceKeyArgument<T> implements ArgumentType<ResourceKey<T>> {
    ...
    public static <T> ResourceKey<T> getRegistryKey(CommandContext<CommandListenerWrapper> commandcontext, String s, ResourceKey<IRegistry<T>> resourcekey, DynamicCommandExceptionType dynamiccommandexceptiontype) throws CommandSyntaxException {
    ...
    private static <T> IRegistry<T> getRegistry(CommandContext<CommandListenerWrapper> commandcontext, ResourceKey<? extends IRegistry<T>> resourcekey) {
        return ((CommandListenerWrapper) commandcontext.getSource()).getServer().registryAccess().lookupOrThrow(resourcekey);
    }
    ...
    private static <T> Holder.c<T> resolveKey(CommandContext<CommandListenerWrapper> commandcontext, String s, ResourceKey<IRegistry<T>> resourcekey, DynamicCommandExceptionType dynamiccommandexceptiontype) throws CommandSyntaxException {
        ...
        return (Holder.c) getRegistry(commandcontext, resourcekey).get(resourcekey1).orElseThrow(() -> {
            return dynamiccommandexceptiontype.create(resourcekey1.location());
        });
    }
    ...
    public static RecipeHolder<?> getRecipe(CommandContext<CommandListenerWrapper> commandcontext, String s) throws CommandSyntaxException {
        ...
        ResourceKey<IRecipe<?>> resourcekey = getRegistryKey(commandcontext, s, Registries.RECIPE, ResourceKeyArgument.ERROR_INVALID_RECIPE);

        return (RecipeHolder) craftingmanager.byKey(resourcekey).orElseThrow(() -> {
            return ResourceKeyArgument.ERROR_INVALID_RECIPE.create(resourcekey.location());
        });
    }
    ...
    public static AdvancementHolder getAdvancement(CommandContext<CommandListenerWrapper> commandcontext, String s) throws CommandSyntaxException {
        ResourceKey<Advancement> resourcekey = getRegistryKey(commandcontext, s, Registries.ADVANCEMENT, ResourceKeyArgument.ERROR_INVALID_ADVANCEMENT);
        AdvancementHolder advancementholder = ((CommandListenerWrapper) commandcontext.getSource()).getServer().getAdvancements().get(resourcekey.location());

        if (advancementholder == null) {
            throw ResourceKeyArgument.ERROR_INVALID_ADVANCEMENT.create(resourcekey.location());
        } else {
