...
public class PlayerSelector {
        ...
        if (PlayerSelector.OPTIONS.isEmpty()) {
                    ...
                    if (enumgamemode == null) {
                        ...
                        argumentparserselector.addPredicate((entity) -> {
                            if (entity instanceof EntityPlayer entityplayer) {
                                EnumGamemode enumgamemode1 = entityplayer.gameMode();

                                return enumgamemode1 == enumgamemode ^ flag;
                            } else {
                                return false;
                            }
                        });
            ...
            register("type", (argumentparserselector) -> {
                argumentparserselector.setSuggestions((suggestionsbuilder, consumer) -> {
                    ...
                    ICompletionProvider.suggestResource(BuiltInRegistries.ENTITY_TYPE.getTags().map((holderset_named) -> {
                ...
                if (argumentparserselector.isTypeLimitedInversely() && !flag) {
                    ...
                    if (argumentparserselector.isTag()) {
                        TagKey<EntityTypes<?>> tagkey = TagKey.<EntityTypes<?>>create(Registries.ENTITY_TYPE, MinecraftKey.read(argumentparserselector.getReader()));
            ...
            register("nbt", (argumentparserselector) -> {
                boolean flag = argumentparserselector.shouldInvertValue();
                NBTTagCompound nbttagcompound = MojangsonParser.parseCompoundAsArgument(argumentparserselector.getReader());
            ...
            register("scores", (argumentparserselector) -> {
                ...
                if (!map.isEmpty()) {
                    argumentparserselector.addPredicate((entity) -> {
                        ...
                        for (Map.Entry<String, CriterionConditionValue.IntegerRange> map_entry : map.entrySet()) {
                            ScoreboardObjective scoreboardobjective = scoreboard.getObjective((String) map_entry.getKey());
                            ...
                            if (!((CriterionConditionValue.IntegerRange) map_entry.getValue()).matches(readonlyscoreinfo.value())) {
                                return false;
                            }
                        }
            ...
            register("advancements", (argumentparserselector) -> {
                ...
                while (stringreader.canRead() && stringreader.peek() != '}') {
                    ...
                    if (stringreader.canRead() && stringreader.peek() == '{') {
                        ...
                        stringreader.expect('}');
                        stringreader.skipWhitespace();
                        map.put(minecraftkey, (Predicate) (advancementprogress) -> {
                ...
                if (!map.isEmpty()) {
                    argumentparserselector.addPredicate((entity) -> {
                        if (!(entity instanceof EntityPlayer entityplayer)) {
                            ...
                            for (Map.Entry<MinecraftKey, Predicate<AdvancementProgress>> map_entry : map.entrySet()) {
                                AdvancementHolder advancementholder = advancementdataworld.get((MinecraftKey) map_entry.getKey());

                                if (advancementholder == null || !((Predicate) map_entry.getValue()).test(advancementdataplayer.getOrStartProgress(advancementholder))) {
                                    return false;
                                }
                            }
            ...
            register("predicate", (argumentparserselector) -> {
                ...
                argumentparserselector.addPredicate((entity) -> {
                    if (!(entity.level() instanceof WorldServer)) {
                        ...
                        Optional<LootItemCondition> optional = worldserver.getServer().reloadableRegistries().lookup().get(resourcekey).map(Holder::value);
    ...
    public static void suggestNames(ArgumentParserSelector argumentparserselector, SuggestionsBuilder suggestionsbuilder) {
        ...
        for (Map.Entry<String, PlayerSelector.b> map_entry : PlayerSelector.OPTIONS.entrySet()) {
            if (((PlayerSelector.b) map_entry.getValue()).canUse.test(argumentparserselector) && ((String) map_entry.getKey()).toLowerCase(Locale.ROOT).startsWith(s)) {
                suggestionsbuilder.suggest((String) map_entry.getKey() + "=", ((PlayerSelector.b) map_entry.getValue()).description);
            }
        }
