...
public class ArgumentParticle implements ArgumentType<ParticleParam> {
    ...
    public static ParticleParam readParticle(StringReader stringreader, HolderLookup.a holderlookup_a) throws CommandSyntaxException {
        ...
        return readParticle(ArgumentParticle.VALUE_PARSER, stringreader, particle, holderlookup_a);
    }
    ...
    private static <T extends ParticleParam, O> T readParticle(MojangsonParser<O> mojangsonparser, StringReader stringreader, Particle<T> particle, HolderLookup.a holderlookup_a) throws CommandSyntaxException {
        RegistryOps<O> registryops = holderlookup_a.<O>createSerializationContext(mojangsonparser.getOps());
        O o0;

        if (stringreader.canRead() && stringreader.peek() == '{') {
            o0 = mojangsonparser.parseAsArgument(stringreader);
        } else {
            o0 = registryops.emptyMap();
        }

        DataResult dataresult = particle.codec().codec().parse(registryops, o0);
