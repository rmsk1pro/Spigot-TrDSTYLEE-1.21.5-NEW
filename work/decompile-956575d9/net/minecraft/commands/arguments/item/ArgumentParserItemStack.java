...
public class ArgumentParserItemStack {
    ...
    public ArgumentParserItemStack(HolderLookup.a holderlookup_a) {
        ...
        this.registryOps = holderlookup_a.<NBTBase>createSerializationContext(DynamicOpsNBT.INSTANCE);
        this.tagParser = MojangsonParser.<NBTBase>create(this.registryOps);
    }
    ...
    public ArgumentParserItemStack.a parse(StringReader stringreader) throws CommandSyntaxException {
        ...
        this.parse(stringreader, new ArgumentParserItemStack.d() {
            @Override
    ...
    private static void validateComponents(StringReader stringreader, Holder<Item> holder, DataComponentPatch datacomponentpatch) throws CommandSyntaxException {
        DataComponentMap datacomponentmap = PatchedDataComponentMap.fromPatch(((Item) holder.value()).components(), datacomponentpatch);
    ...
    private class b {
            ...
            while (this.reader.canRead() && this.reader.peek() != ']') {
                ...
                if (this.reader.canRead() && this.reader.peek() == '!') {
                    ...
                    if (!set.add(datacomponenttype1)) {
                        throw ArgumentParserItemStack.ERROR_REPEATED_COMPONENT.create(datacomponenttype1);
                    }
                    ...
                    this.visitor.visitSuggestions(ArgumentParserItemStack.SUGGEST_NOTHING);
                    this.reader.skipWhitespace();
                    this.readComponent(ArgumentParserItemStack.this.tagParser, ArgumentParserItemStack.this.registryOps, datacomponenttype1);
                    this.reader.skipWhitespace();
                }
        ...
        private <T, O> void readComponent(MojangsonParser<O> mojangsonparser, RegistryOps<O> registryops, DataComponentType<T> datacomponenttype) throws CommandSyntaxException {
            int i = this.reader.getCursor();
            O o0 = mojangsonparser.parseAsArgument(this.reader);
            DataResult<T> dataresult = datacomponenttype.codecOrThrow().parse(registryops, o0);
