...
public class ArgumentVec2 implements ArgumentType<IVectorPosition> {
    ...
    public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> commandcontext, SuggestionsBuilder suggestionsbuilder) {
        if (!(commandcontext.getSource() instanceof ICompletionProvider)) {
            ...
            Collection<ICompletionProvider.b> collection;

            if (!s.isEmpty() && s.charAt(0) == '^') {
                collection = Collections.singleton(ICompletionProvider.b.DEFAULT_LOCAL);
            } else {
                collection = ((ICompletionProvider) commandcontext.getSource()).getAbsoluteCoordinates();
            }

            return ICompletionProvider.suggest2DCoordinates(s, collection, suggestionsbuilder, CommandDispatcher.createValidator(this::parse));
        }
    }
