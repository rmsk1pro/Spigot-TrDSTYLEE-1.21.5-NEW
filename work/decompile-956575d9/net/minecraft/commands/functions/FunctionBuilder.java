...
class FunctionBuilder<T extends ExecutionCommandSource<T>> {
    ...
    public CommandFunction<T> build(MinecraftKey minecraftkey) {
        return (CommandFunction<T>) (this.macroEntries != null ? new MacroFunction(minecraftkey, this.macroEntries, this.macroArguments) : new PlainTextFunction(minecraftkey, this.plainEntries));
    }
}
