...
public class LightEngineThreaded extends LevelLightEngine implements AutoCloseable {
    ...
    public LightEngineThreaded(ILightAccess ilightaccess, PlayerChunkMap playerchunkmap, boolean flag, ConsecutiveExecutor consecutiveexecutor, ChunkTaskDispatcher chunktaskdispatcher) {
    ...
    protected void updateChunkStatus(ChunkCoordIntPair chunkcoordintpair) {
        this.addTask(chunkcoordintpair.x, chunkcoordintpair.z, () -> {
            ...
            for (int j = this.levelHeightAccessor.getMinSectionY(); j <= this.levelHeightAccessor.getMaxSectionY(); ++j) {
                super.updateSectionStatus(SectionPosition.of(chunkcoordintpair, j), true);
            }
    ...
    private void runUpdate() {
        ...
        for (j = 0; objectlistiterator.hasNext() && j < i; ++j) {
            Pair<LightEngineThreaded.Update, Runnable> pair = (Pair) objectlistiterator.next();
        ...
        for (int k = 0; objectlistiterator.hasNext() && k < i; ++k) {
            Pair<LightEngineThreaded.Update, Runnable> pair1 = (Pair) objectlistiterator.next();

            if (pair1.getFirst() == LightEngineThreaded.Update.POST_UPDATE) {
