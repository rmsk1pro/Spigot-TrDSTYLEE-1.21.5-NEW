...
public class PlayerChunkSender {
    ...
    private List<Chunk> collectChunksToSend(PlayerChunkMap playerchunkmap, ChunkCoordIntPair chunkcoordintpair) {
        ...
        if (!this.memoryConnection && this.pendingChunks.size() > i) {
            ...
            LongStream longstream = ((List) stream.collect(Comparators.least(i, Comparator.comparingInt(chunkcoordintpair::distanceSquared)))).stream().mapToLong(Long::longValue);
            ...
            Objects.requireNonNull(playerchunkmap);
            list = longstream1.mapToObj(playerchunkmap::getChunkToSend).filter(Objects::nonNull).sorted(Comparator.comparingInt((chunk) -> {
