...
public class RaidCommand {
    ...
    public static void register(CommandDispatcher<CommandListenerWrapper> commanddispatcher, CommandBuildContext commandbuildcontext) {
        commanddispatcher.register((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) net.minecraft.commands.CommandDispatcher.literal("raid").requires((commandlistenerwrapper) -> {
            ...
            return playSound((CommandListenerWrapper) commandcontext.getSource(), ArgumentChatComponent.getResolvedComponent(commandcontext, "type"));
    ...
    private static int glow(CommandListenerWrapper commandlistenerwrapper) throws CommandSyntaxException {
        ...
        if (raid != null) {
            for (EntityRaider entityraider : raid.getAllRaiders()) {
    ...
    private static int spawnLeader(CommandListenerWrapper commandlistenerwrapper) {
        ...
        EntityRaider entityraider = EntityTypes.PILLAGER.create(commandlistenerwrapper.getLevel(), EntitySpawnReason.COMMAND);
        ...
        if (entityraider == null) {
            ...
            entityraider.setItemSlot(EnumItemSlot.HEAD, Raid.getOminousBannerInstance(commandlistenerwrapper.registryAccess().lookupOrThrow(Registries.BANNER_PATTERN)));
            ...
            entityraider.finalizeSpawn(commandlistenerwrapper.getLevel(), commandlistenerwrapper.getLevel().getCurrentDifficultyAt(BlockPosition.containing(commandlistenerwrapper.getPosition())), EntitySpawnReason.COMMAND, (GroupDataEntity) null);
    ...
    private static int playSound(CommandListenerWrapper commandlistenerwrapper, @Nullable IChatBaseComponent ichatbasecomponent) {
        if (ichatbasecomponent != null && ichatbasecomponent.getString().equals("local")) {
            ...
            worldserver.playSeededSound((Entity) null, vec3d.x, vec3d.y, vec3d.z, SoundEffects.RAID_HORN, SoundCategory.NEUTRAL, 2.0F, 1.0F, worldserver.random.nextLong());
        }
