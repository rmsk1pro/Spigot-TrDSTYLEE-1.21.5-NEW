...
public class CommandTag {
    ...
    public static void register(CommandDispatcher<CommandListenerWrapper> commanddispatcher) {
        commanddispatcher.register((LiteralArgumentBuilder) ((LiteralArgumentBuilder) net.minecraft.commands.CommandDispatcher.literal("tag").requires((commandlistenerwrapper) -> {
            ...
            return ICompletionProvider.suggest(getTags(ArgumentEntity.getEntities(commandcontext, "targets")), suggestionsbuilder);
        ...
        if (i == 0) {
            ...
            if (collection.size() == 1) {
                commandlistenerwrapper.sendSuccess(() -> {
                    return IChatBaseComponent.translatable("commands.tag.add.success.single", s, ((Entity) collection.iterator().next()).getDisplayName());
                }, true);
            } else {
                commandlistenerwrapper.sendSuccess(() -> {
                    return IChatBaseComponent.translatable("commands.tag.add.success.multiple", s, collection.size());
                }, true);
            }
    ...
    private static int removeTag(CommandListenerWrapper commandlistenerwrapper, Collection<? extends Entity> collection, String s) throws CommandSyntaxException {
        ...
        if (i == 0) {
            ...
            if (collection.size() == 1) {
                commandlistenerwrapper.sendSuccess(() -> {
                    return IChatBaseComponent.translatable("commands.tag.remove.success.single", s, ((Entity) collection.iterator().next()).getDisplayName());
                }, true);
            } else {
                commandlistenerwrapper.sendSuccess(() -> {
                    return IChatBaseComponent.translatable("commands.tag.remove.success.multiple", s, collection.size());
                }, true);
            }
    ...
    private static int listTags(CommandListenerWrapper commandlistenerwrapper, Collection<? extends Entity> collection) {
        ...
        if (collection.size() == 1) {
            ...
            if (set.isEmpty()) {
                commandlistenerwrapper.sendSuccess(() -> {
                    return IChatBaseComponent.translatable("commands.tag.list.single.empty", entity1.getDisplayName());
                }, false);
            } else {
                commandlistenerwrapper.sendSuccess(() -> {
                    return IChatBaseComponent.translatable("commands.tag.list.single.success", entity1.getDisplayName(), set.size(), ChatComponentUtils.formatList(set));
                }, false);
            }
        } else if (set.isEmpty()) {
            commandlistenerwrapper.sendSuccess(() -> {
                return IChatBaseComponent.translatable("commands.tag.list.multiple.empty", collection.size());
            }, false);
        } else {
            commandlistenerwrapper.sendSuccess(() -> {
                return IChatBaseComponent.translatable("commands.tag.list.multiple.success", collection.size(), set.size(), ChatComponentUtils.formatList(set));
            }, false);
        }
