            ...
            for (Entity entity : collection) {
                if (entity instanceof EntityLiving) {
                    ...
                    if (!itemstack.isEmpty()) {
                        if (enchantment.canEnchant(itemstack) && EnchantmentManager.isEnchantmentCompatible(EnchantmentManager.getEnchantmentsForCrafting(itemstack).keySet(), holder)) {
                            itemstack.enchant(holder, i);
                            ++j;
                        } else if (collection.size() == 1) {
                            throw CommandEnchant.ERROR_INCOMPATIBLE.create(itemstack.getHoverName().getString());
                        }
                    } else if (collection.size() == 1) {
            ...
            if (j == 0) {
                ...
                if (collection.size() == 1) {
                    commandlistenerwrapper.sendSuccess(() -> {
                        return IChatBaseComponent.translatable("commands.enchant.success.single", Enchantment.getFullname(holder, i), ((Entity) collection.iterator().next()).getDisplayName());
                    }, true);
                } else {
                    commandlistenerwrapper.sendSuccess(() -> {
                        return IChatBaseComponent.translatable("commands.enchant.success.multiple", Enchantment.getFullname(holder, i), collection.size());
                    }, true);
                }
