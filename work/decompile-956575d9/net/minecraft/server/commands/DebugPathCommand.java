...
public class DebugPathCommand {
    ...
    private static int fillBlocks(CommandListenerWrapper commandlistenerwrapper, BlockPosition blockposition) throws CommandSyntaxException {
        ...
        if (!(entity instanceof EntityInsentient entityinsentient)) {
            ...
            NavigationAbstract navigationabstract = new Navigation(entityinsentient, commandlistenerwrapper.getLevel());
            PathEntity pathentity = navigationabstract.createPath(blockposition, 0);

            PacketDebug.sendPathFindingPacket(commandlistenerwrapper.getLevel(), entityinsentient, pathentity, navigationabstract.getMaxDistanceToWaypoint());
            if (pathentity == null) {
