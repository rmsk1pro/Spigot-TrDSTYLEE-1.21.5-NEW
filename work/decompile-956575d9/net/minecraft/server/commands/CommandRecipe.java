...
public class CommandRecipe {
    ...
    public static void register(CommandDispatcher<CommandListenerWrapper> commanddispatcher) {
        ...
        })).then(net.minecraft.commands.CommandDispatcher.literal("give").then(((RequiredArgumentBuilder) net.minecraft.commands.CommandDispatcher.argument("targets", ArgumentEntity.players()).then(net.minecraft.commands.CommandDispatcher.argument("recipe", ResourceKeyArgument.key(Registries.RECIPE)).executes((commandcontext) -> {
            return giveRecipes((CommandListenerWrapper) commandcontext.getSource(), ArgumentEntity.getPlayers(commandcontext, "targets"), Collections.singleton(ResourceKeyArgument.getRecipe(commandcontext, "recipe")));
        ...
        }))))).then(net.minecraft.commands.CommandDispatcher.literal("take").then(((RequiredArgumentBuilder) net.minecraft.commands.CommandDispatcher.argument("targets", ArgumentEntity.players()).then(net.minecraft.commands.CommandDispatcher.argument("recipe", ResourceKeyArgument.key(Registries.RECIPE)).executes((commandcontext) -> {
            return takeRecipes((CommandListenerWrapper) commandcontext.getSource(), ArgumentEntity.getPlayers(commandcontext, "targets"), Collections.singleton(ResourceKeyArgument.getRecipe(commandcontext, "recipe")));
        }))).then(net.minecraft.commands.CommandDispatcher.literal("*").executes((commandcontext) -> {
    ...
    private static int giveRecipes(CommandListenerWrapper commandlistenerwrapper, Collection<EntityPlayer> collection, Collection<RecipeHolder<?>> collection1) throws CommandSyntaxException {
        ...
        for (EntityPlayer entityplayer : collection) {
            i += entityplayer.awardRecipes(collection1);
        }
    ...
    private static int takeRecipes(CommandListenerWrapper commandlistenerwrapper, Collection<EntityPlayer> collection, Collection<RecipeHolder<?>> collection1) throws CommandSyntaxException {
        ...
        for (EntityPlayer entityplayer : collection) {
            i += entityplayer.resetRecipes(collection1);
        }
