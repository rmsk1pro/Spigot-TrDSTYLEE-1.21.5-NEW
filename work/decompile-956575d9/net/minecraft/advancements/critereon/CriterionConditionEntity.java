...
public record CriterionConditionEntity(Optional<CriterionConditionEntityType> entityType, Optional<CriterionConditionDistance> distanceToPlayer, Optional<MovementPredicate> movement, CriterionConditionEntity.b location, Optional<CriterionConditionMobEffect> effects, Optional<CriterionConditionNBT> nbt, Optional<CriterionConditionEntityFlags> flags, Optional<CriterionConditionEntityEquipment> equipment, Optional<EntitySubPredicate> subPredicate, Optional<Integer> periodicTick, Optional<CriterionConditionEntity> vehicle, Optional<CriterionConditionEntity> passenger, Optional<CriterionConditionEntity> targetedEntity, Optional<String> team, Optional<SlotsPredicate> slots, DataComponentMatchers components) {
    ...
    public boolean matches(WorldServer worldserver, @Nullable Vec3D vec3d, @Nullable Entity entity) {
        if (entity == null) {
            ...
            if (this.movement.isPresent()) {
                ...
                if (!((MovementPredicate) this.movement.get()).matches(vec3d2.x, vec3d2.y, vec3d2.z, entity.fallDistance)) {
                    return false;
                }
            }
            ...
            if (this.location.located.isPresent() && !((CriterionConditionLocation) this.location.located.get()).matches(worldserver, entity.getX(), entity.getY(), entity.getZ())) {
                ...
                if (this.location.steppingOn.isPresent()) {
                    Vec3D vec3d3 = Vec3D.atCenterOf(entity.getOnPos());

                    if (!entity.onGround() || !((CriterionConditionLocation) this.location.steppingOn.get()).matches(worldserver, vec3d3.x(), vec3d3.y(), vec3d3.z())) {
                        return false;
                    }
                }

                if (this.location.affectsMovement.isPresent()) {
                    Vec3D vec3d4 = Vec3D.atCenterOf(entity.getBlockPosBelowThatAffectsMyMovement());

                    if (!((CriterionConditionLocation) this.location.affectsMovement.get()).matches(worldserver, vec3d4.x(), vec3d4.y(), vec3d4.z())) {
                        return false;
                    }
                }
                ...
                if (this.effects.isPresent() && !((CriterionConditionMobEffect) this.effects.get()).matches(entity)) {
                    ...
                    if (this.slots.isPresent() && !((SlotsPredicate) this.slots.get()).matches(entity)) {
                        ...
                        return this.nbt.isEmpty() || ((CriterionConditionNBT) this.nbt.get()).matches(entity);
                    }
                }
            }
        }
    ...
    public static class a {
        ...
        public CriterionConditionEntity.a of(HolderGetter<EntityTypes<?>> holdergetter, EntityTypes<?> entitytypes) {
            this.entityType = Optional.of(CriterionConditionEntityType.of(holdergetter, entitytypes));
            return this;
        }

        public CriterionConditionEntity.a of(HolderGetter<EntityTypes<?>> holdergetter, TagKey<EntityTypes<?>> tagkey) {
            this.entityType = Optional.of(CriterionConditionEntityType.of(holdergetter, tagkey));
            return this;
        }
        ...
        public CriterionConditionEntity.a components(DataComponentMatchers datacomponentmatchers) {
