...
public abstract class CriterionTriggerAbstract<T extends CriterionTriggerAbstract.a> implements CriterionTrigger<T> {
    ...
    protected void trigger(EntityPlayer entityplayer, Predicate<T> predicate) {
        ...
        if (set != null && !set.isEmpty()) {
            ...
            for (CriterionTrigger.a<T> criteriontrigger_a : set) {
                T t0 = criteriontrigger_a.trigger();
            ...
            if (list != null) {
                for (CriterionTrigger.a<T> criteriontrigger_a1 : list) {
                    criteriontrigger_a1.run(advancementdataplayer);
                }
            }
