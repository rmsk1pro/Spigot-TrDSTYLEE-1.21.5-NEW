...
public record CriterionConditionEntityEquipment(Optional<CriterionConditionItem> head, Optional<CriterionConditionItem> chest, Optional<CriterionConditionItem> legs, Optional<CriterionConditionItem> feet, Optional<CriterionConditionItem> body, Optional<CriterionConditionItem> mainhand, Optional<CriterionConditionItem> offhand) {
    ...
    public static CriterionConditionEntityEquipment captainPredicate(HolderGetter<Item> holdergetter, HolderGetter<EnumBannerPatternType> holdergetter1) {
        return CriterionConditionEntityEquipment.a.equipment().head(CriterionConditionItem.a.item().of(holdergetter, Items.WHITE_BANNER).withComponents(DataComponentMatchers.a.components().exact(DataComponentExactPredicate.someOf(Raid.getOminousBannerInstance(holdergetter1).getComponents(), DataComponents.BANNER_PATTERNS, DataComponents.ITEM_NAME)).build())).build();
    }
