...
public record CriterionConditionPlayer(CriterionConditionValue.IntegerRange level, GameTypePredicate gameType, List<CriterionConditionPlayer.e<?>> stats, Object2BooleanMap<ResourceKey<IRecipe<?>>> recipes, Map<MinecraftKey, CriterionConditionPlayer.c> advancements, Optional<CriterionConditionEntity> lookingAt, Optional<InputPredicate> input) implements EntitySubPredicate {
    ...
    public boolean matches(Entity entity, WorldServer worldserver, @Nullable Vec3D vec3d) {
        if (!(entity instanceof EntityPlayer entityplayer)) {
            ...
            return false;
        } else if (!this.gameType.matches(entityplayer.gameMode())) {
            return false;
        } else {
            StatisticManager statisticmanager = entityplayer.getStats();

            for (CriterionConditionPlayer.e<?> criterionconditionplayer_e : this.stats) {
                if (!criterionconditionplayer_e.matches(statisticmanager)) {
                    return false;
                }
            }
            ...
            while (objectiterator.hasNext()) {
                Object2BooleanMap.Entry<ResourceKey<IRecipe<?>>> object2booleanmap_entry = (Entry) objectiterator.next();

                if (recipebookserver.contains((ResourceKey) object2booleanmap_entry.getKey()) != object2booleanmap_entry.getBooleanValue()) {
                    return false;
                }
            }
            ...
            if (!this.advancements.isEmpty()) {
                ...
                for (Map.Entry<MinecraftKey, CriterionConditionPlayer.c> map_entry : this.advancements.entrySet()) {
                    AdvancementHolder advancementholder = advancementdataworld.get((MinecraftKey) map_entry.getKey());

                    if (advancementholder == null || !((CriterionConditionPlayer.c) map_entry.getValue()).test(advancementdataplayer.getOrStartProgress(advancementholder))) {
                        return false;
                    }
                }
            }
            ...
            if (this.input.isPresent() && !((InputPredicate) this.input.get()).matches(entityplayer.getLastClientInput())) {
                return false;
            } else {
                return true;
            }
    ...
    private static record a(Object2BooleanMap<String> criterions) implements CriterionConditionPlayer.c {
        ...
        public boolean test(AdvancementProgress advancementprogress) {
            ...
            while (objectiterator.hasNext()) {
                ...
                CriterionProgress criterionprogress = advancementprogress.getCriterion((String) object2booleanmap_entry.getKey());
    ...
    public static class d {
        ...
        private final ImmutableList.Builder<CriterionConditionPlayer.e<?>> stats;
        private final Object2BooleanMap<ResourceKey<IRecipe<?>>> recipes;
        ...
        public CriterionConditionPlayer.d addRecipe(ResourceKey<IRecipe<?>> resourcekey, boolean flag) {
            this.recipes.put(resourcekey, flag);
            return this;
        }
        ...
        public CriterionConditionPlayer.d hasInput(InputPredicate inputpredicate) {
            this.input = Optional.of(inputpredicate);
            return this;
        }

        public CriterionConditionPlayer build() {
            return new CriterionConditionPlayer(this.level, this.gameType, this.stats.build(), this.recipes, this.advancements, this.lookingAt, this.input);
        }
    }
}
