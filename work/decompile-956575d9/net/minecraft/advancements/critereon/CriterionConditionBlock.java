...
public record CriterionConditionBlock(Optional<HolderSet<Block>> blocks, Optional<CriterionTriggerProperties> properties, Optional<CriterionConditionNBT> nbt, DataComponentMatchers components) {
    ...
    public static final Codec<CriterionConditionBlock> CODEC = RecordCodecBuilder.create((instance) -> {
        return instance.group(RegistryCodecs.homogeneousList(Registries.BLOCK).optionalFieldOf("blocks").forGetter(CriterionConditionBlock::blocks), CriterionTriggerProperties.CODEC.optionalFieldOf("state").forGetter(CriterionConditionBlock::properties), CriterionConditionNBT.CODEC.optionalFieldOf("nbt").forGetter(CriterionConditionBlock::nbt), DataComponentMatchers.CODEC.forGetter(CriterionConditionBlock::components)).apply(instance, CriterionConditionBlock::new);
    });
    ...
    public boolean matches(WorldServer worldserver, BlockPosition blockposition) {
        if (!worldserver.isLoaded(blockposition)) {
            return false;
        } else if (!this.matchesState(worldserver.getBlockState(blockposition))) {
            return false;
        } else {
            if (this.nbt.isPresent() || !this.components.isEmpty()) {
                TileEntity tileentity = worldserver.getBlockEntity(blockposition);

                if (this.nbt.isPresent() && !matchesBlockEntity(worldserver, tileentity, (CriterionConditionNBT) this.nbt.get())) {
                    return false;
                }

                if (!this.components.isEmpty() && !matchesComponents(tileentity, this.components)) {
                    return false;
                }
            }
    ...
    private static boolean matchesComponents(@Nullable TileEntity tileentity, DataComponentMatchers datacomponentmatchers) {
        return tileentity != null && datacomponentmatchers.test((DataComponentGetter) tileentity.collectComponents());
    }
    ...
    public static class a {
        ...
        public CriterionConditionBlock.a of(HolderGetter<Block> holdergetter, Block... ablock) {
            return this.of(holdergetter, (Collection) Arrays.asList(ablock));
        }

        public CriterionConditionBlock.a of(HolderGetter<Block> holdergetter, Collection<Block> collection) {
            this.blocks = Optional.of(HolderSet.direct(Block::builtInRegistryHolder, collection));
            return this;
        }

        public CriterionConditionBlock.a of(HolderGetter<Block> holdergetter, TagKey<Block> tagkey) {
        ...
        public CriterionConditionBlock.a components(DataComponentMatchers datacomponentmatchers) {
